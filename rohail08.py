    ...
    def send_file(self, chat_id, file_path, caption, entities=None, add_bold=False):
        ...
        try:
            with open(file_path, "rb") as f:
                return _tg_api(
                    "sendPhoto", payload,
                    files={"photo": (os.path.basename(file_path) or "chart.png", f, "image/png")},
                    timeout=30,
                )
        except Exception as e:
            print(f"[SEND FILE] {e}")
...
async def chart_analyzer_photo_handler(
    ...
    try:
        ...
        result = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, _gemini_analyze_chart, bytes(photo_bytes))
