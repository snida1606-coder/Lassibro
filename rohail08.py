...
async def run_backtest_local(
    ...
    for sig in signals:
        ...
        if api in cache:
            continue
        # New source: GitHub raw JSON (10 days of pre-fetched candles per
        # pair). Filename pattern is "{PAIR}_otc.json" — `api` is already
        # built above as e.g. "ETHUSD_otc", so it slots in directly.
        # These files are big (~14k candles) and take ~6s each to download,
        # so timeout is set generously above that.
        url = f"https://raw.githubusercontent.com/ghulammujtabaquotex-cloud/Quotex-server/refs/heads/main/{api}.json"
        try:
...
def generate_pattern_replay_signals(uid, context):
    ...
    try:
        ...
        def fetch_one_pair(pair):
            ...
            # New source: GitHub raw JSON, pre-fetched candles per pair.
            # Filename pattern is "{PAIR}_otc.json" with a lowercase "_otc"
            # suffix (e.g. "ATOUSD_otc.json"), while `pair` here comes in
            # as "AUDCAD_OTC" (uppercase) — normalize before building the URL.
            pair_up = pair.upper()
            gh_pair = pair_up[:-4] + "_otc" if pair_up.endswith("_OTC") else pair_up
            url = f"https://raw.githubusercontent.com/ghulammujtabaquotex-cloud/Quotex-server/refs/heads/main/{gh_pair}.json"
            ...
            for attempt in range(max_retries):
                try:
                    # Files are ~14k candles and take ~6s each to download,
                    # so timeout is generous above that per attempt.
