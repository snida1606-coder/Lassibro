...
def fetch_new_api_candles(pair: str, count: int = 1500, timeframe="M1",
                           retries: int = 2, timeout: int = 8):
    """
    Fetch candles from mrbeaxt.com and normalize the payload so it's a
...
# ================================================================
# Direct fetch from the qxcandledata.beaxtapi.online candle source
# (same provider ST4/Backtest/Live-FS already use). Used specifically
# for Blackout FS's result checker so it verifies against the same
# fresh candle data as everything else, instead of the older
# mrbeaxt.com endpoint that fetch_new_api_candles() still points to.
...
def fetch_beaxt_candles(pair: str, count: int = 2000,
                         retries: int = 2, timeout: int = 8):
    api_pair = to_new_api_pair(pair)
    url = f"https://qxcandledata.beaxtapi.online/candles?pair={api_pair}&timeframe=M1&count={count}"
    for attempt in range(retries):
        try:
            resp = _HTTP.get(url, timeout=timeout)
            if resp.status_code != 200:
...
async def run_advanced_checker(update: Update, context: ContextTypes.DEFAULT_TYPE, signals, mtg_level, date_str, user_utc, payout_filter):
    ...
    def fetch_pair(api):
        result = fetch_beaxt_candles(api, count=fetch_count, retries=3, timeout=25)
        if not result:
...
def run_trend_filter(uid, signals, mode, ema_period, context):
    """
    ...
    pair_candles = {}
    def fetch_pair(pair):
        data = fetch_new_api_candles(pair, count=600, timeframe="M1")
        if data and data.get('candles'):
...
async def run_backtest_local(
    ...
    for sig in signals:
        ...
        # the qxcandledata API's OTC pair format directly.
        url = f"https://qxcandledata.beaxtapi.online/candles?pair={api}&timeframe=M1&count=14000"
        try:
...
def fetch_candles_mrbeaxt(pair: str, count: int = 300, timeframe: str = "1m"):
    """
    Fetch candles using the mrbeaxt.com candle API.
...
def fetch_candle_at_epoch_with_retry(pair, target_epoch, count=50, timeframe="1m",
                                      tolerance=45, attempts=8, delay=1.2):
...
async def start_multi_nonstop(update: Update, context: ContextTypes.DEFAULT_TYPE, pairs: list):
    ...
    thread = threading.Thread(target=multi_nonstop_loop, args=(uid, context, multi_session_token), daemon=True)
    thread.start()
...
async def start_multi_nonstop_with_strategy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if filter_type == 'manual':
        ...
        pairs = MULTI_ENGINE_LIVE
    else:
        pairs = MULTI_ENGINE_OTC + MULTI_ENGINE_LIVE
    ...
    thread = threading.Thread(
        ...
        args=(uid, context, multi_session_token, strategy),  # ✅ strategy passed here
        daemon=True
    )
    thread.start()

def multi_nonstop_loop(uid, context, session_token, strategy_type=1):
        ...
        while context.user_data.get('multi_running', False):
            try:
                ...
                # ===== Strategy 02 =====
                    ...
                    ticks = fetch_live_ticks(pair, limit=50)
...
def detect_pullback_with_ticks(candles, ticks, pair="UNKNOWN"):
    ...
    check_ticks = ticks[-10:] if len(ticks) > 10 else ticks
...
def fetch_live_ticks(pair, limit=50):
    """Fetch live ticks from beaxtapi"""
    api_pair = to_new_api_pair(pair)
    url = f"https://qxcandledata.beaxtapi.online/ticks?pair={api_pair}&limit={limit}"
...
def _run_livefs_worker(uid: int, context):
    ...
    for idx, pair in enumerate(pairs, 1):
        ...
        url = f"https://qxcandledata.beaxtapi.online/candles?pair={pair}&timeframe=M1&count=14000"
        success = False
        candles = []
...
def run_livefs_checker(uid, date_str, mtg_level, signals_text, context, payout_filter=False):
    """
    ...
    def fetch_pair(pair):
        result = fetch_beaxt_candles(pair, count=fetch_count, retries=3, timeout=25)
        if not result:
            return pair, None
        candles = result.get('candles', [])
        candle_by_time = {}
    ...
    # ==================== MULTI ENGINE ====================
       ...
       if pair_data is None:
          ...
          async def fetch_payout(p):
              _, _, payout = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, fetch_candles_mrbeaxt, p, 1)
              return p, payout
          tasks = [fetch_payout(p) for p in pairs]
          results = await asyncio.gather(*tasks)
          pair_data = [(pair, payout) for pair, payout in results]
...
def generate_pattern_replay_signals(uid, context):
    ...
    try:
        ...
        def fetch_one_pair(pair):
            """Fetch full candle history for a pair from the qxcandledata
            (beaxtapi) API — replaces the old GitHub raw-JSON source.
            ...
            api_pair = to_new_api_pair(pair)
            url = f"https://qxcandledata.beaxtapi.online/candles?pair={api_pair}&timeframe=M1&count=14000"
