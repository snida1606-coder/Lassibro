...
def fetch_new_api_candles(pair: str, count: int = 1500, timeframe="M1",
    ...
    if broker == "pocket":
        url = f"{POCKET_CANDLE_API_BASE}?pair={api_pair}&timeframe={api_tf}&count={count}"
    elif broker == "tradowix":
        url = f"{TRADOWIX_CANDLE_API_BASE}?pair={api_pair}&timeframe={api_tf}&count={count}"
    else:
...
def fetch_beaxt_candles(pair: str, count: int = 2000,
    ...
    api_pair = to_new_api_pair(pair)
    if broker == "pocket":
        url = f"{POCKET_CANDLE_API_BASE}?pair={api_pair}&timeframe=M1&count={count}"
    elif broker == "tradowix":
        url = f"{TRADOWIX_CANDLE_API_BASE}?pair={api_pair}&timeframe=M1&count={count}"
    else:
        url = f"{NEW_CANDLE_API_BASE}?pair={api_pair}&timeframe=M1&count={count}"
    for attempt in range(retries):
        try:
            resp = _HTTP.get(url, timeout=timeout)
            if resp.status_code != 200:
...
async def run_backtest_local(
    ...
    for sig in signals:
        ...
        if broker == "pocket":
            url = f"{POCKET_CANDLE_API_BASE}?pair={api}&timeframe=M1&count={candle_count}"
        elif broker == "tradowix":
            url = f"{TRADOWIX_CANDLE_API_BASE}?pair={api}&timeframe=M1&count={candle_count}"
        else:
            url = f"{NEW_CANDLE_API_BASE}?pair={api}&timeframe=M1&count={candle_count}"
        try:
...
def _run_livefs_worker(uid: int, context):
    ...
    for idx, pair in enumerate(pairs, 1):
        ...
        if broker == "pocket":
            url = f"{POCKET_CANDLE_API_BASE}?pair={pair}&timeframe=M1&count=14000"
        elif broker == "tradowix":
            url = f"{TRADOWIX_CANDLE_API_BASE}?pair={pair}&timeframe=M1&count=14000"
        else:
            url = f"{NEW_CANDLE_API_BASE}?pair={pair}&timeframe=M1&count=14000"
        success = False
        candles = []
...
def generate_pattern_replay_signals(uid, context):
    ...
    try:
        ...
        def fetch_one_pair(pair):
            ...
            api_pair = to_new_api_pair(pair)
            url = f"{fut4_candle_api_base(broker)}?pair={api_pair}&timeframe=M1&count=14000"
