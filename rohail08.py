        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                try:
                    ...
                    pass
                time.sleep(min(retry_after + 0.15, 8))
                continue
            try:
                body = resp.json()
            except Exception:
        ...
        async with sem:
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    # (uncapped, but bounded to something sane) rather than
                    # just 5s — under-waiting here is what causes repeat
                    # 429s that burn through the retry budget.
                    if "retry after" in err_str:
                        try:
                            ...
                            wait = 2 ** attempt
                        print(f"[BROADCAST] Rate limited for {uid}, waiting {wait}s")
                        await asyncio.sleep(wait)
                        continue
