...
from telegram import (
    Update, InlineKeyboardButton as _PTBInlineKeyboardButton,
...
def _style_value(style):
    ...
    if isinstance(style, str):
        return style
    return getattr(style, "value", None) or str(style)


def _button_to_raw(btn):
    """Raw button dict: text + callback/url + style + premium icon."""
    if btn is None:
        return None
    if isinstance(btn, dict):
        data = dict(btn)
    elif hasattr(btn, "to_dict"):
        data = dict(btn.to_dict() or {})
    else:
        data = {}
    extra = {}
    for name in ("api_kwargs", "_api_kwargs"):
        ak = getattr(btn, name, None)
        if isinstance(ak, dict):
            extra.update(ak)
    style = _style_value(extra.get("style") or data.get("style"))
    icon = extra.get("icon_custom_emoji_id") or data.get("icon_custom_emoji_id")
    if style:
        data["style"] = style
    if icon:
        data["icon_custom_emoji_id"] = str(icon)
    return data


def _markup_to_raw(markup):
    ...
    if isinstance(markup, dict):
        if "inline_keyboard" in markup:
            return {
                "inline_keyboard": [
                    [_button_to_raw(b) for b in row] for row in markup["inline_keyboard"]
                ]
            }
        return markup
    ...
    if keyboard is None:
        if hasattr(markup, "to_dict"):
            return _markup_to_raw(markup.to_dict())
        return None
    return {
        "inline_keyboard": [
            [_button_to_raw(b) for b in row] for row in keyboard
        ]
    }


class InlineKeyboardButton(_PTBInlineKeyboardButton):
    """Colored buttons + premium icon_custom_emoji_id."""

    def __init__(self, text, *args, style=None, icon_custom_emoji_id=None, **kwargs):
        extra = dict(kwargs.pop("api_kwargs", None) or {})
        sval = _style_value(style)
        if sval:
            extra["style"] = sval
    ...
    def to_dict(self, *args, **kwargs):
        ...
        extra = {}
        for name in ("api_kwargs", "_api_kwargs"):
            ak = getattr(self, name, None)
            if isinstance(ak, dict):
                extra.update(ak)
        style = _style_value(extra.get("style"))
        icon = extra.get("icon_custom_emoji_id")
        if style:
            data["style"] = style
        if icon:
            data["icon_custom_emoji_id"] = str(icon)
...
class InlineKeyboardMarkup(_PTBInlineKeyboardMarkup):
    def to_dict(self, *args, **kwargs):
        return _markup_to_raw(self) or {"inline_keyboard": []}
...
BOT_APP = None      # the python-telegram-bot Application, set in main() — used by reset_user_state() to drop a user's context.user_data

async def safe_query_answer(query, *args, **kwargs):
    """Wrapper around `await query.answer()` used everywhere in this file.

    A Telegram callback query expires ~15-30s after the button was tapped.
    If the bot is even briefly congested (slow Supabase check, send-queue
    backlog, etc.) an update can sit long enough that by the time we get to
    it, answering the query raises BadRequest("Query is too old..."). This
    file registers no global PTB error handler, so that exception used to
    silently abort WHATEVER handler was running it — the user's button
    press would just do nothing, no menu, no reply, no visible error
    (this is what was making Strategy/MM/OTC buttons look "stuck").
    Swallowing just this one specific, harmless failure lets every handler
    carry on and still send its actual reply.
    """
    try:
        return await query.answer(*args, **kwargs)
    except BadRequest as e:
        print(f"[QUERY ANSWER SKIP] {e}")
        return None
import concurrent.futures as _cf
from requests.adapters import HTTPAdapter

_HTTP = requests.Session()
_HTTP.mount("https://", HTTPAdapter(pool_connections=100, pool_maxsize=100))
_HTTP.mount("http://", HTTPAdapter(pool_connections=16, pool_maxsize=16))
_HTTP.headers.update({"User-Agent": "SMZX-Bot/2.0"})

_cpu = os.cpu_count() or 2
_CHART_RENDER_EXECUTOR = _cf.ThreadPoolExecutor(
    max_workers=4, thread_name_prefix="chart-render")
_AUTH_EXECUTOR = _cf.ThreadPoolExecutor(
    max_workers=16, thread_name_prefix="auth-check")
_NETWORK_EXECUTOR = _cf.ThreadPoolExecutor(
    max_workers=100, thread_name_prefix="net-io")

_send_pacing_lock = threading.Lock()
_last_send_per_chat = {}
_last_global_send_time = [0.0]
_GLOBAL_MIN_INTERVAL = 1.0 / 28.0
_PER_CHAT_MIN_INTERVAL = 0.05


class _TgMsg:
    """Minimal stand-in for PTB Message so callers can use .id / .chat_id."""
    __slots__ = ("id", "message_id", "chat_id", "chat", "_raw")

    def __init__(self, raw=None):
        raw = raw or {}
        self._raw = raw
        self.id = raw.get("message_id")
        self.message_id = self.id
        chat = raw.get("chat") or {}
        self.chat_id = chat.get("id")
        self.chat = type("Chat", (), {"id": self.chat_id})()


def _tg_obj_to_dict(obj):
    if obj is None:
        return None
    if hasattr(obj, "inline_keyboard") or (isinstance(obj, dict) and "inline_keyboard" in obj):
        return _markup_to_raw(obj)
    if hasattr(obj, "to_dict"):
        return obj.to_dict()
    if isinstance(obj, (list, tuple)):
        return [_tg_obj_to_dict(x) for x in obj]
    return obj


def _pace_send(chat_id):
    with _send_pacing_lock:
        ...
        if len(_last_send_per_chat) > 4000:
            cutoff = t_now - 120
            stale = [k for k, v in _last_send_per_chat.items() if v < cutoff]
            for k in stale:
                _last_send_per_chat.pop(k, None)


def _tg_token():
    return BOT_TOKEN or os.environ.get("BOT_TOKEN") or ""


def _tg_api(method, payload=None, files=None, timeout=20, max_retries=4):
    token = _tg_token()
    if not token:
        print("[TG API] BOT_TOKEN missing")
        return None
    ...
    for attempt in range(max_retries):
        try:
            ...
            if files:
                ...
                resp = _HTTP.post(url, json=payload or {}, timeout=timeout)
            if resp.status_code == 429:
                ...
                try:
                    ...
                    pass
                # Previously capped at 8s regardless of what Telegram asked
                # for — under-waiting a real 429 and retrying immediately is
                # what turns a normal per-chat rate limit into a full
                # bot-token flood block. Honor the real value (bounded to a
                # sane ceiling so one bad response can't hang a thread for
                # hours) instead of always retrying almost immediately.
                time.sleep(min(retry_after + 0.15, 60))
                continue
            try:
                body = resp.json()
            except Exception:
                print(f"[TG API] {method} non-json HTTP {resp.status_code}")
                time.sleep(0.3 * (attempt + 1))
                continue
            if body.get("ok"):
                raw = body.get("result")
                return _TgMsg(raw) if isinstance(raw, dict) else raw
            ...
            if "too many requests" in low:
                ...
                continue
            print(f"[TG API] {method} fail: {desc}")
            return None
        except Exception as e:
            print(f"[TG API] {method} error: {type(e).__name__}: {e}")
            time.sleep(0.35 * (attempt + 1))
    return None


def _enqueue_send(chat_id, coro_factory, timeout=20, max_retries=4):
    """Legacy wrapper for leftover async PTB factories.
    NEVER blocks MAIN_LOOP (that was the old deadlock)."""
    try:
        running = asyncio.get_running_loop()
    except RuntimeError:
        running = None
    if running is not None and running is MAIN_LOOP:
        MAIN_LOOP.create_task(coro_factory())
        return None
    if MAIN_LOOP is None:
        return None
    try:
        fut = asyncio.run_coroutine_threadsafe(coro_factory(), MAIN_LOOP)
        return fut.result(timeout=timeout)
    except Exception as e:
        print(f"[LEGACY SEND] chat={chat_id}: {type(e).__name__}: {e}")
        return None
...
def run_uptime_server():
    """Railway/Render healthcheck — bind PORT without Flask."""
    try:
        from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

        class _Health(BaseHTTPRequestHandler):
            def do_GET(self):
                body = b"SMZX BOT IS LIVE"
        ...
        httpd = ThreadingHTTPServer(("0.0.0.0", port), _Health)
...
threading.Thread(target=run_uptime_server, daemon=True, name="health-http").start()

def force_reset_user_session(uid, context):
    """
    Forcefully stop ALL running sessions for a user.
    Handles missing attributes safely.
    """
    # 1. Main Signal State
    st = get_state(uid)
    st.running = False
    ...
    # 2. Auto Trade State (use get_auto_trader)
    try:
        at = get_auto_trader(uid)
        at.running = False
        at.paused = False
        # If there is a thread, we can't kill it, but setting running=False will stop the loop
        # Optionally, we could try to join, but avoid blocking
    except Exception:
        pass  # Auto trade might not be initialized
    
    # 3. Multi Engine State
    ...
    # 6. Reset any other flags that may be stuck
    context.user_data.pop('ai_filter_signals', None)
    context.user_data.pop('ai_filter_signals', None)  # avoid duplicate
    
    print(f"[FORCE RESET] User {uid} session forcefully stopped.")
...
class TradoWixClient:
    ...
    def __init__(
        ...
        # FIX: watchdog thread that restarts the WS background thread if it
        # ever dies unexpectedly (belt-and-suspenders alongside the
        # broadened exception handling in _ws_main_loop).
    ...
    def connect(self, blocking: bool = False):
        ...
        if blocking:
            ...
            # FIX: start watchdog once, alongside the WS thread, so that if
            # the WS thread ever dies (any uncaught exception, OS-level
            # thread crash, etc.) it gets automatically restarted instead
            # of silently leaving this user with a dead price feed forever.
    ...
    def _watchdog_loop(self, check_interval: float = 15.0):
        """
        Runs for the lifetime of this client. Periodically checks whether
        the WS background thread is still alive. If it has died while we
        are not intentionally disconnected (_stop_event not set), it
        restarts the connection from scratch (fresh event loop, fresh
        thread). This is a safety net independent of the reconnect logic
        inside _ws_main_loop, so even a bug we haven't anticipated can't
        leave a user's feed dead for hours.
        """
        while not self._stop_event.is_set():
            ...
            if thread is None or not thread.is_alive():
                logger.error(
                    "WS watchdog: background thread found dead, restarting connection")
                try:
                    self._connected = False
                    self._authenticated = False
                    self._ws_thread = threading.Thread(
                    ...
                    logger.error("WS watchdog: failed to restart: %s", e)
    ...
    async def _ws_main_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # FIX: any other exception (bad handshake, protocol error,
                # unexpected server payload, DNS hiccup, etc.) used to fall
                # through uncaught here, which killed _ws_main_loop and, in
                # turn, silently ended the whole background thread with no
                # retry. That was the cause of a handful of users' price
                # feed going permanently dead until the whole process
                # restarted. Now we treat it the same as a disconnect and
                # keep retrying.
                logger.error(
                    "WebSocket unexpected error: %s. Reconnecting in %ds...",
                    e,
                    retry_delay)
                self._connected = False
                self._authenticated = False
...
# ================================================================
#   NEW CANDLE DATA API — replaces gm-vlni.onrender.com
#   PRIMARY: qxcandledata.beaxtapi.online, FALLBACK: mrbeaxt.com
#   (swapped from the original mrbeaxt-primary setup)
#   Differences vs the old API, handled here so nothing downstream
#   has to change:
#     1) Candles come back NEWEST-FIRST (index 0 = latest).
#        We reverse it -> oldest-first, so candles[-1] is still "latest"
#        exactly like every existing function in this file expects.
#     2) The 'epoch' field is a normal UTC unix timestamp
...
# ================================================================
NEW_CANDLE_API_BASE = "https://qxcandledata.beaxtapi.online/api/v1/candles"
# Quotex live-tick endpoint on the mrbeaxt.com API, used everywhere
# the Quotex (default) branch needs raw tick data instead of candles.
NEW_TICK_API_BASE = "https://mrbeaxt.com/Qxapi/qx_ticks.php"
# Backup Quotex candle source — same {'success','data':[{epoch,open,high,
# low,close,payout,...}]} response shape as the primary (newest-first), so
# it's a drop-in fallback. Every Quotex candle-fetch site in this file
# automatically shifts to this URL whenever the primary qxcandledata API
# above is unreachable or returns no usable data.
NEW_CANDLE_API_FALLBACK_BASE = "https://mrbeaxt.com/Qxapi/qx.php"
...
def to_new_api_pair(pair: str) -> str:
    ...
    mrbeaxt.com uses two different endpoints/formats:
...
def fetch_new_api_candles(pair: str, count: int = 1500, timeframe="M1",
                           retries: int = 2, timeout: int = 8, broker: str = "quotex"):
    """
    Fetch candles from mrbeaxt.com (Quotex) or pocandledata.beaxtapi.Online
    (Pocket Option) and normalize the payload so it's a drop-in replacement
    for the old gm-vlni response:
    ...
    NOTE on retries/timeout: this function has its OWN internal retry loop,
    and it's called from inside OTHER retry loops (e.g. once per pair while
    scanning ~50 OTC pairs, and once per attempt inside the 25-attempt
    candle-verification retry). The old defaults (retries=3, timeout=20)
    meant a single slow/unresponsive API call could eat up to ~60s *by
    itself* — across 50 scanned pairs, or across 25 verification attempts,
    that compounds into many minutes of the bot looking completely stuck
    ("0% Scanning" not moving) even though nothing had actually crashed.
    A normal successful call takes well under a second, so 8s×2 is still
    generous for real slowness while capping the worst case per call to
    ~17s instead of ~60s.
    """
    api_pair = to_new_api_pair(pair)
    api_tf = to_new_api_timeframe(timeframe)
    if broker == "pocket":
        urls_to_try = [f"{POCKET_CANDLE_API_BASE}?pair={api_pair}&timeframe={api_tf}&count={count}"] * retries
    elif broker == "tradowix":
        urls_to_try = [f"{TRADOWIX_CANDLE_API_BASE}?pair={api_pair}&timeframe={api_tf}&count={count}"] * retries
    else:
        # Quotex: interleave qxcandledata.beaxtapi.online (primary) with the
        # mrbeaxt.com backup — primary, fallback, primary,
        # fallback, ... — instead of exhausting primary first. If primary
        # has gone slow (not down, just high-latency), this lets fallback
        # get a chance much sooner, roughly halving worst-case time in that
        # scenario instead of always paying the full `retries` cost on a
        # struggling primary before ever trying the backup.
        primary_url = f"{NEW_CANDLE_API_BASE}?pair={api_pair}&timeframe={api_tf}&count={count}"
        fallback_url = f"{NEW_CANDLE_API_FALLBACK_BASE}?pair={api_pair}&timeframe={api_tf}&count={count}"
        urls_to_try = [u for pair_urls in zip([primary_url] * retries, [fallback_url] * retries) for u in pair_urls]
    for attempt, url in enumerate(urls_to_try):
        try:
            ...
            if not data.get('success') or not data.get('data'):
                continue
            raw = data['data']  # newest-first from mrbeaxt.com
            candles = []
            for c in reversed(raw):  # -> oldest-first
                epoch = c.get('epoch')
                if epoch is None:
                    continue
                # mrbeaxt.com (Quotex) readable time is UTC+6; pocandledata
                # (Pocket Option) and trwcandledata (Tradowix) follow the
                # same +5 convention as the rest of this bot's beaxtapi
                # integrations (Strategy 4 / Checker).
                tz_shift = 5 if broker in ("pocket", "tradowix") else 6
                dt_local = datetime.fromtimestamp(
                    epoch, tz=timezone.utc) + timedelta(hours=tz_shift)
                candles.append({
                    ...
                    'readable_time': dt_local.strftime(", %H:%M:"),
                })
            if not candles:
                continue
...
# ================================================================
# Direct fetch from the same mrbeaxt.com (Quotex) / pocandledata /
# trwcandledata candle providers used across ST4/Backtest/Live-FS.
# Returns the exact same {'candles': [...], 'payout': ...} shape as
# fetch_new_api_candles() so it's a drop-in replacement at call sites.
# ================================================================
def fetch_beaxt_candles(pair: str, count: int = 2000,
                         retries: int = 2, timeout: int = 8, broker: str = "quotex"):
    api_pair = to_new_api_pair(pair)
    if broker == "pocket":
        urls_to_try = [f"{POCKET_CANDLE_API_BASE}?pair={api_pair}&timeframe=M1&count={count}"] * retries
    elif broker == "tradowix":
        urls_to_try = [f"{TRADOWIX_CANDLE_API_BASE}?pair={api_pair}&timeframe=M1&count={count}"] * retries
    else:
        # Quotex: interleave qxcandledata.beaxtapi.online (primary) with the
        # mrbeaxt.com backup instead of exhausting primary
        # first — see fetch_new_api_candles() for the rationale.
        primary_url = f"{NEW_CANDLE_API_BASE}?pair={api_pair}&timeframe=M1&count={count}"
        fallback_url = f"{NEW_CANDLE_API_FALLBACK_BASE}?pair={api_pair}&timeframe=M1&count={count}"
        urls_to_try = [u for pair_urls in zip([primary_url] * retries, [fallback_url] * retries) for u in pair_urls]
    for attempt, url in enumerate(urls_to_try):
        try:
            resp = _HTTP.get(url, timeout=timeout)
            ...
            if not data.get('success') or not data.get('data'):
                continue
            raw = data['data']  # newest-first
            candles = []
            for c in reversed(raw):  # -> oldest-first
                epoch = c.get('epoch')
                if epoch is None:
                    continue
                # 'epoch' is a true UTC unix timestamp (same convention as
                # ST4/Backtest/Live-FS); readable Pakistan time = epoch + 5h.
                dt_pk = datetime.fromtimestamp(
                    epoch, tz=timezone.utc) + timedelta(hours=5)
                candles.append({
                    'time': epoch,  # raw UTC epoch — matches fetch_new_api_candles convention
                    'open': c.get('open'),
                    'high': c.get('high'),
                    'low': c.get('low'),
                    'close': c.get('close'),
                    'volume': c.get('volume', 1),
                    'payout': c.get('payout', 92),
                    'readable_time': dt_pk.strftime(", %H:%M:"),
                })
            if not candles:
                continue
            latest_payout = str(candles[-1].get('payout', 92))
            ...
            print(f"[BEAXT-API] fetch error for {pair}: {e}")
            time.sleep(0.2)
    return None
# ================================================================
...
# ================================================================
# MAINTENANCE MODE (owner-toggled)
# When "on", every authorized user's button click is intercepted and
# replaced with the owner's custom message + ETA — instead of running
# the normal action. Unauthorized users are completely unaffected: they
# keep seeing whatever they see today (contact-owner flow etc.).
# In-memory only (resets on restart) by design — this is meant for
# short, ad-hoc "we know we're broken right now" windows, not a
# persistent config.
# ================================================================
MAINTENANCE_MODE = {"on": False, "message": "", "eta": ""}
_MAINTENANCE_LAST_NOTICE = {}   # uid -> last time (monotonic) we sent them the notice
_MAINTENANCE_NOTICE_COOLDOWN = 20  # seconds — so rapid re-clicking doesn't spam sends

try:
    import news as news_mod
except Exception as _news_imp_err:
    print(f"[News] import failed: {_news_imp_err}")
    news_mod = None

LEADERBOARD_EMOJI_IDS = {
    "👑": "6102842622865318037",
    "📈": "6102609195687747429",
    "📋": "6102862147786645919",
    "✅": "6102561165068476007",
...
LEADERBOARD_MEDAL_IDS = {
    "🥇": "6219812792996731656",
    "🥈": "5447203607294265305",
    "🥉": "6104729414883348710",
    "4️⃣": "5375190543477260772",
    "5️⃣": "5375120574165040899",
}
LEADERBOARD_BUTTON_EMOJI_ID = "6102842622865318037"
LEADERBOARD_REFRESH_EMOJI_ID = "5346321684574003384"


def build_leaderboard_entities(text: str) -> list:
    """Custom emoji + bold entities for the leaderboard message specifically.
    Kept separate from the global PREMIUM_EMOJI_IDS/build_custom_emoji_entities
    on purpose: several of these characters (👑 📈 📋 ✅ ❌) already map to
    DIFFERENT premium emoji ids elsewhere in the bot, so reusing the global
    dict here would have silently changed those other messages' icons too.
    """
    entities = list(build_bold_entities(text, ["Top Traders — Today"]))
    i = 0
    n = len(text)
    while i < n:
        matched_seq = None
        for seq, eid in LEADERBOARD_MEDAL_IDS.items():
        ...
        if matched_seq:
            ...
            length = len(seq.encode('utf-16-le')) // 2
            entities.append(MessageEntity(type='custom_emoji', offset=offset, length=length, custom_emoji_id=eid))
            i += len(seq)
            continue
        ch = text[i]
        eid = LEADERBOARD_EMOJI_IDS.get(ch)
        if eid:
            offset = len(text[:i].encode('utf-16-le')) // 2
...
def record_trade_hour(uid: int, broker: str, won: bool) -> None:
    """Fire-and-forget: logs one trade result (with broker + exact
    timestamp) to Supabase for the ⏰ Best Trading Hours report. Separate
    RPC/table from the leaderboard's record_trade_result() — this one
    needs per-trade timestamps + broker to support variable day-ranges
    and per-broker filtering, which the leaderboard's simple aggregate
    counters can't do."""
    def _send():
        headers = {
        ...
        url = f"{SUPABASE_URL}/rest/v1/rpc/record_trade_hour"
        try:
            _HTTP.post(
                url, headers=headers,
                json={"p_telegram_id": uid, "p_broker": broker or "quotex", "p_won": bool(won)},
                timeout=5,
            )
        except Exception as e:
            print(f"[BEST HOURS] record_trade_hour failed for {uid}: {e}")
    threading.Thread(target=_send, daemon=True).start()


def fetch_hourly_stats(broker: str, days: int, telegram_id=None):
    """Live (uncached) fetch via the get_hourly_stats() Supabase RPC.
    Returns a list of {hour_pkt, wins, losses} dicts, or None on failure."""
    headers = {
        "apikey": SUPABASE_ANON_KEY,
        "Authorization": f"Bearer {SUPABASE_ANON_KEY}",
        "Content-Type": "application/json",
    }
    url = f"{SUPABASE_URL}/rest/v1/rpc/get_hourly_stats"
    payload = {"p_broker": broker, "p_days": days}
    if telegram_id is not None:
        payload["p_telegram_id"] = telegram_id
    try:
        resp = _HTTP.post(url, headers=headers, json=payload, timeout=8)
        if resp.status_code != 200:
            print(f"[BEST HOURS] fetch failed HTTP {resp.status_code}: {resp.text[:200]}")
            return None
        return resp.json()
    except Exception as e:
        print(f"[BEST HOURS] fetch error: {e}")
        return None


_BROKER_DISPLAY = {"quotex": "Quotex", "pocket": "Pocket Option", "tradowix": "Tradowix"}
_HOURS_MIN_TRADES = 10  # an hour bucket needs at least this many trades to be ranked

HOURS_HEADER_EMOJI_IDS = {
    "⏰": "5215327832040811010",
    "📅": "5208561604987208729",
    "🌍": "5287292843763713628",
    "🚫": "5936025362944233351",
}
HOURS_WARN_ID = "6230838304758046212"   # ⚠️ (weak-hour line marker)
HOURS_BROKER_ICON_IDS = {
    "quotex": "6217713374327738118",
    "pocket": "5258033997535012714",
    "tradowix": "6337101525147458464",
}

# ----- AI Filter FS result message (custom emoji + bold house style) -----
AI_FILTER_EMOJI_IDS = {
    "🤖": "6102709517533847037",   # header icon (used before + after title)
    "🖥": "6323287102142292336",   # System line
    "📊": "6325824147913975037",   # Original count
    "✅": "6325662644258743099",   # Passed count
    "❌": "6325476251268031672",   # Failed count
    "📋": "6102449465854008793",   # "Passed Signals:" section header
    "💀": "6102488215048952369",   # footer start
    "👻": "6113676493185882466",   # footer end
}


def build_ai_filter_header(system_label: str, original_count: int, passed_count: int, failed_count: int) -> list:
    """Header lines shared by every chunk of the AI Filter results (title + run stats)."""
    return [
        f"🤖{fancy_font('SMZX BOT AI FILTER')}🤖",
        "",
        f"🖥 System : {system_label}",
        f"📊 Original: {original_count} signals",
        f"✅ Passed : {passed_count} signals",
        f"❌ Failed : {failed_count} signals",
        "",
    ]


def build_ai_filter_signal_lines(section_title: str, signals: list, empty_label: str) -> list:
    """A section header + fancy-font signal lines (or an empty-state line).
    fancy_font() turns ':' into '：' (fullwidth) and '-' into '—' (em dash) —
    fine for prose, but it makes times/pair-names look gappy/wrong ("03:  32",
    "USDPKR —OTC"), so those two are converted back to plain ':' and '-' here."""
    lines = [section_title, ""]
    if signals:
        for sig in signals:
            styled = fancy_font(sig.replace("-OTC", " -OTC"))
            styled = styled.replace("：", ":").replace("—", "-")
            lines.append(styled)
    else:
        lines.append(fancy_font(empty_label))
    return lines


def build_ai_filter_entities(text: str) -> list:
    """Whole message is bold, plus custom-emoji markers at each icon position.
    Length must be computed in UTF-16 code units (not len(ch)) because emojis
    like 🤖 💀 👻 📋 🖥 📊 are astral-plane characters that take 2 UTF-16 units —
    a hardcoded length=1 splits the surrogate pair and Telegram rejects the
    whole message with 'ends in a middle of a UTF-16 symbol'."""
    entities = [MessageEntity(type='bold', offset=0, length=len(text.encode('utf-16-le')) // 2)]
    i = 0
    n = len(text)
    while i < n:
        ch = text[i]
        eid = AI_FILTER_EMOJI_IDS.get(ch)
        if eid:
            offset = len(text[:i].encode('utf-16-le')) // 2
            length = len(ch.encode('utf-16-le')) // 2
            entities.append(MessageEntity(type='custom_emoji', offset=offset, length=length, custom_emoji_id=eid))
...
_AI_FILTER_MAX_CHARS = 3500  # safety margin below Telegram's 4096 UTF-16 limit per message


def send_ai_filter_section(sender, uid, header_lines: list, body_lines: list, footer_lines: list):
    """Sends a styled section (header + a list of signal lines + optional
    footer) as one or more Telegram messages, each staying under Telegram's
    4096-char cap. Splits only between whole signal lines — never mid-line —
    and re-applies the header to every chunk so each message stands alone and
    is still fully readable/bold/emoji-styled on its own."""
    def _text_len(lines):
        return len("\n".join(lines).encode('utf-16-le')) // 2

    chunks = []
    current = list(header_lines)
    for line in body_lines:
        trial = current + [line]
        if _text_len(trial) > _AI_FILTER_MAX_CHARS and len(current) > len(header_lines):
    ...
    if footer_lines:
        chunks[-1] = chunks[-1] + [""] + footer_lines

    for chunk_lines in chunks:
        text = "\n".join(chunk_lines)
        entities = build_ai_filter_entities(text)
        sender.send_message(uid, text, entities=entities)


def build_best_hours_entities(text: str, broker: str) -> list:
    """Custom emoji entities for the Best Trading Hours message. The 🔌
    broker icon is conditional on which broker the report is for; 📈 📋
    ✅ ❌ and the medal icons reuse the exact same ids as the leaderboard
    (per spec: 'same mapping we did in leaderboard option')."""
    char_map = dict(LEADERBOARD_EMOJI_IDS)  # 📈 📋 ✅ ❌ (👑 unused here)
    char_map.update(HOURS_HEADER_EMOJI_IDS)
    broker_icon_id = HOURS_BROKER_ICON_IDS.get(broker)
    if broker_icon_id:
        char_map["🔌"] = broker_icon_id
    ...
    while i < n:
        ...
        if matched_seq:
            ...
            i += len(seq)
            continue
        if text.startswith("⚠️", i):  # warning sign + variation selector = 2 codepoints
            offset = len(text[:i].encode('utf-16-le')) // 2
            length = len("⚠️".encode('utf-16-le')) // 2
            entities.append(MessageEntity(type='custom_emoji', offset=offset, length=length, custom_emoji_id=HOURS_WARN_ID))
            i += len("⚠️")
            continue
        ch = text[i]
        eid = char_map.get(ch)
...
def build_best_hours_message(broker: str, days: int, telegram_id=None) -> str:
    rows = fetch_hourly_stats(broker, days, telegram_id)
    broker_label = _BROKER_DISPLAY.get(broker, broker.title())
    today_pkt = (datetime.now(timezone.utc) + timedelta(hours=5)).date()
    start_pkt = today_pkt - timedelta(days=days - 1)
    date_range = f"{start_pkt.isoformat()} → {today_pkt.isoformat()}"
    day_label = "Last 1 day" if days == 1 else f"Last {days} days"
    scope_label = "👤 Your Trades" if telegram_id is not None else "🌍 All Users Trades"

    header = (
        f"⏰ Best Trading Hours\n"
        f"📅 {day_label}  ·  {date_range}\n"
        f"🔌 {broker_label}  ·  {scope_label}\n"
        f"✺━━━━・━━━━✺✺━━━━・━━━✺\n"
    )

    if rows is None:
        return header + "\n⚠️ Data load nahi ho saka. Thodi der baad try karein."
    ...
    for r in rows:
        ...
        total = wins + losses
        if total < _HOURS_MIN_TRADES:
            continue
        buckets.append({
            "hour": r.get("hour_pkt", 0),
            "wins": wins, "losses": losses, "total": total,
            "rate": round(100.0 * wins / total, 0) if total else 0,
        })

    if not buckets:
        scope_note = "Aapke" if telegram_id is not None else "Community ke"
        return header + f"\n🔍 {scope_note} paas abhi is range/broker me kaafi data nahi hai (min {_HOURS_MIN_TRADES} trades/hour chahiye). Trade karte rahein aur dobara check karein!"

    buckets.sort(key=lambda b: (-b["rate"], -b["total"]))
    best = buckets[:5]
    worst = sorted(buckets, key=lambda b: (b["rate"], -b["total"]))[:3]
    worst = [b for b in worst if b not in best]  # don't repeat an hour in both lists

    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
    lines = [header, f"{scope_label}"]
    for i, b in enumerate(best):
        h = b["hour"]
        lines.append(f"{medals[i]}{h:02d}:00–{(h+1)%24:02d}:00")
        lines.append(f"   📈{b['rate']:.0f}%  ✅ {b['wins']}  ❌ {b['losses']}  📋 {b['total']} trades")
    ...
    if worst:
        ...
        for b in worst:
            h = b["hour"]
            lines.append(f"⚠️{h:02d}:00–{(h+1)%24:02d}:00  →  {b['rate']:.0f}%  ✅ {b['wins']}  ❌ {b['losses']}")

    lines.append("")
    lines.append("All hours shown in UTC+5 (PKT).")
    lines.append("✺━━━━・━━━━✺✺━━━━・━━━✺")
    return "\n".join(lines)


def record_trade_result(uid: int, won: bool) -> None:
    """Fire-and-forget: logs one trade result to Supabase for the 🏆
    leaderboard. Calls the record_trade_result() RPC (atomic upsert with
    lazy PKT-date daily reset — see leaderboard_supabase.sql). Runs in a
    background thread so it never adds latency to sending the actual
    ...
    def _send():
        ...
        url = f"{SUPABASE_URL}/rest/v1/rpc/record_trade_result"
        try:
            _HTTP.post(
                url, headers=headers,
                json={"p_telegram_id": uid, "p_won": bool(won)},
                timeout=5,
            )
        except Exception as e:
            print(f"[LEADERBOARD] record_trade_result failed for {uid}: {e}")
    threading.Thread(target=_send, daemon=True).start()


def fetch_leaderboard():
    """Live (uncached) fetch of the top-5 leaderboard via the
    get_leaderboard() Supabase RPC. Returns a list of row dicts, or None
    on failure (caller should show a friendly error, not crash)."""
    headers = {
        "apikey": SUPABASE_ANON_KEY,
        "Authorization": f"Bearer {SUPABASE_ANON_KEY}",
        "Content-Type": "application/json",
    ...
    try:
        resp = _HTTP.post(url, headers=headers, json={}, timeout=8)
        if resp.status_code != 200:
            print(f"[LEADERBOARD] fetch failed HTTP {resp.status_code}: {resp.text[:200]}")
            return None
        return resp.json()
    except Exception as e:
...
def build_leaderboard_message() -> str:
    ...
    if rows is None:
        return "⚠️ Leaderboard abhi load nahi ho saka. Thodi der baad try karein."
    if not rows:
        return (
            "👑 Top Traders — Today\n"
            "✺━━━━・━━━━✺✺━━━━・━━━✺\n\n"
            "Abhi koi bhi user minimum 5 trades tak nahi pahoncha aaj. "
            "Thodi der baad dubara check karein."
        )
    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
    lines = ["👑 Top Traders — Today", "✺━━━━・━━━━✺✺━━━━・━━━✺", ""]
    for i, row in enumerate(rows[:5]):
        name = row.get("first_name") or "Trader"
        username = row.get("username")
        handle = f" @{username}" if username else ""
        wins = row.get("today_wins", 0)
        losses = row.get("today_losses", 0)
        total = wins + losses
        win_rate = row.get("win_rate", 0)
        lines.append(f"{medals[i]} {name}{handle}")
        lines.append(f"   📈 {win_rate}%")
        lines.append(f"   📋 {total}   ✅ {wins}   ❌ {losses}")
        lines.append(f"   🆔 {row.get('telegram_id')}")
        if i < len(rows[:5]) - 1:
            lines.append("┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈")
    lines.append("")
    lines.append("✺━━━━・━━━━✺✺━━━━・━━━✺")
...
def log_user_to_all_users(update: Update) -> None:
    ...
    try:
        _HTTP.post(url, headers=headers, json=data, params={"on_conflict": "telegram_id"}, timeout=5)
    except Exception as e:
...
def get_all_telegram_ids() -> List[int]:
    ...
    try:
        resp = _HTTP.get(url, headers=headers, timeout=5)
        if resp.status_code == 200:
            data = resp.json()
...
_auth_cache: Dict[int, Tuple[float, bool]] = {}
_AUTH_CACHE_TTL = 45  # seconds

# Tracks each user's last known real (non-cached) auth result, so we can
# detect the exact moment someone goes False -> True (revoked, then
# re-granted). That transition is when we wipe their state — see
# reset_user_state() below.
_last_known_auth: Dict[int, bool] = {}


def reset_user_state(uid: int):
    """Wipes ALL in-memory state for one user so the bot treats their next
    interaction exactly like a brand-new user. This is the actual fix for
    'stuck after re-adding access' — a returning user's old UserState object
    (running flag, session_token, strategy filters, loss cooldowns, signal
    history, etc.) and old context.user_data were never being cleared on
    revoke/re-grant, so whatever caused them to get stuck was still sitting
    there waiting the moment access was restored. A genuinely new user never
    hit this because they'd never had a UserState object built for them."""
    old = user_states.get(uid)
    if old is not None:
        # Bump/flip these on the EXISTING object first (not just delete it),
        # so if a background loop thread is still alive and holding a
        # reference to this same object (e.g. what actually made them
        # "stuck"), it sees running=False / stop_requested=True and the
        # stale session_token, and exits on its next check instead of
        # continuing to run underneath the fresh session.
        old.running = False
        old.stop_requested = True
        old.session_token += 1
    user_states.pop(uid, None)
    _auth_cache.pop(uid, None)
    if BOT_APP is not None:
        try:
            BOT_APP.drop_user_data(uid)
        except Exception:
            try:
                if uid in BOT_APP.user_data:
                    BOT_APP.user_data[uid].clear()
            except Exception:
                pass
    print(f"[AUTH] {uid} re-authorized after revoke — state wiped, treated as fresh user.")

async def _show_broadcast_preview(update, context, msg_text, msg_entities, photo_file_id=None):
    """Stores the drafted broadcast and shows it back to the owner exactly
    as recipients will see it, with Yes/No confirmation buttons, instead
    ...
    confirm_text = f"👆 This is exactly how it will look.\n\n📤 Send this to {total} users?"
    buttons = [
        [colored_button(" ✅ Yes, Send ", "broadcast_send_yes", KeyboardButtonStyle.SUCCESS, "6145553439809084250"),
         colored_button(" ❌ No, Cancel ", "broadcast_send_no", KeyboardButtonStyle.DANGER, "6145317070578916456")]
    ]
    markup = InlineKeyboardMarkup(buttons)
    entities = build_custom_emoji_entities(confirm_text)
    await update.message.reply_text(confirm_text, entities=entities, reply_markup=markup)


async def _execute_broadcast(update, context):
    """Actually sends the broadcast that was drafted+confirmed via
    _show_broadcast_preview(). Reads context.user_data['broadcast_pending']."""
    ...
    msg_entities = pending['entities']
    photo_file_id = pending.get('photo_file_id')

    # Get all user IDs from all_users table
    user_ids = get_all_telegram_ids()
    if not user_ids:
        await update.effective_chat.send_message("❌ No users found in database.")
        context.user_data['state'] = None
        return

    total = len(user_ids)
    progress_msg = await update.effective_chat.send_message(f"📡 Sending broadcast to {total} users...\n⏳ Please wait.")

    # Send broadcast with retry and rate limiting
    success, failed = await send_broadcast_with_retry(
        context, user_ids, msg_text, msg_entities, photo_file_id=photo_file_id
    )
    ...
    if failed > 0:
        report += f"\n⚠️ {failed} users failed (blocked or invalid)."
...
class _BroadcastRateLimiter:
    """Global token-bucket limiter shared across ALL concurrent broadcast
    senders, so the bot never exceeds Telegram's aggregate outgoing-message
    rate limit no matter how many workers are sending at once. Without
    this, high concurrency alone triggers a storm of 429 "Too Many
    Requests" errors early in a large broadcast — every 429 eats into a
    user's small retry budget, which is very likely why a 900-user
    broadcast was previously landing ~150 permanent failures."""
    def __init__(self, rate_per_sec: float = 20):
        self._interval = 1.0 / rate_per_sec
        self._lock = asyncio.Lock()
        self._next_slot = 0.0
        # Shared global pause: when ANY worker gets told by Telegram to
        # back off, every concurrent worker must honor that same pause —
        # not just the one that got the 429. Without this, the other
        # concurrency-1 workers keep hammering Telegram during the pause,
        # which is what escalates a normal per-chat/aggregate 429 into a
        # full bot-token-wide flood block (the 49830s case).
        self._pause_until = 0.0

    async def acquire(self):
        async with self._lock:
            now = asyncio.get_event_loop().time()
            wait = max(self._next_slot, self._pause_until) - now
            self._next_slot = max(self._next_slot, now) + self._interval
        if wait > 0:
            await asyncio.sleep(wait)

    async def pause_all(self, seconds: float):
        """Force every worker sharing this limiter to stop sending until
        `seconds` from now, regardless of the normal per-message pacing."""
        async with self._lock:
            now = asyncio.get_event_loop().time()
            self._pause_until = max(self._pause_until, now + seconds)


async def _broadcast_send_pass(context, user_ids, text, entities, photo_file_id,
                                rate_limiter, max_retries=4, concurrency=8):
    """One full pass over `user_ids`. Returns (succeeded_ids, blocked_ids,
    transient_failed_ids) so the caller can decide to retry the transient
    failures again (blocked/deactivated users are never retried)."""
    import re

    sem = asyncio.Semaphore(concurrency)
    succeeded, blocked, transient_failed = [], [], []
        ...
        async with sem:
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    err_str = str(e).lower()
                    # If user blocked bot, don't retry — this can never succeed.
                    if "forbidden" in err_str or "user is deactivated" in err_str or "bot was blocked" in err_str or "chat not found" in err_str:
                        blocked.append(uid)
                        return

                    # If rate limited, wait the FULL time Telegram asked for.
                    # Previously this was capped at 30s, so a real 429 asking
                    # for e.g. 60-120s would get under-waited and immediately
                    # re-hit by up to `concurrency` workers at once — that
                    # repeated under-waiting during an active flood window is
                    # what escalates Telegram's penalty into a full
                    # bot-token-wide block (we saw a 49830s block from this).
                    # Cap only at a generous ceiling (5 min) so one pathological
                    # value can't hang a worker forever, and pause every OTHER
                    # concurrent worker too via the shared rate limiter instead
                    # of just this one.
                    if "retry after" in err_str:
                        try:
                            wait = int(re.search(r'retry after (\d+)', err_str).group(1))
                            wait = min(wait, 300)
                        except Exception:
                            wait = 2 ** attempt
                        print(f"[BROADCAST] Rate limited for {uid}, pausing ALL workers {wait}s")
                        await rate_limiter.pause_all(wait)
                        await asyncio.sleep(wait)
                        continue
                    ...
                    if attempt < max_retries - 1:
                        ...
                        transient_failed.append(uid)
                        print(f"[BROADCAST] Failed to send to {uid}: {e}")
                        return

    await asyncio.gather(*(send_to_user(uid) for uid in user_ids))
    return succeeded, blocked, transient_failed


async def send_broadcast_with_retry(context, user_ids, text, entities=None, photo_file_id=None):
    """
    Send broadcast with:
    - A global rate limiter shared by every concurrent sender (avoids
      Telegram flood limits, the main cause of mass failures on big
      broadcasts).
    - Concurrency limit + per-user retry with backoff.
    - Skip (never retry) users who have blocked the bot.
    - A second full pass that retries ONLY the users who transiently
      failed on the first pass (e.g. still got flooded, brief network
      blip) — recovers most of what would otherwise be reported as
      "failed" on a large broadcast.
    - Optional photo: if photo_file_id is given, sends the photo with
      `text` as the caption instead of a plain text message.
    """
    rate_limiter = _BroadcastRateLimiter(rate_per_sec=20)

    succeeded, blocked, transient_failed = await _broadcast_send_pass(
        context, user_ids, text, entities, photo_file_id, rate_limiter
    )

    if transient_failed:
        print(f"[BROADCAST] Retrying {len(transient_failed)} transiently-failed users after first pass")
        await asyncio.sleep(2)
        succeeded2, blocked2, transient_failed2 = await _broadcast_send_pass(
            context, transient_failed, text, entities, photo_file_id, rate_limiter
        )
        succeeded += succeeded2
        blocked += blocked2
        transient_failed = transient_failed2

    success_count = len(succeeded)
    blocked_count = len(blocked)
    failed_count = blocked_count + len(transient_failed)
    return success_count, failed_count

def is_authorized(uid: int) -> bool:
    # Every command/button/loop-tick in this bot calls is_authorized(), and
    # it's a *blocking* requests.get() sitting inside async Telegram
    # handlers — a slow/timed-out Supabase call (we've seen it hit the 3s
    # timeout) freezes the bot's single event loop for everyone, not just
    # this user, which is what was making things like /stop feel late.
    # Cache the result briefly so repeated checks for the same user don't
    # re-hit the network every single time.
    now = time.monotonic()
    cached = _auth_cache.get(uid)
    ...
    params = {
        ...
        "select": "id"}
    try:
        resp = _HTTP.get(url, headers=headers, params=params, timeout=3)
        if resp.status_code == 200:
            result = len(resp.json()) > 0
            _auth_cache[uid] = (now, result)
            prev = _last_known_auth.get(uid)
            if prev is False and result is True:
                reset_user_state(uid)
            _last_known_auth[uid] = result
            return result
        ...
        _auth_cache[uid] = (now, True)
        return True
    except Exception as e:
        print(f"Auth Bypass Check: {e}")
        # Don't cache the fail-open result from a timeout/exception — we
        # want the *next* call to retry the real check soon rather than
        # being stuck open (or blocking again) for the full TTL.
        return True

async def is_authorized_async(uid: int) -> bool:
    """Async wrapper for is_authorized() so the 20+ async handlers that
    gate on it (/start, every button callback, every text state) don't
    block MAIN_LOOP for the whole bot on a cache-miss — is_authorized()
    does a blocking requests.get() with up to a 3s timeout, and it's the
    very first thing almost every handler does. This was the cause of the
    bot randomly "getting stuck" on /start, a confidence prompt, or
    anything else: whichever user's cache happened to be cold blocked
    every other user's message for up to 3s. asyncio.to_thread() runs the
    check off the event loop; cache-hits still return effectively
    instantly since is_authorized() checks the cache before ever touching
    the network."""
    return await MAIN_LOOP.run_in_executor(_AUTH_EXECUTOR, is_authorized, uid)


async def _maintenance_gate_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Runs in handler group -1 (before every other button handler).
    If maintenance mode is on and the clicking user is authorized (and
    not the owner), swap their click for the owner's maintenance
    message + ETA and stop the update from reaching the real handler.
    Unauthorized users and the owner are untouched — they fall through
    to the normal handlers exactly as before."""
    if not MAINTENANCE_MODE["on"]:
        return
    ...
    if uid == OWNER_ID:
        return
    # Owner must still be able to turn maintenance back OFF.
    if query.data == "admin_maintenance_toggle":
        return
    if not await is_authorized_async(uid):
        return  # unauthorized users: unaffected, normal flow continues

    now = time.monotonic()
    last = _MAINTENANCE_LAST_NOTICE.get(uid, 0)
    if now - last < _MAINTENANCE_NOTICE_COOLDOWN:
        await safe_query_answer(query)
        raise ApplicationHandlerStop()
    _MAINTENANCE_LAST_NOTICE[uid] = now

    await safe_query_answer(query)
    msg = MAINTENANCE_MODE["message"] or "Bot is under maintenance."
    eta = MAINTENANCE_MODE["eta"]
    text = f"🛠 Bot under maintenance.\n\n{msg}"
    if eta:
        text += f"\n\n⏰ Estimated time: {eta}"
    text += "\n\n🙏 Please don't spam — wait until the estimated time."
    try:
        await context.bot.send_message(chat_id=uid, text=text)
    except Exception as e:
        print(f"[MAINTENANCE GATE] send failed for {uid}: {e}")
    raise ApplicationHandlerStop()


# ══════════════ TIMEZONE SETTINGS (per user, per feature) ══════════════
# Each entry: (key, display label, default offset in MINUTES from UTC).
# `key` is also used inside callback_data ("tzpick_<key>", "tzset_<key>_<mins>"),
# so keep it short and free of "_" collisions with the parsing below.
TZ_FEATURES = [
    ("checker",         "Signal Checkers Timezone", 300),   # UTC +05:00
    ("otcfuture",       "OTC Future Signal",        300),   # UTC +05:00
    ("otclive",         "OTC LIVE MODE",             300),   # UTC +05:00
    ("multiengine",     "MULTI ENGINE",              300),   # UTC +05:00
    ("blackout",        "Blackout Future",           300),   # UTC +05:00
    ("whiteout",        "Whiteout Future",           300),   # UTC +05:00
    ("backtest",        "Backtest",                  300),   # UTC +05:00
    ("aiotc",           "AI OTC",                    300),   # UTC +05:00
    ("stratfinder",     "Strategy finder",           300),   # UTC +05:00
    ("autosignal",      "Auto signal",               300),   # UTC +05:00
    ("livefs",          "Live FS",                   300),   # UTC +05:00
    ("trendfilter",     "Trend Filter",              300),   # UTC +05:00
    ("aifilterfs",      "AI Filter FS",              300),   # UTC +05:00
]
TZ_DEFAULT_MINUTES = {key: mins for key, _label, mins in TZ_FEATURES}
TZ_LABELS = {key: label for key, label, _mins in TZ_FEATURES}

# Common UTC offsets (minutes) shown in the picker — every whole hour from
# -12 to +14 plus the real-world half/quarter-hour zones.
TZ_OFFSET_CHOICES = sorted(set(
    [m * 60 for m in range(-12, 15)] +
    [-570, -210, 330, 345, 390, 570, 630, 765, 780]
))


def format_utc_offset(minutes: int) -> str:
    """-180 -> 'UTC -03:00', 330 -> 'UTC +05:30'."""
    sign = "+" if minutes >= 0 else "-"
    a = abs(minutes)
    return f"UTC {sign}{a // 60:02d}:{a % 60:02d}"


def get_tz_minutes(uid: int, feature_key: str) -> int:
    """Current stored offset (in minutes) for a feature, for this user."""
    st = get_state(uid)
    return st.tz_settings.get(feature_key, TZ_DEFAULT_MINUTES.get(feature_key, 0))


def get_tz_delta(uid: int, feature_key: str) -> timedelta:
    """Same as get_tz_minutes() but ready to add/subtract from a datetime."""
    return timedelta(minutes=get_tz_minutes(uid, feature_key))


def build_tz_settings_text(uid: int) -> str:
    lines = ["🕐 TIMEZONE SETTINGS", "━━━━━━━━━━━━━━━━━━━━", ""]
    for key, label, _default in TZ_FEATURES:
        mins = get_tz_minutes(uid, key)
        lines.append(label)
        lines.append(f"└ Active: {format_utc_offset(mins)}")
        lines.append("")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append("Tap a button below to change timezone.")
    return "\n".join(lines)


def build_tz_settings_keyboard() -> "InlineKeyboardMarkup":
    rows = []
    for key, label, _default in TZ_FEATURES:
        rows.append([colored_button(label, f"tzpick_{key}", KeyboardButtonStyle.PRIMARY)])
    rows.append([colored_button(" Back", "smzx_settings_back", KeyboardButtonStyle.DANGER)])
    return InlineKeyboardMarkup(rows)


def build_smzx_settings_text() -> str:
    return (
        "🥸 SMZX SETTINGS\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Manage your timezones and personalise Name template.\n\n"
        "🪱 Change Timezone\n"
        "└ Adjust all checker / signal timezones.\n\n"
        "🐱 Name Template\n"
        "└ Set your @name to result and signal messages.\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "Pick what you want to manage."
    )


def build_smzx_settings_keyboard() -> "InlineKeyboardMarkup":
    rows = [
        [colored_button("   Change Timezone  ", "smzx_settings_timezone", KeyboardButtonStyle.SUCCESS, "5447410659077661506")],
        [colored_button("   Name Template  ", "smzx_settings_name_template", KeyboardButtonStyle.SUCCESS, "5787604998435115723")],
        [colored_button(" Back", "back_to_main", KeyboardButtonStyle.DANGER)],
    ]
    return InlineKeyboardMarkup(rows)


async def show_smzx_settings(update: Update, context: ContextTypes.DEFAULT_TYPE, edit: bool = False):
    uid = update.effective_user.id if update.effective_user else update.callback_query.from_user.id
    text = build_smzx_settings_text()
    entities = build_custom_emoji_entities(text)
    reply_markup = build_smzx_settings_keyboard()
    if edit and update.callback_query:
        await update.callback_query.edit_message_text(text, entities=entities, reply_markup=reply_markup)
    else:
        await context.bot.send_message(chat_id=uid, text=text, entities=entities, reply_markup=reply_markup)


def build_tz_picker_keyboard(uid: int, feature_key: str) -> "InlineKeyboardMarkup":
    current = get_tz_minutes(uid, feature_key)
    rows = []
    row = []
    for mins in TZ_OFFSET_CHOICES:
        label = format_utc_offset(mins).replace("UTC ", "")
        if mins == current:
            label = f"✅ {label}"
        row.append(colored_button(label, f"tzset_{feature_key}_{mins}", KeyboardButtonStyle.SUCCESS))
        if len(row) == 4:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    rows.append([colored_button(" Back", "tzback_settings", KeyboardButtonStyle.DANGER)])
    return InlineKeyboardMarkup(rows)


async def show_timezone_settings(update: Update, context: ContextTypes.DEFAULT_TYPE, edit: bool = False):
    uid = update.effective_user.id if update.effective_user else update.callback_query.from_user.id
    text = build_tz_settings_text(uid)
    entities = build_custom_emoji_entities(text)
    reply_markup = build_tz_settings_keyboard()
    if edit and update.callback_query:
        await update.callback_query.edit_message_text(text, entities=entities, reply_markup=reply_markup)
    else:
        await context.bot.send_message(chat_id=uid, text=text, entities=entities, reply_markup=reply_markup)
...
async def timezone_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the Settings → Timezones list, the per-feature offset
    picker, and saving a new offset. Registered separately from
    menu_callback since its callback_data ("tzpick_*"/"tzset_*") doesn't
    match any of that handler's existing patterns."""
    query = update.callback_query
    await safe_query_answer(query)
    uid = query.from_user.id
    data = query.data
    ...
    if data == "tzback_settings":
        await show_timezone_settings(update, context, edit=True)
        return

    if data.startswith("tzpick_"):
        feature_key = data[len("tzpick_"):]
        if feature_key not in TZ_LABELS:
            return
        text = f"🕐 {TZ_LABELS[feature_key]}\n\nCurrent: {format_utc_offset(get_tz_minutes(uid, feature_key))}\n\nSelect the new UTC offset:"
        entities = build_custom_emoji_entities(text)
        await query.edit_message_text(text, entities=entities,
                                       reply_markup=build_tz_picker_keyboard(uid, feature_key))
        return

    if data.startswith("tzset_"):
        # tzset_<feature_key>_<minutes>  (minutes may start with "-")
        rest = data[len("tzset_"):]
        feature_key, _, mins_str = rest.rpartition("_")
        if feature_key not in TZ_LABELS:
            return
        try:
            mins = int(mins_str)
            ...
            return
        st = get_state(uid)
        st.tz_settings[feature_key] = mins
        # Reverse-sync into the legacy per-feature UTC keys some older
        # features still read directly, so the new consolidated menu is
        # a true single source of truth for them too.
        legacy_hours = mins // 60
        if feature_key == "checker":
            context.user_data['checker_utc'] = legacy_hours
        elif feature_key == "backtest":
            context.user_data['bt2_utc'] = legacy_hours
        elif feature_key == "livefs":
            context.user_data['lfc_utc'] = legacy_hours
        elif feature_key == "blackout":
            context.user_data['bl_checker_utc'] = legacy_hours
        await safe_query_answer(query, f"✅ {TZ_LABELS[feature_key]} set to {format_utc_offset(mins)}", show_alert=False)
        await show_timezone_settings(update, context, edit=True)
        return
...
class UserState:
    def __init__(self):
        ...
        self.running = False
        # Bumped every time a new auto-signal session starts. A background
        # loop thread captures this value when it starts and keeps checking
        # it matches — so if an old, still-alive thread (e.g. stuck in a
        # slow network call) wakes up after a NEW session has already set
        # st.running=True, it recognizes it's stale and exits instead of
        # continuing to run alongside/interfering with the new session.
        ...
        self.ai_broker = "quotex"
        # Per-feature timezone settings (see ⚙️ Settings menu)
        self.tz_settings = dict(TZ_DEFAULT_MINUTES)
        # Global "Name Template" (⚙️ Settings → 🎯 Name Template) — the
        # owner/developer @username shown at the bottom of signal messages
        # across OTC Live, AI OTC, Auto Signal, Multi Engine and Forex
        # Live modes (Format 1/2/3).
        self.name_template = "@Rohailtrader"
...
def get_state(uid: int) -> UserState:
    ...
    if not getattr(st, "tz_settings", None):
        st.tz_settings = dict(TZ_DEFAULT_MINUTES)
    if not getattr(st, "name_template", None):
        st.name_template = "@Rohailtrader"
    return st


def get_name_template(uid: int) -> str:
    """Current global owner/developer @username set via ⚙️ Settings →
    🎯 Name Template. Used as the default `owner_name` for Format 1/2/3
    signal messages across every mode."""
    return get_state(uid).name_template
...
# Pocket Option equivalent of BACKTEST_SUPPORTED_PAIRS (base symbols, no
# OTC suffix / dashes — derived from POCKET_OTC_PAIRS below).
POCKET_BACKTEST_SUPPORTED_PAIRS = [
    "ADAUSD", "AEDCNY", "AMD", "AMZN", "AUDCAD", "AUDCHF",
    "AUDJPY", "AUDNZD", "AUDUSD", "AUS200", "AVAX", "BABA",
    "BHDCNY", "BITB", "BNBUSD", "BTCUSD", "CADCHF", "CADJPY",
    "CHFJPY", "CHFNOK", "CITI", "COIN", "D30EUR", "DJI30",
    "DOGE", "DOTUSD", "E35EUR", "E50EUR", "ETHUSD", "EURCHF",
    "EURGBP", "EURHUF", "EURJPY", "EURNZD", "EURRUB", "EURTRY",
    "EURUSD", "F40EUR", "FDX", "GBPAUD", "GBPJPY", "GBPUSD",
    "GME", "IRRUSD", "JODCNY", "JPN225", "KESUSD", "LBPUSD",
    "LINK", "LTCUSD", "MADUSD", "MARA", "MATIC", "NASUSD",
    "NFLX", "NGNUSD", "NZDJPY", "NZDUSD", "OMRCNY", "PLTR",
    "QARCNY", "SARCNY", "SOLUSD", "SP500", "SYPUSD", "TNDUSD",
    "TONUSD", "TRXUSD", "UAHUSD", "UKBrent", "USCrude", "USDARS",
    "USDBDT", "USDBRL", "USDCAD", "USDCHF", "USDCLP", "USDCNH",
    "USDCOP", "USDDZD", "USDEGP", "USDIDR", "USDINR", "USDJPY",
    "USDMXN", "USDMYR", "USDPHP", "USDPKR", "USDRUB", "USDSGD",
    "USDTHB", "USDVND", "VISA", "VIX", "XAGUSD", "XAUUSD",
    "XNGUSD", "XPDUSD", "XPTUSD", "YERUSD", "ZARUSD"
]

# Tradowix equivalent of BACKTEST_SUPPORTED_PAIRS (base symbols, no
# OTC suffix — derived from TRADOWIX_OTC_PAIRS below).
TRADOWIX_BACKTEST_SUPPORTED_PAIRS = [
    "AAPL", "AMD", "AMZN", "ANTH", "APTUSD", "ARBUSD",
    "ATOMUSD", "AUDCAD", "AUDCHF", "AUDJPY", "AUDNZD", "AUDUSD",
    "AVAXUSD", "AXP", "AXSUSD", "BA", "BCHUSD", "BCOUSD",
    "BNBUSD", "BTCUSD", "CADCHF", "CADJPY", "CHFJPY", "COPPERUSD",
    "DASHUSD", "DASUSD", "DIS", "DOGEUSD", "DOTUSD", "ETCUSD",
    "ETHUSD", "EURAUD", "EURCAD", "EURCHF", "EURGBP", "EURJPY",
    "EURNZD", "EURSGD", "EURUSD", "GALAUSD", "GBPAUD", "GBPCAD",
    "GBPCHF", "GBPJPY", "GBPNZD", "GBPUSD", "GOOG", "GS",
    "INTC", "JNJ", "JPM", "KO", "LINKUSD", "LINUSD",
    "LTCUSD", "MANAUSD", "MCD", "MELANIAUSD", "META", "MSFT",
    "NFLX", "NKE", "NVDA", "NZDCAD", "NZDCHF", "NZDJPY",
    "NZDUSD", "OAIA", "PFE", "SOLUSD", "TIAUSD", "TONUSD",
    "TRUMPUSD", "TRXUSD", "TSLA", "USDARS", "USDBDT", "USDBRL",
    "USDCAD", "USDCHF", "USDCOP", "USDDZD", "USDEGP", "USDIDR",
    "USDINR", "USDJPY", "USDMXN", "USDNGN", "USDPHP", "USDPKR",
    "USDTRY", "USDZAR", "V", "WIFUSD", "WMT", "WTIUSD",
    "XAGUSD", "XAUUSD", "XOM", "XPDUSD", "XPTUSD", "XRPUSD",
    "ZECUSD"
]
...
# ===== STRATEGY 4 – PATTERN REPLAY: POCKET OPTION BROKER PAIR LISTS =====
# Candle source: https://pocandledata.beaxtapi.Online/?pair=<PAIR>&timeframe=M1&count=14000
POCKET_CANDLE_API_BASE = "https://pocandledata.beaxtapi.Online/"

# ─── Broker helper lookups (Future Signal: Quotex / Pocket Option / Tradowix) ───
def fut4_broker_label(broker: str) -> str:
    return {"pocket": "POCKET OPTION", "tradowix": "TRADOWIX"}.get(broker, "QUOTEX")

def fut4_candle_api_base(broker: str) -> str:
    if broker == "pocket":
        return POCKET_CANDLE_API_BASE
    elif broker == "tradowix":
        return TRADOWIX_CANDLE_API_BASE
    return NEW_CANDLE_API_BASE

def fut4_pairs_for(broker: str, market: str = "otc"):
    """market: 'otc' or 'live'. Returns the correct pair list so OTC/Live
    pairs from one broker's naming convention never leak into a request
    aimed at a different broker's candle API."""
    if broker == "pocket":
        return POCKET_OTC_PAIRS if market == "otc" else POCKET_LIVE_PAIRS
    elif broker == "tradowix":
        return TRADOWIX_OTC_PAIRS if market == "otc" else TRADOWIX_LIVE_PAIRS
    return FUT4_OTC_PAIRS if market == "otc" else FUT4_LIVE_PAIRS

POCKET_OTC_PAIRS = [
    "ADA-USD_otc", "AEDCNY_otc", "AMD_otc", "AMZN_otc",
    "AUDCAD_otc", "AUDCHF_otc", "AUDJPY_otc", "AUDNZD_otc",
    "AUDUSD_otc", "AUS200_otc", "AVAX_otc", "BABA_otc",
    "BHDCNY_otc", "BITB_otc", "BNB-USD_otc", "BTCUSD_otc",
    "CADCHF_otc", "CADJPY_otc", "CHFJPY_otc", "CHFNOK_otc",
    "CITI_otc", "COIN_otc", "D30EUR_otc", "DJI30_otc",
    "DOGE_otc", "DOTUSD_otc", "E35EUR_otc", "E50EUR_otc",
    "ETHUSD_otc", "EURCHF_otc", "EURGBP_otc", "EURHUF_otc",
    "EURJPY_otc", "EURNZD_otc", "EURRUB_otc", "EURTRY_otc",
    "EURUSD_otc", "F40EUR_otc", "FDX_otc", "GBPAUD_otc",
    "GBPJPY_otc", "GBPUSD_otc", "GME_otc", "IRRUSD_otc",
    "JODCNY_otc", "JPN225_otc", "KESUSD_otc", "LBPUSD_otc",
    "LINK_otc", "LTCUSD_otc", "MADUSD_otc", "MARA_otc",
    "MATIC_otc", "NASUSD_otc", "NFLX_otc", "NGNUSD_otc",
    "NZDJPY_otc", "NZDUSD_otc", "OMRCNY_otc", "PLTR_otc",
    "QARCNY_otc", "SARCNY_otc", "SOL-USD_otc", "SP500_otc",
    "SYPUSD_otc", "TNDUSD_otc", "TON-USD_otc", "TRX-USD_otc",
    "UAHUSD_otc", "UKBrent_otc", "USCrude_otc", "USDARS_otc",
    "USDBDT_otc", "USDBRL_otc", "USDCAD_otc", "USDCHF_otc",
    "USDCLP_otc", "USDCNH_otc", "USDCOP_otc", "USDDZD_otc",
    "USDEGP_otc", "USDIDR_otc", "USDINR_otc", "USDJPY_otc",
    "USDMXN_otc", "USDMYR_otc", "USDPHP_otc", "USDPKR_otc",
    "USDRUB_otc", "USDSGD_otc", "USDTHB_otc", "USDVND_otc",
    "VISA_otc", "VIX_otc", "XAGUSD_otc", "XAUUSD_otc",
    "XNGUSD_otc", "XPDUSD_otc", "XPTUSD_otc", "YERUSD_otc",
    "ZARUSD_otc"
]

POCKET_LIVE_PAIRS = [
    "AEX25", "AUDCAD", "AUDCHF", "AUDJPY",
    "AUDUSD", "AUS200", "BABA", "BCHEUR",
    "BCHGBP", "BCHJPY", "BTCGBP", "BTCJPY",
    "BTCUSD", "CAC40", "CADCHF", "CADJPY",
    "CHFJPY", "CITI", "D30EUR", "DASH_USD",
    "DJI30", "E35EUR", "E50EUR", "ETHUSD",
    "EURAUD", "EURCAD", "EURCHF", "EURGBP",
    "EURJPY", "EURUSD", "F40EUR", "GBPAUD",
    "GBPCAD", "GBPCHF", "GBPJPY", "GBPUSD",
    "H33HKD", "JPN225", "LNKUSD", "NASUSD",
    "NFLX", "SMI20", "SP500", "UKBrent",
    "USCrude", "USDCAD", "USDCHF", "USDJPY",
    "XAGEUR", "XAGUSD", "XAUEUR", "XAUUSD",
    "XNGUSD", "XPDUSD", "XPTUSD"
]

TRADOWIX_CANDLE_API_BASE = "https://trwcandledata.beaxtapi.online/"

TRADOWIX_OTC_PAIRS = [
    "AAPL_otc", "AMD_otc", "AMZN_otc", "ANTH_otc",
    "APTUSD_otc", "ARBUSD_otc", "ATOMUSD_otc", "AUDCAD_otc",
    "AUDCHF_otc", "AUDJPY_otc", "AUDNZD_otc", "AUDUSD_otc",
    "AVAXUSD_otc", "AXP_otc", "AXSUSD_otc", "BA_otc",
    "BCHUSD_otc", "BCOUSD_otc", "BNBUSD_otc", "BTCUSD_otc",
    "CADCHF_otc", "CADJPY_otc", "CHFJPY_otc", "COPPERUSD_otc",
    "DASHUSD_otc", "DASUSD_otc", "DIS_otc", "DOGEUSD_otc",
    "DOTUSD_otc", "ETCUSD_otc", "ETHUSD_otc", "EURAUD_otc",
    "EURCAD_otc", "EURCHF_otc", "EURGBP_otc", "EURJPY_otc",
    "EURNZD_otc", "EURSGD_otc", "EURUSD_otc", "GALAUSD_otc",
    "GBPAUD_otc", "GBPCAD_otc", "GBPCHF_otc", "GBPJPY_otc",
    "GBPNZD_otc", "GBPUSD_otc", "GOOG_otc", "GS_otc",
    "INTC_otc", "JNJ_otc", "JPM_otc", "KO_otc",
    "LINKUSD_otc", "LINUSD_otc", "LTCUSD_otc", "MANAUSD_otc",
    "MCD_otc", "MELANIAUSD_otc", "META_otc", "MSFT_otc",
    "NFLX_otc", "NKE_otc", "NVDA_otc", "NZDCAD_otc",
    "NZDCHF_otc", "NZDJPY_otc", "NZDUSD_otc", "OAIA_otc",
    "PFE_otc", "SOLUSD_otc", "TIAUSD_otc", "TONUSD_otc",
    "TRUMPUSD_otc", "TRXUSD_otc", "TSLA_otc", "USDARS_otc",
    "USDBDT_otc", "USDBRL_otc", "USDCAD_otc", "USDCHF_otc",
    "USDCOP_otc", "USDDZD_otc", "USDEGP_otc", "USDIDR_otc",
    "USDINR_otc", "USDJPY_otc", "USDMXN_otc", "USDNGN_otc",
    "USDPHP_otc", "USDPKR_otc", "USDTRY_otc", "USDZAR_otc",
    "V_otc", "WIFUSD_otc", "WMT_otc", "WTIUSD_otc",
    "XAGUSD_otc", "XAUUSD_otc", "XOM_otc", "XPDUSD_otc",
    "XPTUSD_otc", "XRPUSD_otc", "ZECUSD_otc"
]

TRADOWIX_LIVE_PAIRS = [
    "AUDCAD", "AUDCHF", "AUDJPY", "AUDUSD",
    "BTCUSD", "CADJPY", "CHFJPY", "ETHUSD",
    "EURAUD", "EURCAD", "EURCHF", "EURGBP",
    "EURJPY", "EURUSD", "GBPAUD", "GBPCAD",
    "GBPCHF", "GBPJPY", "GBPUSD", "NZDUSD",
    "USDBRL", "USDCAD", "USDCHF", "USDCOP",
    "USDJPY", "USDPHP"
]
...
def multi_engine_pairs(broker: str, market: str = "otc"):
    """Broker-isolated pair pool for Multi Engine — mirrors fut4_pairs_for()
    so Pocket/Tradowix pairs (and their own _otc naming) never leak into a
    request aimed at a different broker's candle/tick API, and vice versa."""
    if broker == "pocket":
        return POCKET_OTC_PAIRS if market == "otc" else POCKET_LIVE_PAIRS
    elif broker == "tradowix":
        return TRADOWIX_OTC_PAIRS if market == "otc" else TRADOWIX_LIVE_PAIRS
    return MULTI_ENGINE_OTC if market == "otc" else MULTI_ENGINE_LIVE
...
class PTBSender:
    """Sends via Telegram Bot HTTP API from any thread."""
    ...
    def send_message(self, chat_id, text, buttons=None, entities=None):
        ...
        ent = _tg_obj_to_dict(entities)
        if ent:
            # Telegram hard-cap is 100 entities. Over that the whole message is dropped.
            payload["entities"] = ent[:90]
        markup = _markup_to_raw(buttons)
        if markup:
            payload["reply_markup"] = markup
        return _tg_api("sendMessage", payload)

    def send_file(self, chat_id, file_path, caption, entities=None, add_bold=False):
        if not _tg_token() or not file_path:
            return None
        if entities is None:
            entities = build_custom_emoji_entities(caption or "")
        if add_bold and caption:
            entities = list(entities or [])
            entities.append(MessageEntity(
                type='bold', offset=0,
                length=len(caption.encode('utf-16-le')) // 2))
        payload = {"chat_id": chat_id, "caption": caption or ""}
        ent = _tg_obj_to_dict(entities)
        if ent:
            payload["caption_entities"] = ent
        try:
            # Read the whole file into memory ONCE instead of passing a live
            # file handle into _tg_api. _tg_api's own retry loop (on a
            # ReadTimeout, network blip, etc.) reuses the same `files` dict
            # for the next attempt — a file handle is a stream that's
            # already been fully consumed/EOF'd by the failed first attempt,
            # so the retry silently uploads 0 bytes and Telegram rejects it
            # with "file must be non-empty" right after the timeout. Raw
            # bytes have no position/EOF state, so every retry attempt
            # re-sends the full image correctly.
            with open(file_path, "rb") as f:
                file_bytes = f.read()
            if not file_bytes:
                print(f"[SEND FILE] {file_path} is empty on disk, sending text instead.")
                return self.send_message(chat_id, caption or "⚠️ Chart failed to send.")
            return _tg_api(
                "sendPhoto", payload,
                files={"photo": (os.path.basename(file_path) or "chart.png", file_bytes, "image/png")},
                timeout=30,
            )
        except Exception as e:
            print(f"[SEND FILE] {e}")
    ...
    def edit_message(self, chat_id, msg_id, text, entities=None, buttons=None):
        if not _tg_token() or not msg_id:
            return None
        if entities is None:
            entities = build_custom_emoji_entities(text)
        payload = {
...
def _send_via_ptb_fallback(uid, context, text):
    """Used by Multi Engine if a send returns nothing — now just HTTP again."""
    return sender.send_message(uid, text)


_MAX_SIGS_PER_MSG = 35
_TG_SAFE_CHARS = 3500


def send_signal_list_parts(uid, header, lines, footer="", lines_per_msg=_MAX_SIGS_PER_MSG):
    """Send a long signal list in pages."""
    if not lines:
    ...
    for line in lines:
        ...
        body = "\n".join(trial)
        extra = 80
        if buf and (len(trial) > lines_per_msg or (len(header) + extra + len(body) + len(footer or "")) > _TG_SAFE_CHARS):
            chunks.append(buf)
            buf = [line]
        else:
            buf = trial
    ...
    for i, chunk in enumerate(chunks):
        ...
        if n_pages > 1:
            parts.append(f"\n📄 PAGE {i + 1}/{n_pages}  ·  {len(lines)} signals\n")
        parts.append("━━━━━━━━━━━━━━━━━━\n")
        parts.append("\n".join(chunk))
        if is_last and footer:
            parts.append(footer if footer.startswith("\n") else "\n" + footer)
        msg = "".join(parts)
        ok = sender.send_message(uid, msg)
        if ok:
            sent += 1
        else:
            print(f"[SIGNAL PARTS] page {i + 1}/{n_pages} failed uid={uid} chars={len(msg)}")
        if not is_last:
            time.sleep(0.35)
    return sent > 0
...
def run_smz_hacking_mode(uid, days, start_time, end_time, tf, selected_pairs, broker="quotex"):
    ...
    for pair in selected_pairs:
        try:
            # Unified candle source for all brokers — fetch_beaxt_candles()
            # already routes Quotex to mrbeaxt.com/Qxapi/qx.php (true-UTC
            # 'epoch'), Pocket Option to pocandledata, and Tradowix to
            # trwcandledata. Every branch returns oldest-first candles with
            # a raw UTC-epoch 'time' key, so the +5 (UTC -> Pakistan)
            # conversion below is correct for all three — no +6 drift.
            result = fetch_beaxt_candles(pair, count=required_candles, retries=2, timeout=15, broker=broker)
            ...
            for idx in range(50, len(candles)-1):
                ...
                dt_pk = dt_utc + get_tz_delta(uid, "otcfuture")
                ...
                acc = (filters / 4) * 100
                if acc >= MIN_ACCURACY:
                    # Normalize display style to the bot's usual "PAIR-OTC"
                    # convention regardless of broker pair format (Pocket's
                    # list uses lowercase "pair_otc").
                    display_pair = pair.upper().replace('_OTC', '-OTC')
                    raw_signals.append({
                        'pair': display_pair,
                        'time': time_str,
                        'dir': dir_pred,
    ...
    now_pk = datetime.now(timezone.utc) + get_tz_delta(uid, "otcfuture")
    date_str = now_pk.strftime("%Y-%m-%d")
    broker_label = fut4_broker_label(broker)
    tz_label = format_utc_offset(get_tz_minutes(uid, "otcfuture"))
    # Use fancy_font for non‑signal lines
    header = (
        f"{fancy_font('🏐 𝚂𝙼𝚉 𝙱𝙾𝚃 𝙵𝚄𝚃𝚄𝚁𝙴 🏐')} ({broker_label})\n\n"
        f"{fancy_font('🗓 ')}{fancy_font(date_str)}\n\n"
        f"{fancy_font('💎 Timezone: ' + tz_label)}\n\n"
...
async def run_blackout_fs(uid, start_time, end_time, pairs_list):
    ...
    for pair in pairs_list:
        # fetch_blackout_signals() does a blocking requests.get() (up to
        # 15s) and this whole function runs directly on MAIN_LOOP (it's
        # `await`-ed from a handler), so calling it plain here froze the
        # event loop for everyone during blackout scans — same class of
        # bug as the backtest one. Offload to a thread instead.
        times = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, fetch_blackout_signals, pair, start_time, end_time)
        if times:
            for t in times:
    ...
    # Build output
    now_pk = datetime.now(timezone.utc) + get_tz_delta(uid, "blackout")
    date_str = now_pk.strftime("%Y.%m.%d")
    tz_line = fancy_font(format_utc_offset(get_tz_minutes(uid, "blackout")) + " (PK TIME)")
    header = (
...
def run_blackout_checker_worker(uid, date_str, signals, mtg_level, context, payout_filter=False):
    """Background thread: verify each signal using live candles (timestamp-based).
    Rewritten to match Run Advance Checker's speed: dedupe pairs and fetch
    them all in parallel (ThreadPoolExecutor), instead of one blocking
    request per signal. Also matches Advance Checker's custom-UTC input
    (signals arrive as (pair, display_time, api_time) — api_time is
    already converted to UTC+5 for candle lookups) and payout filter."""
    import threading
    ...
    # Same dynamic count scaling as Run Advance Checker: further back the
    # requested date, the more candles we need to ask for.
    days_back = max(1, (now_utc5.date() - target_date).days)
    fetch_count = min(40000, max(3000, (days_back + 2) * 1440))

    unique_pairs = list({pair for pair, _, _ in signals})
    lookup_by_pair = {}

    def fetch_pair(pair):
        data = fetch_beaxt_candles(pair, count=fetch_count, retries=3, timeout=25)
        if not data:
            return pair, None
        ...
        for c in candles:
            ...
            lookup.setdefault(date_key, {})[time_key] = c
        return pair, lookup

    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(fetch_pair, p): p for p in unique_pairs}
        for future in as_completed(futures):
            pair, lookup = future.result()
            lookup_by_pair[pair] = lookup

    results = []  # (signal_line, result_icon, error_msg)
    skipped_payout = 0

    for pair, display_time, api_time in signals:
        # Check if signal time is in the future (make aware) — uses api_time
        # (UTC+5) since that's what the candle data / date_str are keyed on.
        signal_dt = datetime.strptime(f"{date_str} {api_time}", "%Y-%m-%d %H:%M")
        signal_dt = signal_dt.replace(tzinfo=tz_pk)
        if signal_dt > now_utc5:
            results.append((f"M1 {pair} {display_time}", "⏳", ""))
            continue

        lookup = lookup_by_pair.get(pair)
        if not lookup:
            results.append((f"M1 {pair} {display_time}", "❌", "API error"))
            continue
        try:
            day_lookup = lookup.get(date_str, {})
            if not day_lookup:
                results.append((f"M1 {pair} {display_time}", "❌", "no candles for date"))
                continue
            ...
            if not prev_candle:
                results.append((f"M1 {pair} {display_time}", "❌", "no previous candle"))
                continue
            ...
            if not entry_candle:
                results.append((f"M1 {pair} {display_time}", "❌", "no entry candle"))
                continue

            payout = entry_candle.get('payout', 92)
            if payout_filter and payout < 80:
                results.append((f"M1 {pair} {display_time} {payout}%", "", ""))
                skipped_payout += 1
                continue
                ...
                if result_icon == "❌" and mtg_level >= 2:
                    ...
                    if next2_candle:
                        ...
                        if next2_dir == expected_dir:
                            result_icon = "✅²"
            results.append((f"M1 {pair} {display_time}", result_icon, ""))
        except Exception as e:
            results.append((f"M1 {pair} {display_time}", "❌", str(e)[:50]))

    # Count wins, losses, pending (exclude pending/skipped from win/loss)
    ...
    summary = (
        ...
        + (f"{fancy_font('💰 Skipped (low payout): ')}{fancy_font(str(skipped_payout))}\n" if payout_filter else "")
        + f"{fancy_font('━━━━━━━━━━━ • ━━━━━━━━━━━')}"
    )
...
async def white_pair_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "white_pair_all":
        ...
        msg = f"⏰ 𝙴𝚗𝚝𝚎𝚛 𝚜𝚝𝚊𝚛𝚝 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {format_utc_offset(get_tz_minutes(uid, 'whiteout'))}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 09:00"
        entities = build_custom_emoji_entities(msg)
        await query.edit_message_text(msg, entities=entities)
        return
        ...
        selected = context.user_data.get('white_selected_pairs', set())
        if not selected:
            await safe_query_answer(query, "❌ Select at least one pair!", show_alert=True)
            return
...
def run_whiteout_signals(uid, context):
    ...
    try:
        ...
        now_utc5 = datetime.now(timezone(get_tz_delta(uid, "whiteout")))
        date_list = []
        ...
        # ---------- BUILD OUTPUT ----------
        now_pk = datetime.now(timezone(get_tz_delta(uid, "whiteout")))
        date_str = now_pk.strftime("%d-%b-%Y").upper()
        tz_label = format_utc_offset(get_tz_minutes(uid, "whiteout"))
...
def build_signal_message(pair, entry_time, direction, payout, trend_text, tf="M1", owner_name="@Rohailtrader"):
...
def build_result_message_first_win(pair, entry_time, payout, wins, losses, owner_name="@Rohailtrader"):
...
def build_result_message_second_win(pair, entry_time, payout, wins, losses, owner_name="@Rohailtrader"):
...
def build_result_message_loss(pair, entry_time, payout, wins, losses, owner_name="@Rohailtrader"):
...
def build_signal_format2(pair, entry_time, direction, tf="M1", owner_name="@Rohailtrader"):
    dir_emoji = "📉" if direction == "CALL" else "📈"
    return (
        f"📊{pair}\n\n"
...
def run_ai_mode(uid, broker=None):
    ...
    st = get_state(uid)
    if broker is None:
        broker = getattr(st, 'ai_broker', 'quotex')
    if st.running:
    ...
    bot = SMZXBot(uid, broker=broker, tz_feature="aiotc")
    pairs = bot.pairs
    all_hits = []
    ...
    entry_t = bot.display_time(top['entry_dt'])
    ...
    st.signal_history.append({
        ...
        'time': bot.display_time(entry_dt_utc5),
        'result': "WIN" if win1 else "LOSS",
        'type': trade_type
    })
    if not win1:
        st.last_loss[pair] = datetime.now(timezone.utc)
    if win1:
        st.stats['wins'] += 1
        record_trade_result(uid, True)
        record_trade_hour(uid, broker, True)
    ...
    if win2:
        ...
        st.stats['losses'] += 1
        record_trade_result(uid, False)
        record_trade_hour(uid, broker, False)
        result_msg = _ai_build_result_msg(
            pair,
            direction,
            "LOSS",
...
def draw_loss_chart(pair, entry_time_str, direction, candles, entry_index=None, user_offset=5):
    ...
    try:
        ...
        font_bold = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 19)
    except:
...
def draw_pro_chart(candles, pair, entry_time, direction, payout, result=None, confidence=0,
    ...
    try:
        font_small = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", F(18))
        font_med = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", F(23))
        font_bold = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", F(26))
        font_big = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", F(29))
        font_small_bold = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", F(16))
        font_result = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", F(52))
        font_watermark = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", F(80))
        font_level = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", F(17))
    except:
...
def build_signal_format3(pair, entry_time, direction, payout, owner_name="@Rohailtrader", tf="M1", tz_label="UTC +05:00"):
    """
    ...
    msg = (
        ...
        f"🐠 𝚃𝙸𝙼𝙴𝙵𝚁𝙰𝙼𝙴 ➜ {tf}\n"
        f"🐟 𝚃𝙸𝙼𝙴𝚉𝙾𝙽𝙴 ➜ {fancy_font(tz_label)}\n"
        f"🐬 𝙿𝙰𝚈𝙾𝚄𝚃 ➜ {payout}%\n"
...
def _draw_v4_chart(
    ...
    BAR_BG = (22, 28, 42)
    f_header = _get_chart_font(20, medium=True)
    f_price = _get_chart_font(15)
    f_small = _get_chart_font(13)
    f_sidebar_ttl = _get_chart_font(13)
    f_sidebar_lbl = _get_chart_font(16)
    f_sidebar_val = _get_chart_font(16, bold=True)
    f_ema = _get_chart_font(13, bold=True)
    f_vol = _get_chart_font(13)
    f_time = _get_chart_font(13)
    f_brand = _get_chart_font(23, bold=True)
    f_brand_sm = _get_chart_font(13)
    f_badge = _get_chart_font(17, bold=True)
    f_hl = _get_chart_font(12)
    f_conf = _get_chart_font(18, bold=True)
    f_marker = _get_chart_font(13, bold=True)
...
class SMZXBot:
    def __init__(self, uid, broker="quotex", tz_feature="otclive"):
        self.uid = uid
        # Which Settings > UTC Selection feature key this bot instance's
        # entry-time calculations should follow. Different features
        # (OTC Live Mode, AI OTC, Auto Signal) all share this class, so the
        # caller tells us which one it is instead of us guessing.
        self.tz_feature = tz_feature
        st = get_state(uid)
        self.market_type = st.market_type
        self.pairs = st.pairs if st.pairs else (
            POCKET_OTC_PAIRS.copy() if broker == "pocket"
            else TRADOWIX_OTC_PAIRS.copy() if broker == "tradowix"
            else DEFAULT_OTC_PAIRS.copy()
        )
        self.base_url = NEW_CANDLE_API_BASE  # unused by fetch_data(), kept for compat
        self.broker = broker  # "quotex" (mrbeaxt.com) or "pocket" (pocandledata)
    ...
    def fetch_data(self, pair, limit=600, timeframe="M1"):
        data = fetch_new_api_candles(pair, count=limit, timeframe=timeframe, broker=self.broker)
    ...
    def send_signal_with_chart(
        ...
        signal_text = build_signal_message(
            ...
            owner_name=get_name_template(self.uid))
    ...
    def display_time(self, dt_utc5):
        """Convert an internal UTC+5-anchored datetime (used for exact
        candle matching, must never change) into the wall-clock string the
        user should actually see, per this bot's tz_feature setting."""
        return (dt_utc5 - timedelta(hours=5) + get_tz_delta(self.uid, self.tz_feature)).strftime("%H:%M")

    def fetch_candle_at_time_with_retry(
            self, pair, target_dt_utc5, limit=750, attempts=25, delay=3, timeframe="M1"):
        """
        ...
        for i in range(attempts):
            ...
            if candles:
                ...
                if found:
                    print(f"[CANDLE RETRY OK] {pair} matched on attempt {i+1}/{attempts} (t+{elapsed}s)")
                    return candles, found
                nearest = min(candles, key=lambda c: abs(c['time'] - int((target_dt_utc5 - timedelta(hours=5)).timestamp())))
                gap = nearest['time'] - int((target_dt_utc5 - timedelta(hours=5)).timestamp())
                print(f"[CANDLE RETRY MISS] {pair} attempt {i+1}/{attempts} (t+{elapsed}s) nearest_gap={gap}s")
            else:
                print(f"[CANDLE RETRY EMPTY] {pair} attempt {i+1}/{attempts} (t+{elapsed}s) fetch_data returned no candles")
            if get_state(self.uid).stop_requested:
                print(f"[CANDLE RETRY ABORTED] {pair} stop_requested at attempt {i+1}/{attempts} (t+{elapsed}s)")
                break
            time.sleep(delay)
        print(f"[CANDLE RETRY GAVE UP] {pair} after {attempts} attempts (t+{round(time.monotonic()-t_start,1)}s)")
        return candles, None
            ...
            for idx, pair in enumerate(self.pairs):
                ...
                if bias:
                    ...
                    if self.same_pair_count > 2:
                        continue
                    entry_t = self.display_time(entry_dt)
                    sender.edit_message(
    ...
    def handle_signal_result(
        ...
        if not candles:
            sender.send_message(
                self.uid,
                f"⚠️ {pair} {self.display_time(entry_dt_utc5)}: couldn't fetch candles to verify result (API issue). Skipped.")
            return
        if not first:
            sender.send_message(
                self.uid,
                f"⚠️ {pair} {self.display_time(entry_dt_utc5)}: couldn't find the entry candle to verify result. Skipped.")
            return
        win1 = (
            first['close'] > first['open']) if direction == "CALL" else (
            first['close'] < first['open'])
        ...
        st.signal_history.append({'pair': pair,
                                  'direction': direction,
                                  'time': self.display_time(entry_dt_utc5),
                                  'result': "WIN" if win1 else "LOSS",
                                  'type': trade_type})
        if not win1:
            st.last_loss[pair] = datetime.now(timezone.utc)
        if win1:
            st.stats['wins'] += 1
            record_trade_result(self.uid, True)
            record_trade_hour(self.uid, self.broker, True)
            self.send_result_with_chart(
                pair,
                self.display_time(entry_dt_utc5),
                first,
                None,
                payout,
                "WIN",
        ...
        if not candles2:
            sender.send_message(
                self.uid,
                f"⚠️ {pair} {self.display_time(entry_dt_utc5)} MTG: couldn't fetch candles to verify result (API issue). Skipped.")
            return
        if not second:
            sender.send_message(
                self.uid,
                f"⚠️ {pair} {self.display_time(entry_dt_utc5)} MTG: couldn't find the MTG candle to verify result. Skipped.")
            return
        win2 = (
            second['close'] > second['open']) if direction == "CALL" else (
            second['close'] < second['open'])
        if win2:
            ...
            st.stats['losses'] += 1
            record_trade_result(self.uid, False)
            record_trade_hour(self.uid, self.broker, False)
            self.send_result_with_chart(
                pair,
                self.display_time(entry_dt_utc5),
                first,
...
async def run_advanced_checker(update: Update, context: ContextTypes.DEFAULT_TYPE, signals, mtg_level, date_str, user_utc, payout_filter, broker="quotex"):
    from datetime import datetime, timedelta, timezone
    import re
    from concurrent.futures import ThreadPoolExecutor, as_completed
    import threading
    ...
    # Custom dates can go up to 25 days back, so scale how many candles we
    # ask for accordingly (M1 candles: ~1440/day). Today/Yesterday keep the
    # old 3000 (~2 days) since that's all they ever need.
    days_back = max(1, (now_utc5.date() - date_utc5.date()).days)
    fetch_count = min(40000, max(3000, (days_back + 2) * 1440))
    print(f"🗓 CHECKER: date is {days_back} day(s) back -> requesting {fetch_count} candles")

    def fetch_pair(api):
        result = fetch_beaxt_candles(api, count=fetch_count, retries=3, timeout=25, broker=broker)
        if not result:
    ...
    def _fetch_all_pairs():
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(futures):
                ...
                local_cache[api] = cbs
                local_full[api] = tsl
        return local_cache, local_full

    cache, full_lookup = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, _fetch_all_pairs)
...
def run_trend_filter(uid, signals, mode, ema_period, context, broker="quotex"):
    """
    Trend Filter – background thread.
    signals: list of (pair, time, direction)
    mode: "EMA" or "SMA"
    ema_period: int (20, 40, 100, 200, custom)
    """
    import time as ttime
    from datetime import datetime, timedelta, timezone
    from concurrent.futures import ThreadPoolExecutor, as_completed
    ...
    total = len(signals)
    if total == 0:
        sender.send_message(uid, "❌ No signals to process.")
        return
    
    progress_msg = sender.send_message(uid, "⏳ Processing signals...\n🔄 0% [░░░░░░░░░░] 0/0")
    if not progress_msg:
        return
    msg_id = progress_msg.id
    
    def update_progress(current, total, pair="", found=0, elapsed=0):
        pct = int((current / total) * 100) if total > 0 else 0
        bar = "█" * (pct // 10) + "░" * (10 - pct // 10)
        elapsed_str = f"{elapsed // 60}:{elapsed % 60:02d}"
        text = (
            f"⏳ {fancy_font('Processing signals...')}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔄 {pct}%  [{bar}]  {current}/{total}\n"
            f"⏱ {elapsed_str}  |  📊 {pair}\n"
            f"💎 {fancy_font('Found:')} {found} signals"
        )
        try:
    ...
    # Group signals by pair to fetch candles once per pair
    pair_signals = {}
    for pair, tm, direction in signals:
        pair_signals.setdefault(pair, []).append((tm, direction))
    
    # Fetch candles for each pair in parallel
    pair_candles = {}
    def fetch_pair(pair):
        data = fetch_new_api_candles(pair, count=600, timeframe="M1", broker=broker)
        if data and data.get('candles'):
            return pair, data['candles']
        return pair, None
    
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(fetch_pair, p): p for p in pair_signals.keys()}
        done = 0
        for future in as_completed(futures):
            pair, candles = future.result()
            pair_candles[pair] = candles
            done += 1
            elapsed = int(ttime.time() - start_time)
            update_progress(done, len(pair_signals), pair, len(accepted)+len(rejected), elapsed)
    
    # Process each signal
    for pair, tm, direction in signals:
        candles = pair_candles.get(pair)
        if not candles:
            rejected.append((pair, tm, direction, "No candles"))
            continue
        
        try:
            now_utc5 = datetime.now(timezone(get_tz_delta(uid, "trendfilter")))
            base_date = now_utc5.strftime("%Y-%m-%d")
            dt_str = f"{base_date} {tm}"
            entry_dt = datetime.strptime(dt_str, "%Y-%m-%d %H:%M")
            entry_dt = entry_dt.replace(tzinfo=timezone(get_tz_delta(uid, "trendfilter")))
            target_ts = int((entry_dt - get_tz_delta(uid, "trendfilter")).timestamp())
        except Exception:
            rejected.append((pair, tm, direction, "Invalid time"))
            continue
        
        entry_candle = None
        entry_idx = None
        for i, c in enumerate(candles):
            if abs(c['time'] - target_ts) < 60:
                entry_candle = c
                entry_idx = i
                break
        
        if entry_candle is None or entry_idx is None or entry_idx < ema_period:
            rejected.append((pair, tm, direction, "Not enough history"))
            continue
        
        price_window = [c['close'] for c in candles[entry_idx - ema_period : entry_idx]]
        if len(price_window) < ema_period:
            rejected.append((pair, tm, direction, "Not enough history"))
            continue
        ...
        if mode == "EMA":
            def ema(prices, period):
                ...
                return ema_val
            trend_val = ema(price_window + [entry_candle['close']], ema_period)
        else:  # SMA
            trend_val = sum(price_window) / len(price_window)
        
        if trend_val is None:
            rejected.append((pair, tm, direction, "Calculation error"))
            continue
        
        entry_close = entry_candle['close']
        if entry_close > trend_val:
            trend_dir = "UP"
        elif entry_close < trend_val:
            trend_dir = "DOWN"
        else:
            trend_dir = "SIDEWAYS"
        
        if (direction == "CALL" and trend_dir == "UP") or (direction == "PUT" and trend_dir == "DOWN"):
            accepted.append((pair, tm, direction))
        else:
            rejected.append((pair, tm, direction, f"Trend {trend_dir}"))
        
        processed = len(accepted) + len(rejected)
        elapsed = int(ttime.time() - start_time)
        update_progress(processed, total, pair, len(accepted), elapsed)
    
    # Final loading message
    try:
        sender.edit_message(uid, msg_id, "✅ Processing complete!\n📊 Generating results...")
    except:
        pass
    ...
    acceptance_rate = (total_accepted / total_signals * 100) if total_signals > 0 else 0
    
    header = (
        f"📊 {fancy_font('TREND FILTER RESULTS')}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ {fancy_font('Accepted:')} {total_accepted}/{total_signals}\n\n"
    )
    
    accepted_lines = [f"M1;{p};{t};{d}" for p, t, d in accepted]
    body_accepted = "\n".join(accepted_lines) if accepted_lines else "—— None ——"
    
    rejected_lines = [f"M1;{p};{t};{d}  (Reason: {r})" for p, t, d, r in rejected]
    body_rejected = "\n".join(rejected_lines) if rejected_lines else "—— None ——"
    
    summary = (
        f"\n━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"❌ {fancy_font('Rejected:')} {total_rejected}\n\n"
        f"{body_rejected}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 {fancy_font('Acceptance Rate:')} {acceptance_rate:.1f}%\n"
        f"📈 {fancy_font('Mode:')} {mode} (Period: {ema_period})\n"
        f"⏱ {fancy_font('Processed in:')} {int(ttime.time() - start_time)}s"
    )
    
    full_text = header + body_accepted + "\n" + summary
    ...
    header_len = len(header.encode('utf-16-le')) // 2
    summary_start = len((header + body_accepted + "\n").encode('utf-16-le')) // 2
    summary_len = len(summary.encode('utf-16-le')) // 2
    entities.append(MessageEntity(type='bold', offset=0, length=header_len))
    entities.append(MessageEntity(type='bold', offset=summary_start, length=summary_len))
    
    buttons = [[colored_button(" Back to Menu", "back_to_main", KeyboardButtonStyle.SUCCESS, "5416041192905265756")]]
    reply_markup = InlineKeyboardMarkup(buttons)
...
async def show_loss_candles(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not loss_data_list:
        await safe_query_answer(query, "No loss signals to display.", show_alert=True)
        return
    ...
    await safe_query_answer(query, "Generating loss charts...")
...
async def run_backtest_local(
    ...
    signals = []
    if broker == "pocket":
        supported_pairs = POCKET_BACKTEST_SUPPORTED_PAIRS
    elif broker == "tradowix":
        supported_pairs = TRADOWIX_BACKTEST_SUPPORTED_PAIRS
    else:
        supported_pairs = BACKTEST_SUPPORTED_PAIRS
    ...
    # Candle count MUST scale with the selected day range, not be fixed.
    # M1 timeframe = 1 candle/minute, so `days` worth of history needs
    # days*1440 candles. +1 extra day as a safety buffer (API boundary /
    # timezone rounding) so the oldest selected day is never short.
    candle_count = (days + 1) * 1440
    cache = {}
    for sig in signals:
        api = sig['api_pair']
        if api in cache:
            continue
        # `api` is already built above as e.g. "ETHUSD_otc", which matches
        # the qxcandledata/pocandledata API's OTC pair format directly.
        if broker == "pocket":
            urls_to_try = [f"{POCKET_CANDLE_API_BASE}?pair={api}&timeframe=M1&count={candle_count}"]
        elif broker == "tradowix":
            urls_to_try = [f"{TRADOWIX_CANDLE_API_BASE}?pair={api}&timeframe=M1&count={candle_count}"]
        else:
            # Quotex: qxcandledata.beaxtapi.online first, then automatically shift
            # to the mrbeaxt.com backup if it fails.
            urls_to_try = [
                f"{NEW_CANDLE_API_BASE}?pair={api}&timeframe=M1&count={candle_count}",
                f"{NEW_CANDLE_API_FALLBACK_BASE}?pair={api}&timeframe=M1&count={candle_count}",
            ]
        data = None
        for url in urls_to_try:
            try:
                # Was a blocking requests.get() sitting directly inside an async
                # handler on MAIN_LOOP — for every unique pair (up to 20s each)
                # this froze the bot's single event loop for ALL users, not just
                # the one running backtest, causing bot-wide "Send error:" /
                # "[SEND RETRY] waiting for MAIN_LOOP" pile-ups elsewhere.
                # asyncio.to_thread() runs it off the event loop instead.
                resp = await MAIN_LOOP.run_in_executor(
                    _NETWORK_EXECUTOR, lambda u=url: _HTTP.get(u, timeout=20))
                if resp.status_code != 200:
                    print(f"Backtest: HTTP {resp.status_code} for {api} ({url})")
                    continue
                candidate = resp.json()
                if not candidate.get('success') or not candidate.get('data'):
                    print(f"Backtest: No candles for {api} ({url})")
                    continue
                data = candidate
                break
            except Exception as e:
                # A timeout/connection error on THIS url (e.g. mrbeaxt.com
                # down/slow) must not stop the loop from trying the next
                # url (the qxcandledata fallback) — previously this except
                # wrapped the whole loop, so one bad primary request meant
                # the fallback was never attempted at all.
                print(f"Backtest error {api} ({url}): {e}")
                continue
        if data is None:
            cache[api] = None
            continue
        try:
            candles = data['data']
            # Build lookup with PK time. The API's own 'time' string is
            # UTC+6, but 'epoch' is a true UTC unix timestamp — convert
            # epoch -> UTC+5 ourselves so this matches the bot's
            # Pakistan-time convention everywhere else (previously the
            # old GitHub source's 'time' field was already UTC+5).
            lookup = {}
            for c in candles:
                epoch = c.get('epoch')
                if epoch is None:
                ...
                try:
                    ...
                    lookup[(date_ymd, hhmm)] = {
                        'time': dt_pk.strftime("%Y-%m-%d %H:%M:%S"),
                        'open': o,
                        'close': cl,
                        'high': float(c.get('high', 0)),
                        'low': float(c.get('low', 0)),
...
def run_ai_filter_pattern_match(uid, signals_text, threshold, context):
    ...
    # User's chosen offset for the "AI Filter FS" timezone setting — the
    # candle time-matching below uses this instead of the fixed per-broker
    # shift baked into fetch_new_api_candles()'s 'readable_time' field.
    user_tz_delta = get_tz_delta(uid, "aifilterfs")
    ...
    # ----- Fetch historical candles for each unique pair — in PARALLEL, with
    # an extra outer retry on top of fetch_new_api_candles()'s own internal
    # primary/fallback retries, so scanning many pairs doesn't turn into a
    # slow one-by-one sequential wait. -----
    def _fetch_pair_candles(api):
        data = None
        for attempt in range(3):  # outer retry loop (in addition to internal retries)
            try:
                data = fetch_new_api_candles(api, count=2000)
            except Exception as e:
                print(f"AI Filter fetch error {api} (attempt {attempt + 1}): {e}")
                data = None
            if data and data.get('candles'):
                break
            time.sleep(0.3 * (attempt + 1))  # small backoff before retrying
        try:
            ...
            candles.sort(key=lambda x: x['time'])
            for c in candles:
                dt_user = datetime.fromtimestamp(c['time'], tz=timezone.utc) + user_tz_delta
                c['hhmm'] = dt_user.strftime("%H:%M")
            return api, candles
        except Exception as e:
    ...
    seen_pairs = set()
    for sig in signals:
        if sig['api_pair'] not in seen_pairs:
            seen_pairs.add(sig['api_pair'])
            unique_pairs.append(sig['api_pair'])

    cache = {}
    futures = [_NETWORK_EXECUTOR.submit(_fetch_pair_candles, api) for api in unique_pairs]
    for fut in _cf.as_completed(futures):
        api, candles = fut.result()
        cache[api] = candles

    # ----- Helper: normalize an N-candle segment for pattern comparison -----
    ...
    def similarity(pattern1, pattern2):
        if not pattern1 or not pattern2 or len(pattern1) != len(pattern2):
            return 1e9
        return math.sqrt(sum((a - b) ** 2 for a, b in zip(pattern1, pattern2)))

    # ----- Multi-length confirmation settings -----
    MIN_MATCHES = 2                  # minimum non-overlapping historical matches required (relaxed further)
    MAX_MATCHES = 60                 # cap on how many non-overlapping matches we collect

    def find_non_overlapping_matches(candles, current_pattern, pattern_start, plen):
        """Rank all candidate segments by similarity, then greedily keep only
        non-overlapping ones (gap >= half the pattern length from every
        already-chosen match) so the same real-world event isn't double
        counted, without being so strict that it starves the match pool."""
        gap = max(plen // 2, 5)  # relaxed from a full-plen gap so more matches qualify
        similarities = []
        for i in range(0, len(candles) - plen):
            if abs(i - pattern_start) < plen:
                continue  # skip the source pattern itself / near-overlap of it
            seg = candles[i:i + plen]
        ...
        chosen = []
        for dist, i in similarities:
            if len(chosen) >= MAX_MATCHES:
                break
            if all(abs(i - c_i) >= gap for _, c_i in chosen):
                chosen.append((dist, i))
        return chosen
    ...
    for idx, sig in enumerate(signals):
        ...
        # ----- Simple, single-check decision: does this 20-candle pattern's
        # historical win-rate beat the threshold? No multi-length voting, no
        # secondary confirmation — just the core win-rate match. -----
        PATTERN_LEN = 20
        pattern_start = target_idx - PATTERN_LEN
        if pattern_start < 0:
            rejected.append(f"M1 {display} {time_str} {direction}")
            continue
        ...
        chosen = find_non_overlapping_matches(candles, current_pattern, pattern_start, PATTERN_LEN)
        ...
        win_count = 0
        total_count = 0
        for dist, idx_match in chosen:
            outcome_idx = idx_match + PATTERN_LEN
    ...
    header_lines = build_ai_filter_header(
        system_label="CLOUD OPOUS 4.8",
        original_count=len(signals),
        passed_count=len(accepted),
        failed_count=len(rejected),
    )
    passed_body = build_ai_filter_signal_lines("📋 Passed Signals:", accepted, "No accepted signals")
    nightmare_footer = [f"💀{fancy_font('SMZX NIGHTMARE')}👻"]
    send_ai_filter_section(sender, uid, header_lines, passed_body, footer_lines=nightmare_footer)
...
def convert_time_offset(time_str, from_offset, to_offset):
    ...
    (may be fractional, e.g. 5.5 for UTC+05:30)."""
    h, m = map(int, time_str.split(':'))
    total_min = int(round(h * 60 + m + (to_offset - from_offset) * 60))
    total_min %= 24 * 60
    return f"{total_min // 60:02d}:{total_min % 60:02d}"

def _run_future_signals_bg(uid, min_conf, start_time, end_time, selected_pairs):
    """Runs generate_future_signals off MAIN_LOOP (background thread) and
    sends the result itself, so the caller never blocks the event loop."""
    try:
        result = generate_future_signals(uid, min_conf, start_time, end_time, selected_pairs=selected_pairs)
        if result:
        ...
        import traceback
        print(f"[FUTURE SIGNALS ERROR] {e}")
        traceback.print_exc()
        try:
            sender.send_message(uid, f"⚠️ Error generating future signals: {str(e)[:80]}")
        except Exception:
            pass
...
def generate_future_signals(uid, min_conf=75, start_time="00:00", end_time="23:59", selected_pairs=None):
    ...
    all_signals = []
    # User's chosen offset (hours) for the "OTC Future Signal" feature —
    # was previously hardcoded to +5, ignoring Settings > UTC Selection.
    user_tz_hours = get_tz_minutes(uid, "otcfuture") / 60
    # Convert start/end times from the user's offset to UTC+6 (API)
    api_start = convert_time_offset(start_time, user_tz_hours, 6)
    api_end = convert_time_offset(end_time, user_tz_hours, 6)
        ...
        try:
            ...
            for sig in data.get("signals", []):
                try:
                    ...
                    acc = 0
                if acc >= min_conf:
                    # Signal time is in UTC+6, convert to the user's chosen offset for display
                    sig_time_utc6 = sig.get("time", "??:??")
                    display_time = convert_time_offset(sig_time_utc6, 6, user_tz_hours)
                    all_signals.append({
                        'time': display_time,
                        'pair': pair,
    ...
    now_user = datetime.now(timezone.utc) + get_tz_delta(uid, "otcfuture")
    date_str = now_user.strftime("%Y-%m-%d")
    tz_label = format_utc_offset(get_tz_minutes(uid, "otcfuture"))
    header = (
        f"{fancy_font('🏐 𝚂𝙼𝚉 𝙱𝙾𝚃 𝙵𝚄𝚃𝚄𝚁𝙴 🏐')}\n\n"
        f"{fancy_font('🗓 ')}{fancy_font(date_str)}\n\n"
        f"{fancy_font('💎 𝚃𝚒𝚖𝚎𝚣𝚘𝚗𝚎: ')}{fancy_font(tz_label)}\n\n"
        f"{fancy_font('⏳ 𝚃𝙸𝙼𝙴𝙵𝚁𝙰𝙼𝙴: 𝙼𝟷')}\n"
        f"{fancy_font('⏰ 𝚄𝚂𝙴 𝙼𝚃𝙶 𝙾𝙽𝙴 𝙸𝙵 𝚁𝙴𝚀𝚄𝙸𝚁𝙴𝙳')}\n\n"
        f"{fancy_font('━━━━━━━━━━━ • ━━━━━━━━━━━')}\n"
    )
...
def run_pair_payout(uid, context):
    ...
    def _one(pair):
        ...
        if payout != "!" and isinstance(payout, (int, float)):
            return f"🎥 {pair} : 💲 {payout}%"
        return None
    result_lines = []
    with _cf.ThreadPoolExecutor(max_workers=12) as pool:
        for line in pool.map(_one, FUT_PAIRS):
            if line:
    ...
    try:
        if loading_msg and loading_msg.id:
...
def run_market_trend(uid, context):
    ...
    result_lines = []
    def _one(pair):
        return pair, get_trend_from_candles(pair)
    with _cf.ThreadPoolExecutor(max_workers=12) as pool:
        for pair, trend in pool.map(_one, FUT_PAIRS):
            if trend is None:
                continue
            if trend == 'up':
                emoji = '📉'
...
def run_candle_colors(uid, context):
    ...
    def _one(pair):
        ...
        colors = []
        for c in candles:
            colors.append('🟢' if c['close'] >= c['open'] else '🔴')
        return f"➡️ {pair} : {''.join(colors)}"
    with _cf.ThreadPoolExecutor(max_workers=12) as pool:
        for line in pool.map(_one, FUT_PAIRS):
            if line:
                result_lines.append(line)
...
# ─── MRBEAXT API ──────────────────────────────────────────
def fetch_candles_mrbeaxt(pair: str, count: int = 300, timeframe: str = "1m", broker: str = "quotex"):
    """
    Fetch candles using the mrbeaxt.com (Quotex) or pocandledata (Pocket
    Option) candle API, depending on `broker`.
    timeframe: '1m'/'M1', '5m'/'M5', etc. (default '1m')
    Returns: (candles_ascending, current_price, payout)
    """
    print(f"🔍 FETCH: {pair} → API: {to_new_api_pair(pair)} (tf={timeframe}, broker={broker})")

    data = fetch_new_api_candles(pair, count=count, timeframe=timeframe, broker=broker)
...
def fetch_candle_at_epoch_with_retry(pair, target_epoch, count=50, timeframe="1m",
                                      tolerance=45, attempts=8, delay=1.2, broker="quotex"):
    """Retry-aware lookup for a specific candle by target UTC epoch.
    mrbeaxt.com can take a couple of seconds to publish a just-closed
    ...
    `broker` MUST match the broker the signal was generated on — the
    entry candle only lines up with the signal on that same broker's
    feed. Callers must pass through whatever broker they used to
    generate the signal instead of relying on the "quotex" default."""
    candles = None
    for attempt in range(attempts):
        candles, _, _ = fetch_candles_mrbeaxt(pair, count=count, timeframe=timeframe, broker=broker)
        if candles:
            match = next(
...
async def show_multi_pair_list(update: Update, context: ContextTypes.DEFAULT_TYPE, pairs: list, manual: bool, pair_data: list = None):
    ...
    if pair_data is None:
        ...
        async def fetch_payout(pair):
            _, _, payout = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, fetch_candles_mrbeaxt, pair, 1)
            return pair, payout
...
def run_multi_manual_analysis(uid: int, pair: str, context: ContextTypes.DEFAULT_TYPE):
    ...
    broker = context.user_data.get('multi_broker', 'quotex')
    ...
    candles, price, payout = fetch_candles_mrbeaxt(pair, count=300, broker=broker)
    if not candles:
    ...
    if direction and confidence is not None:
        ...
        _send_manual_signal_and_result(uid, pair, candles, direction, entry_dt, confidence, payout, context, broker=broker)
        return
...
def handle_multi_result(uid: int, pair: str, entry_dt: datetime, direction: str, payout: int,
                        ...
                        broker: str = "quotex"):
    wins = losses = 0
    # Multi Engine's display/entry timezone — configurable via ⚙️ Settings.
    # Must be the SAME delta used to build entry_dt below when converting
    # it back to a UTC epoch, or the candle lookup will target the wrong
    # minute entirely.
    tz_delta = get_tz_delta(uid, "multiengine")

    # ─── Wait 1 minute ──────────────────────────────────
    t1 = entry_dt + timedelta(minutes=1)
    while datetime.now(timezone.utc) + tz_delta < t1:
        time.sleep(1)

    entry_epoch = int((entry_dt - tz_delta).timestamp())
    candles, c1 = fetch_candle_at_epoch_with_retry(pair, entry_epoch, broker=broker)
    if not candles:
    ...
    chart_path = None
    result_type = None
    owner = (context.user_data.get('multi_owner_name') if context else None) or get_name_template(uid)

    if win1:
        wins += 1
        result_type = "WIN"
        result_text = build_result_format3(pair, entry_dt.strftime("%H:%M"), direction, "WIN",
                                           c1['open'], c1['close'], c1_color, wins, losses, owner_name=owner)
        ...
        while datetime.now(timezone.utc) + tz_delta < t2:
            time.sleep(1)

        mtg_epoch = int((entry_dt + timedelta(minutes=1) - tz_delta).timestamp())
        candles2, c2 = fetch_candle_at_epoch_with_retry(pair, mtg_epoch, broker=broker)
        if not candles2:
        ...
        if win2:
            ...
            result_text = build_result_format3(pair, entry_dt.strftime("%H:%M"), direction, "MTG WIN",
                                               c2['open'], c2['close'], c2_color, wins, losses, owner_name=owner)
...
async def start_multi_nonstop(update: Update, context: ContextTypes.DEFAULT_TYPE, pairs: list):
    ...
    # Stale-thread guard (same idea as auto_signal's session_token): if an
    # old thread from a previous session wakes up late from a slow network
    # call, it must recognize it's no longer current instead of continuing
    # to run alongside a fresh session for the same user.
    multi_session_token = context.user_data.get('multi_session_token', 0) + 1
    context.user_data['multi_session_token'] = multi_session_token
    context.user_data['multi_paused'] = False
    ...
    broker = context.user_data.get('multi_broker', 'quotex')
    thread = threading.Thread(target=multi_nonstop_loop, args=(uid, context, multi_session_token, 1, broker), daemon=True)
    thread.start()
...
def _send_manual_signal_and_result(uid: int, pair: str, candles: list,
    ...
    owner = context.user_data.get('multi_owner_name') or get_name_template(uid)  # ← NEW
    now_pk = datetime.now(timezone.utc) + get_tz_delta(uid, "multiengine")
    entry_time = (now_pk + timedelta(minutes=1)).replace(second=0, microsecond=0).strftime("%H:%M")
    confidence = int(confidence)

    signal_text = build_signal_format3(pair, entry_time, direction, payout, owner_name=owner,
                                       tz_label=format_utc_offset(get_tz_minutes(uid, "multiengine")))  # ← PASS OWNER
    ...
    handle_multi_result(uid, pair, entry_dt, direction, payout, context, is_manual=True, broker=broker)
...
async def show_multi_ns_strategy_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show Strategy 01 vs Strategy 02 selection for Multi Engine Non-Stop"""
    query = update.callback_query
    await safe_query_answer(query)
    
    context.user_data['state'] = STATE_MULTI_NONSTOP_STRATEGY
    
    msg = (
        f"🎯 {fancy_font('SELECT MULTI ENGINE STRATEGY')}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📌 Choose which strategy to run:\n\n"
        f"🔹 {fancy_font('Strategy 01')} – Current SuperTrend Reversal\n"
        f"🔹 {fancy_font('Strategy 02')} – Flexible Pullback (NEW!)\n"
    )
    buttons = [
        [
            colored_button(" Strategy 01 ", "multi_ns_strategy_1", KeyboardButtonStyle.PRIMARY, "6145553439809084250"),
            colored_button(" Strategy 02 ", "multi_ns_strategy_2", KeyboardButtonStyle.SUCCESS, "6147654280112248427")
        ],
        [colored_button(" Back ", "multi_back_to_main", KeyboardButtonStyle.DANGER, "6145317070578916456")]
    ]
    markup = InlineKeyboardMarkup(buttons)
    entities = build_custom_emoji_entities(msg)
    await query.edit_message_text(msg, entities=entities, reply_markup=markup)

async def start_multi_nonstop_with_strategy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start Non-Stop loop with selected strategy"""
    query = update.callback_query
    uid = query.from_user.id
    
    # Get pairs based on filter
    market = context.user_data.get('multi_ns_market', 'otc')
    filter_type = context.user_data.get('multi_ns_filter', 'all')
    strategy = context.user_data.get('multi_ns_strategy', 1)
    broker = context.user_data.get('multi_broker', 'quotex')
    
    if filter_type == 'manual':
        pairs = list(context.user_data.get('multi_selected_pairs', []))
        if not pairs:
            await safe_query_answer(query, "❌ No pairs selected!", show_alert=True)
            return
    elif market == 'otc':
        pairs = multi_engine_pairs(broker, 'otc')
    elif market == 'live':
        pairs = multi_engine_pairs(broker, 'live')
    else:
        pairs = multi_engine_pairs(broker, 'otc') + multi_engine_pairs(broker, 'live')
    
    if filter_type == 'avoid':
        # Filter pairs with payout >= 80%
        filtered = []
        for p in pairs:
            _, _, payout = fetch_candles_mrbeaxt(p, count=1, broker=broker)
            if payout >= 80:
                filtered.append(p)
        pairs = filtered
        if not pairs:
            await query.edit_message_text("❌ No pairs with payout >= 80%.")
            return
    ...
    # Session token for stale thread guard
    multi_session_token = context.user_data.get('multi_session_token', 0) + 1
    context.user_data['multi_session_token'] = multi_session_token
    ...
    msg_text = (
        "🔁 Non‑Stop Signal Running\n\n"
        f"📊 Pairs: {len(pairs)}\n"
        f"📈 Strategy: {'01 (SuperTrend)' if strategy == 1 else '02 (Pullback)'}\n"
        f"🍏 Status: RUNNING\n"
        f"🗽 Wins: 0  |  Losses: 0\n"
        f"─────────────────────\n"
        f"Use buttons below."
    ...
    await query.edit_message_text("⏳ Non‑Stop started with Strategy {}.".format(strategy))
    
    # Start background thread with strategy parameter
    broker = context.user_data.get('multi_broker', 'quotex')
    thread = threading.Thread(
        target=multi_nonstop_loop,
        args=(uid, context, multi_session_token, strategy, broker),  # ✅ strategy + broker passed here
        daemon=True
    )
    thread.start()

def multi_nonstop_loop(uid, context, session_token, strategy_type=1, broker="quotex"):
    """Background loop with strategy_type: 1=SuperTrend, 2=New Pullback.
    `broker` picks which candle/tick API this whole loop talks to — the
    pairs in `context.user_data['multi_pairs']` must already belong to
    this same broker (see multi_engine_pairs())."""
    ...
    if session_token is None:
        session_token = context.user_data.get('multi_session_token', 0)
    
    idx = 0
    total = len(pairs)
    strategy_name = "SuperTrend Reversal" if strategy_type == 1 else "Pullback Confirmation"
    consecutive_flood_errors = 0
    MAX_CONSECUTIVE_ERRORS = 5  # only counts flood-control errors — after this many in a row, stop

    try:
        while context.user_data.get('multi_running', False):
            try:
                # Stale thread guard
                if context.user_data.get('multi_session_token', 0) != session_token:
                    print(f"[MULTI LOOP] Stale thread for uid={uid}, exiting.")
                    break
                ...
                _update_status(uid, context, f"Scanning {pair} ({idx}/{total}) [{strategy_name}]")
                ...
                if strategy_type == 1:
                    ...
                    if not direction:
                        _update_status(uid, context, f"{pair} – {details.get('reject', 'NO_SETUP')}")
                        time.sleep(0.3)
                        continue
                ...
                # ===== Strategy 02 =====
                    ...
                    ticks = fetch_live_ticks(pair, limit=50, broker=broker)
                    direction, score, details = detect_pullback_with_ticks(asc, ticks, pair)
                    if not direction:
                        reason = details.get('reason', 'No pattern')
                        _update_status(uid, context, f"{pair} – {reason}")
                        time.sleep(0.3)
                        continue
                
                # ==== SIGNAL FOUND ====
                # Multi Engine's display/entry timezone — configurable via
                # ⚙️ Settings. Reused below to convert entry_dt back to a
                # UTC epoch, so it must stay the SAME value for the whole
                # signal→result cycle.
                tz_delta = get_tz_delta(uid, "multiengine")
                now_pk = datetime.now(timezone.utc) + tz_delta
                entry_dt = (now_pk + timedelta(minutes=1)).replace(second=0, microsecond=0)
                entry_time = entry_dt.strftime("%H:%M")
                confidence = int(score) if score else 80
                
                _update_status(uid, context, f"🔥 SIGNAL on {pair} – {direction} @ {entry_time} ({confidence}%)")
                
                # Send signal
                owner = context.user_data.get('multi_owner_name') or get_name_template(uid)
                signal_text = build_signal_format3(pair, entry_time, direction, payout,
                                                   owner_name=owner,
                                                   tz_label=format_utc_offset(get_tz_minutes(uid, "multiengine")))
                ...
                # Wait for 1st candle result
                t1 = entry_dt + timedelta(minutes=1)
                while datetime.now(timezone.utc) + tz_delta < t1:
                    time.sleep(1)
                
                entry_epoch = int((entry_dt - tz_delta).timestamp())
                candles1, c1 = fetch_candle_at_epoch_with_retry(pair, entry_epoch, broker=broker)
                ...
                if candles1 and c1:
                    ...
                    if win1:
                        result_text = build_result_format3(pair, entry_time, direction, "WIN",
                                                           c1['open'], c1['close'], c1_color, wins+1, losses, owner_name=owner)
                ...
                if candles2 and c2:
                    ...
                    if win2:
                        result_text = build_result_format3(pair, entry_time, direction, "MTG WIN",
                                                           c2['open'], c2['close'], c2_color, wins+1, losses, owner_name=owner)
                        ...
                        result_text = build_result_format3(pair, entry_time, direction, "LOSS",
                                                           c2['open'], c2['close'], c2_color, wins, losses+1, owner_name=owner)
                        chart_path = safe_draw_pro_chart_timeout(candles2, pair, entry_time, direction, payout,
                    ...
                    sender.send_message(uid, f"⚠️ {pair} MTG: couldn't fetch candles.")
                
                _update_status(uid, context, f"Cycle complete for {pair}")
                time.sleep(2)
                consecutive_flood_errors = 0  # a full cycle completed without raising — reset the streak

            except Exception as inner_e:
                import traceback
                print(f"Non‑stop loop inner error: {inner_e}")
                traceback.print_exc()

                err_str = str(inner_e).lower()
                if "flood" in err_str or "retry after" in err_str or "retryafter" in err_str:
                    # Being flood-blocked by Telegram — retrying in 2s just
                    # keeps hammering the same blocked chat and can make the
                    # ban worse/longer. Back off hard, and ONLY this specific
                    # failure type counts toward the circuit breaker below —
                    # ordinary transient errors keep retrying forever exactly
                    # like before, since those usually self-recover and
                    # stopping on them was cutting off most users' sessions
                    # over brief, harmless blips.
                    consecutive_flood_errors += 1
                    print(f"[MULTI LOOP] Flood control hit for uid={uid} — backing off 60s "
                          f"(consecutive_flood_errors={consecutive_flood_errors}).")
                    time.sleep(60)
                    if consecutive_flood_errors >= MAX_CONSECUTIVE_ERRORS:
                        print(f"[MULTI LOOP] uid={uid} hit {consecutive_flood_errors} consecutive "
                              f"flood-control errors — stopping session instead of retrying forever.")
                        context.user_data['multi_running'] = False
                        try:
                            sender.send_message(
                                uid,
                                f"🔴 Non-Stop Signal stopped — Telegram is rate-limiting messages to "
                                f"you. Please restart once that clears.")
                        except Exception:
                            pass
                        break
                else:
                    try:
                        _update_status(uid, context, f"⚠️ Error: {str(inner_e)[:50]} – continuing...")
                    except: pass
                    time.sleep(2)
    
    finally:
        # 🔥 CRITICAL: Ensure cleanup happens even if loop crashes
        context.user_data['multi_running'] = False
        context.user_data['multi_paused'] = False
        print(f"[MULTI LOOP] Cleaned up Multi Engine for user {uid}")
        # Also clear status message if any
        try:
            sender.delete_message(uid, context.user_data.get('multi_status_msg_id', 0))
        except: pass
        # Send final message if loop ended normally
        if context.user_data.get('multi_session_token', 0) == session_token:
            sender.send_message(uid, "🔴 Non‑Stop Signal stopped.")

def detect_pullback_with_ticks(candles, ticks, pair="UNKNOWN"):
    """
    Strategy 02: Flexible Pullback + Supertrend + Tick Confirmation
    Running candle counts as pullback if ticks confirm
    Returns: (direction, confidence, details) or (None, 0, details)
    """
    if len(candles) < 10 or not ticks:
        return None, 0, {'reason': 'Insufficient data'}
    
    details = {}
    
    # 1. Supertrend
    trend, st_line = calc_supertrend(candles)
    if trend[-1] is None:
        return None, 0, {'reason': 'No Supertrend'}
    
    current_trend = trend[-1]
    details['trend'] = 'UP' if current_trend == 'up' else 'DOWN'
    
    # 2. Candle Colors (Last 10 Closed)
    last_10 = candles[-10:]
    colors = []
    for c in last_10:
        colors.append('GREEN' if c['close'] > c['open'] else 'RED')
    
    # 3. Count Trend Candles (Closed)
    i = len(colors) - 1
    consecutive_same = 0
    same_color = None
    ...
    if consecutive_same < 2:
        return None, 0, {'reason': f'Need at least 2 trend, found {consecutive_same}'}
    
    details['trend_candles'] = consecutive_same
    details['trend_color'] = same_color
    
    # 4. Count Pullback Candles (Closed)
    pullback_color = 'GREEN' if same_color == 'RED' else 'RED'
    pullback_count = 0
    j = i + 1
    
    while j < len(colors) and pullback_count < 4:
        if colors[j] == pullback_color:
            pullback_count += 1
            j += 1
        else:
            break
    
    # 5. Check Running Candle for Pullback (using ticks)
    running_pullback = False
    if ticks and isinstance(ticks, list) and isinstance(ticks[-1], dict):
        current_open = candles[-1]['close']
        latest_price = ticks[-1].get('price') or ticks[-1].get('last', 0)
        
        if current_trend == 'up' and latest_price < current_open:
            running_pullback = True
        elif current_trend == 'down' and latest_price > current_open:
            running_pullback = True
    
    if running_pullback:
        pullback_count += 1
        details['running_pullback_included'] = True
    
    # Accept 1-3 pullback (including running)
    if pullback_count < 1 or pullback_count > 3:
        return None, 0, {'reason': f'Need 1-3 pullback, found {pullback_count}'}
    
    details['pullback_count'] = pullback_count
    details['pullback_color'] = pullback_color
    
    # 6. Tick Confirmation (running candle direction)
    safe_ticks = ticks if isinstance(ticks, list) else []
    check_ticks = safe_ticks[-10:] if len(safe_ticks) > 10 else safe_ticks
    tick_direction_count = 0
    current_open = candles[-1]['close']
    
    for t in check_ticks:
        if not isinstance(t, dict):
            continue
        p = t.get('price') or t.get('last', 0)
        if current_trend == 'up' and p > current_open:
            tick_direction_count += 1
        elif current_trend == 'down' and p < current_open:
            tick_direction_count += 1
    
    tick_ratio = tick_direction_count / len(check_ticks)
    if tick_ratio < 0.50:
        return None, 0, {'reason': 'Tick confirmation weak'}
    
    # 7. Direction
    if current_trend == 'up' and same_color == 'GREEN' and pullback_color == 'RED':
        direction = 'CALL'
        details['setup'] = 'BULLISH_PULLBACK'
    elif current_trend == 'down' and same_color == 'RED' and pullback_color == 'GREEN':
        direction = 'PUT'
        details['setup'] = 'BEARISH_PULLBACK'
    else:
        return None, 0, {'reason': 'Direction mismatch'}
    
    # 8. Score
    score = 55 + (consecutive_same * 2) + (pullback_count * 3)
    score = min(95, score)
    details['score'] = score
    details['reason'] = f'Pattern: {consecutive_same} trend + {pullback_count} pullback (incl running) + tick confirm'
    
    return direction, score, details

def fetch_live_ticks(pair, limit=50, broker="quotex"):
    """Fetch live ticks from the broker-matching beaxtapi tick endpoint.
    Each broker has its own tick host — never cross-call one broker's pair
    against another broker's endpoint."""
    api_pair = to_new_api_pair(pair)
    if broker == "pocket":
        url = f"{POCKET_CANDLE_API_BASE}ticks?pair={api_pair}&limit={limit}"
    elif broker == "tradowix":
        url = f"{TRADOWIX_CANDLE_API_BASE}ticks?pair={api_pair}&limit={limit}"
    else:
        url = f"{NEW_TICK_API_BASE}?pair={api_pair}&limit={limit}"
    ...
    try:
        ...
        if data.get('success') and data.get('data'):
            raw = data['data']
            # API sometimes returns a single tick dict instead of a list
            if isinstance(raw, dict):
                raw = [raw]
            if not isinstance(raw, list):
                return []
            # keep only well-formed dict ticks
...
def run_strategy_finder(uid: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    broker = context.user_data.get('finder_broker', 'quotex')

    # ─── Select first 10 pairs for speed (broker-isolated pool) ─────
    if market == 'otc':
        pairs = multi_engine_pairs(broker, 'otc')[:10]
    elif market == 'live':
        pairs = multi_engine_pairs(broker, 'live')[:10]
    else:
        pairs = (multi_engine_pairs(broker, 'otc') + multi_engine_pairs(broker, 'live'))[:10]
    ...
    def fetch_pair(pair):
        try:
            candles, _, _ = fetch_candles_mrbeaxt(pair, count=count, timeframe=tf, broker=broker)
            return pair, candles
        except:
            return pair, None
    ...
    lines.append(f"🏦 Broker: {fut4_broker_label(broker)}  |  📅 Market: {market_label}  |  TF: {tf_display}  |  Pairs: {len(pair_candles)}")
    lines.append("")
...
async def s6_candles_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if 10 <= val <= 200:
            ...
            bot = SMZXBot(uid, broker=context.user_data.get('live_broker', 'quotex'))
            if trading_mode == 'forex':
                bot.pairs = POCKET_LIVE_PAIRS if bot.broker == 'pocket' else FOREX_PAIRS
...
STATE_BLACKOUT_CHECKER_PAYOUT = 113
...
STATE_BROADCAST_CONFIRM = 901            # Waiting for Yes/No on the preview

STATE_MAINTENANCE_MSG = 950              # Owner typing the maintenance message
STATE_MAINTENANCE_ETA = 951              # Owner typing the estimated time

# Multi Engine States
STATE_MULTI_ENGINE_BROKER = 999
...
STATE_SMZX_NAME_TEMPLATE = 1210

STATE_AUTO_SIGNAL_TIMEFRAME = 1009

STATE_FINDER_BROKER = 1199
STATE_FINDER_MARKET = 1200
STATE_FINDER_TF = 1201
STATE_FINDER_RUNNING = 1203   # not used directly but for state tracking

STATE_LIVEFS_CHECKER_SETTINGS = 306   # Main settings screen (UTC + CHANGE button)
STATE_LIVEFS_PASTE = 307              # Signals paste (after UTC set)
STATE_LIVEFS_CHECKER_MTG = 308        # MTG selection (after preview)
STATE_LIVEFS_CHECKER_DATE = 309       # Date selection (after MTG)
STATE_LIVEFS_CHECKER_PAYOUT = 310     # Payout filter (after date)

STATE_TREND_FILTER_SETTINGS = 410   # Settings screen
STATE_TREND_FILTER_PASTE = 411      # Signals paste (after settings)
STATE_TREND_FILTER_EMA = 412        # EMA period selection
STATE_TREND_FILTER_RUNNING = 413    # Processing (loading) state

STATE_MULTI_NONSTOP_STRATEGY = 1010   # Strategy selection for Multi Engine Non-Stop
...
async def reset_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    try:
        force_reset_user_session(uid, context)
        await update.message.reply_text(
            "✅ Your session has been force reset.\n"
        ...
        await update.message.reply_text(
            "❌ Reset encountered an error, but most flags have been cleared.\n"
...
def _run_sio_backtest_thread(uid: int, context, signals_text: str):
    ...
    if user_offset is None:
        # Fall back to the unified Settings > UTC Selection value if the
        # user never touched the old per-feature "Change UTC" button.
        user_offset = get_tz_minutes(uid, "backtest") / 60
...
async def bt2_utc_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = query.from_user.id
    # Keep the unified Settings > UTC Selection value in sync.
    get_state(uid).tz_settings['backtest'] = offset * 60
...
async def _ask_livefs_start_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tz_label = format_utc_offset(get_tz_minutes(uid, "livefs"))
    msg = f"🕐 𝙴𝚗𝚝𝚎𝚛 𝚜𝚝𝚊𝚛𝚝 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {tz_label}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 09:00"
...
async def _ask_livefs_end_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tz_label = format_utc_offset(get_tz_minutes(uid, "livefs"))
    msg = f"⏰ 𝙴𝚗𝚝𝚎𝚛 𝚎𝚗𝚍 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {tz_label}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 16:30"
...
def _run_livefs_worker(uid: int, context):
    ...
    broker = context.user_data.get('livefs_broker', 'quotex')
    ...
    def update_progress(current, total, pair, status="🔍"):
        ...
        text = (
            ...
            f"{status} {pair}\n"
            f"⏰ {datetime.now(timezone(get_tz_delta(uid, 'livefs'))).strftime('%H:%M:%S')}"
        )
        try:
            sender.edit_message(uid, msg_id, text)
        except Exception:
    ...
    for idx, pair in enumerate(pairs, 1):
        ...
        # Live FS pairs (LIVE_PAIRS_FS) are plain, no-suffix pairs like
        # "EURUSD" — that's exactly the qxcandledata API's Live-market
        # format, so `pair` slots in directly (no OTC suffix logic needed).
        if broker == "pocket":
            urls_to_try = [f"{POCKET_CANDLE_API_BASE}?pair={pair}&timeframe=M1&count=14000"] * 3
        elif broker == "tradowix":
            urls_to_try = [f"{TRADOWIX_CANDLE_API_BASE}?pair={pair}&timeframe=M1&count=14000"] * 3
        else:
            # Quotex: qxcandledata.beaxtapi.online first, then automatically shift
            # to the mrbeaxt.com backup if it keeps failing.
            urls_to_try = [f"{NEW_CANDLE_API_BASE}?pair={pair}&timeframe=M1&count=14000"] * 3
            urls_to_try += [f"{NEW_CANDLE_API_FALLBACK_BASE}?pair={pair}&timeframe=M1&count=14000"] * 3
        success = False
        candles = []
        ...
        for attempt, url in enumerate(urls_to_try, 1):
            try:
                ...
                if resp.status_code == 200:
                    ...
                    update_progress(idx, total_pairs, pair, f"⚠️ 502 retry {attempt}/{len(urls_to_try)}")
                    t.sleep(1.5)
                    continue
                else:
                ...
                update_progress(idx, total_pairs, pair, f"⚠️ Error retry {attempt}/{len(urls_to_try)}")
                t.sleep(1.5)
                continue
        ...
        try:
            ...
            if not data.get('success') or not data.get('data'):
                ...
                t.sleep(0.3)
                continue
            # Re-key to the single-letter o/h/l/c format the rest of this
            # function (stochastic/bollinger calcs) expects, and keep
            # 'time' as the raw UTC epoch (int) rather than the API's own
            # UTC+6 time string — parse_candle_time() below treats an
            # int/float 'time' as a true UTC epoch and correctly adds the
            # +5h Pakistan offset itself; feeding it the API's UTC+6
            # string instead would double-offset everything.
            candles = [
                {
                    'time': c.get('epoch'),
                    'o': c.get('open'),
                ...
                for c in data['data'] if c.get('epoch') is not None
            ]
            if not candles:
                update_progress(idx, total_pairs, pair, "⚠️ No candles")
                t.sleep(0.3)
            ...
            for i in range(50, len(candles)-1):
                ...
                if dt_utc is None:
                    continue
                dt_pk = dt_utc + get_tz_delta(uid, "livefs")
                time_str = dt_pk.strftime("%H:%M")
    ...
    now_pk = datetime.now(timezone(get_tz_delta(uid, "livefs")))
    day = now_pk.day
    ...
    header = (
        f"🐶{date_formatted} 🐶\n"
        f"🐱UTC/ GMT: ( {tz_label.replace('UTC ', '')} )\n"
        f"🐭1STEP MARTINGAL\n"
        f"\n"
...
def run_livefs_checker(uid, date_str, mtg_level, signals_text, context, payout_filter=False, broker="quotex"):
    """
    Live FS Checker – exactly same structure as run_advanced_checker.
    """
    from datetime import datetime, timedelta, timezone
    import re
    from concurrent.futures import ThreadPoolExecutor, as_completed

    # ---------- Parser (same as advance checker) ----------
    def parse_signal_line(line):
        line = normalize_fancy(line)
        line = re.sub(r'^M\d+\s*', '', line)
        pair_match = re.search(r'([A-Z0-9]+[_-]?[A-Z0-9]*(?:[_-]OTC)?)', line)
    ...
    # ---------- Store raw data for recheck ----------
    context.user_data['lfc_raw_signals'] = signals
    context.user_data['lfc_date'] = date_str
    context.user_data['lfc_mtg'] = mtg_level
    ...
    for sig in signals:
        ...
        sig_dt = sig_dt.replace(tzinfo=tz_pk)
        sig['future'] = sig_dt > now_pk

    try:
        date_utc5 = datetime.strptime(date_str, "%Y-%m-%d")
    except Exception:
        sender.send_message(uid, f"❌ Invalid date: {date_str}\nUse YYYY-MM-DD")
        return
    ...
    print(f"\n🔍 USER DATE: {date_str} (UTC+5)")

    days_back = max(1, (now_pk.date() - date_utc5.date()).days)
    fetch_count = min(40000, max(3000, (days_back + 2) * 1440))
    print(f"🗓 LIVE FS CHECKER: date is {days_back} day(s) back -> requesting {fetch_count} candles")

    unique_pairs = list({sig['pair'] for sig in signals if not sig['future']})

    def fetch_pair(pair):
        result = fetch_beaxt_candles(pair, count=fetch_count, retries=3, timeout=25, broker=broker)
        if not result:
            return pair, None
        candles = result.get('candles', [])
        candle_by_time = {}
        for c in candles:
            ...
            if epoch is None:
                continue
            dt_pk = datetime.fromtimestamp(epoch, tz=timezone.utc) + timedelta(hours=5)
            date_key = dt_pk.strftime("%Y-%m-%d")
            if date_key != date_str:
                continue
            time_key = dt_pk.strftime("%H:%M")
            c['direction'] = 'up' if c['close'] > c['open'] else 'down'
            candle_by_time[time_key] = c
        print(f"🔍 {pair} – {len(candle_by_time)} candles for {date_str}")
        return pair, candle_by_time

    candle_cache = {}
    if unique_pairs:
        update_progress(0, len(unique_pairs), "📡 Fetching candles...")
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(fetch_pair, p): p for p in unique_pairs}
            done = 0
            for future in as_completed(futures):
                pair, candle_by_time = future.result()
                candle_cache[pair] = candle_by_time
                done += 1
                update_progress(done, len(unique_pairs), f"📡 Fetched {pair}")

    # ---------- Process signals ----------
    final_display = []
    wins = losses = pending = skipped = 0
    loss_data_list = []   # for loss candles
    ...
    for sig in signals:
        ...
        if sig['future']:
            final_display.append((f"M1 {pair} {time_str} {direction}", "⏳"))
            pending += 1
            continue

        candle_dict = candle_cache.get(pair)
        if not candle_dict:
            final_display.append((f"M1 {pair} {time_str} {direction}", "❌"))
            losses += 1
            continue

        entry = candle_dict.get(time_str)
        if not entry:
            final_display.append((f"M1 {pair} {time_str} {direction}", "❌"))
            losses += 1
            continue

        # 🔥 PAYOUT FILTER CHECK (same as advance checker)
        payout = entry.get('payout', 92)
        if payout_filter and payout < 80:
            final_display.append((f"M1 {pair} {time_str} {direction} {payout}%", ""))
            skipped += 1
            continue

        candle_dir = entry.get('direction', '')
        win = (direction == "CALL" and candle_dir == "up") or (direction == "PUT" and candle_dir == "down")
        if win:
            final_display.append((f"M1 {pair} {time_str} {direction}", "✅"))
            wins += 1
            continue

        # MTG recovery (same logic)
        recovered = False
        if mtg_level > 0:
            for step in range(1, mtg_level + 1):
                h, m = map(int, time_str.split(':'))
                next_dt = datetime(2000, 1, 1, h, m) + timedelta(minutes=step)
                next_key = next_dt.strftime("%H:%M")
                next_candle = candle_dict.get(next_key)
                if next_candle:
                    next_dir = next_candle.get('direction', '')
                    if (direction == "CALL" and next_dir == "up") or (direction == "PUT" and next_dir == "down"):
                        mtg_icon = f"✅{step}" if step > 1 else "✅¹"
                        final_display.append((f"M1 {pair} {time_str} {direction}", mtg_icon))
                        wins += 1
                        recovered = True
                        break
        if not recovered:
            final_display.append((f"M1 {pair} {time_str} {direction}", "❌"))
            losses += 1
            # 🔥 Collect loss candles data (same as advance checker)
            day_lookup = candle_cache.get(pair, {})
            loss_data = collect_loss_candles(day_lookup, time_str, direction, pair, user_offset=5)
            if loss_data:
                loss_data_list.append(loss_data)

    # Store loss data for "Show Loss Candles" button
    context.user_data['lfc_loss_signals'] = loss_data_list

    total = wins + losses + pending + skipped

    # ---------- Build final output (same style as advance checker) ----------
    header = (
    ...
    body_lines = [f"{sig} {icon}".strip() for sig, icon in final_display]
    body = "\n".join(body_lines)

    extra_lines = ""
    if pending:
        extra_lines += f"\n{fancy_font('⏳ PENDING : ')}{fancy_font(str(pending))}"
    if skipped:
        extra_lines += f"\n{fancy_font('💲 SKIPPED : ')}{fancy_font(str(skipped))}"
    ...
    # Build bold entities (header + summary)
    entities = build_custom_emoji_entities(full_text)
    header_utf16_len = len(header.encode('utf-16-le')) // 2
    summary_start_utf16 = len((header + "\n" + body).encode('utf-16-le')) // 2
    ...
    # ---------- Build buttons (same as advance checker) ----------
    buttons = []
    if loss_data_list:
        buttons.append([colored_button(" Show Loss Candles", "lfc_show_loss", KeyboardButtonStyle.PRIMARY, "5809816842713174497")])
    if not payout_filter:
        buttons.append([colored_button(" Check Again With Payout Filter", "lfc_recheck_payout", KeyboardButtonStyle.PRIMARY, "6104726047628990417")])
...
def build_utc_keyboard_with_pagination(prefix, page=0, per_page=12, back_callback="checker_back_to_settings"):
    ...
    if nav_row:
        buttons.append(nav_row)
    buttons.append([colored_button("🔙 Back", back_callback, KeyboardButtonStyle.DANGER, "6145317070578916456")])
    return buttons, page, total_pages

async def show_utc_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, prefix="checker_utc_", back_callback="checker_back_to_settings"):
    query = update.callback_query
    if query:
        await safe_query_answer(query)
    page = context.user_data.get('utc_page', 0)
    buttons, page, total_pages = build_utc_keyboard_with_pagination(prefix, page, back_callback=back_callback)
...
def _news_draft(context) -> dict:
    d = context.user_data.get("news_draft")
    if not isinstance(d, dict) or not d:
        d = news_mod.default_news_draft() if news_mod else {
            "pairs": [], "n_days": 1, "filter": "all", "page": 0, "ai_mode": True,
        }
        context.user_data["news_draft"] = d
    d.setdefault("pairs", list(getattr(news_mod, "NEWS_GLOBAL_PAIRS", ())) if news_mod else [])
    d.setdefault("n_days", 1)
    d.setdefault("filter", "all")
    d.setdefault("page", 0)
    d.setdefault("ai_mode", True)
    return d


def _news_home_markup(draft) -> InlineKeyboardMarkup:
    n = len(draft.get("pairs") or [])
    days = int(draft.get("n_days") or 1)
    filt = str(draft.get("filter") or "all").upper()
    ai_on = bool(draft.get("ai_mode", True))
    return InlineKeyboardMarkup([
        [colored_button(f" PAIRS ({n}) ", "news_pairs", KeyboardButtonStyle.PRIMARY, "6330188813939251966")],
        [
            colored_button(f" DAYS : {days} ", "news_days", KeyboardButtonStyle.PRIMARY, "6102906733842144545"),
            colored_button(f" FILTER : {filt} ", "news_filter", KeyboardButtonStyle.PRIMARY, "6231237393119191254"),
        ],
        [colored_button(
            " AI ON " if ai_on else " AI OFF ",
            "news_ai_toggle",
            KeyboardButtonStyle.SUCCESS if ai_on else KeyboardButtonStyle.DANGER,
            "6271527128408264959",
        )],
        [colored_button(" GENERATE NEWS SIGNALS ", "news_generate", KeyboardButtonStyle.SUCCESS, "5438571934210082705")],
        [colored_button(" Back ", "back_to_main", KeyboardButtonStyle.DANGER, "5258084656674250503")],
    ])


def _news_home_text(draft) -> str:
    n = len(draft.get("pairs") or [])
    days = int(draft.get("n_days") or 1)
    filt = str(draft.get("filter") or "all").upper()
    ai_on = bool(draft.get("ai_mode", True))
    return (
        ...
        "━━━━━━━━━━━━━━━━━━\n\n"
        f"⚡ PAIRS : {n} selected\n"
        f"📅 DAYS : next {days} day(s)\n"
        f"🏆 IMPACT : {filt}\n"
        f"🧠 AI MODE : {'ON' if ai_on else 'OFF'}\n\n"
        "╰➤ Set options then tap GENERATE"
    )


def _news_pairs_markup(draft) -> InlineKeyboardMarkup:
    selected = set(draft.get("pairs") or [])
    pool = list(news_mod.NEWS_GLOBAL_PAIRS) if news_mod else []
    per = int(getattr(news_mod, "NEWS_PAIRS_PER_PAGE", 8) or 8)
    total_pages = max(1, (len(pool) + per - 1) // per)
    page = max(0, min(int(draft.get("page") or 0), total_pages - 1))
    draft["page"] = page
    ...
    for p in chunk:
        ...
        style = KeyboardButtonStyle.SUCCESS if p in selected else KeyboardButtonStyle.PRIMARY
        row.append(colored_button(f"{mark}{p}", f"news_ptog_{p}", style, None))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
    ...
    nav = []
    if page > 0:
        nav.append(colored_button("◀ Prev", f"news_ppage_{page - 1}", KeyboardButtonStyle.PRIMARY, None))
    if page < total_pages - 1:
        nav.append(colored_button("Next ▶", f"news_ppage_{page + 1}", KeyboardButtonStyle.PRIMARY, None))
    if nav:
        rows.append(nav)
    rows.append([
        colored_button(" Select All ", "news_sel_all", KeyboardButtonStyle.SUCCESS, "6273749318717412886"),
        colored_button(" Clear ", "news_sel_clear", KeyboardButtonStyle.DANGER, "6260072893011469020"),
    ])
    rows.append([colored_button(" Confirm ", "news_pairs_ok", KeyboardButtonStyle.SUCCESS, "6233489978911890876")])
    rows.append([colored_button(" Back ", "menu_news", KeyboardButtonStyle.DANGER, "5258084656674250503")])
    return InlineKeyboardMarkup(rows)


async def _news_edit(query, text, markup):
    entities = build_custom_emoji_entities(text)
    try:
        await query.edit_message_text(text, entities=entities, reply_markup=markup)
    except BadRequest:
        await query.message.reply_text(text, entities=entities, reply_markup=markup)


async def show_news_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if query:
        await safe_query_answer(query)
    if news_mod is None:
        msg = "❌ News module failed to load. Check news.py on the server."
        if query:
            await query.message.reply_text(msg)
        return
    draft = _news_draft(context)
    text = _news_home_text(draft)
    markup = _news_home_markup(draft)
    if query:
        await _news_edit(query, text, markup)
    else:
        await context.bot.send_message(
            chat_id=update.effective_user.id, text=text,
            entities=build_custom_emoji_entities(text), reply_markup=markup)


def _news_send_card(uid, parts, kb=None, use_prem=True):
    text, entities = news_mod.parts_to_text_and_entities(parts, use_prem=use_prem)
    payload = {"chat_id": uid, "text": text}
    if entities:
        payload["entities"] = entities[:90]
    if kb:
        payload["reply_markup"] = _markup_to_raw(kb)
    return _tg_api("sendMessage", payload)


def _run_news_generate(uid, draft):
    pairs = list(draft.get("pairs") or [])
    n_days = int(draft.get("n_days") or 1)
    filt = str(draft.get("filter") or "all")
    use_ai = bool(draft.get("ai_mode", True))
    loading = sender.send_message(
        uid,
        f"⏳ Generating news signals...\n{len(pairs)} pairs · {n_days}d · {filt.upper()} · AI={'ON' if use_ai else 'OFF'}",
    )
    result = news_mod.build_news_events(
        pairs=pairs,
        n_days=n_days,
        newsfilter=filt,
        tz_hours=5.0,
        use_ai=use_ai and bool(GEMINI_API_KEYS),
        gemini_keys=GEMINI_API_KEYS if use_ai else None,
    )
    try:
        mid = getattr(loading, "id", None)
        if mid:
            sender.delete_message(uid, mid)
    except Exception:
        pass

    back_kb = InlineKeyboardMarkup([
        [colored_button(" Generate Again ", "news_generate", KeyboardButtonStyle.SUCCESS, "5438571934210082705")],
        [colored_button(" News Menu ", "menu_news", KeyboardButtonStyle.PRIMARY, "5971837723676249096")],
        [colored_button(" Back Home ", "back_to_main", KeyboardButtonStyle.DANGER, "5258084656674250503")],
    ])

    if not isinstance(result, dict) or result.get("status") != "success":
        msg = (result or {}).get("message") if isinstance(result, dict) else "error"
        sender.send_message(uid, f"📰 NEWS SIGNAL\n\n❌ Failed.\n{msg}", buttons=back_kb)
        return
    ...
    if not events:
        sender.send_message(
            uid,
            "📰 NEWS SIGNAL\n\nℹ️ No tradable events for these filters.\nTry more pairs, more days, or Impact = ALL.",
            buttons=back_kb,
        )
        return

    tz = result.get("tz_label") or "UTC+5"
    ai_used = bool(result.get("ai_used"))
    total = len(events)
    engine = "Gemini AI" if ai_used else "Rule Engine"
    ready = [
        ("▰▱ NEWS TICKET ▱▰\n", None),
        ("📰", "news"), (" READY\n\n", None),
        ("🔥", "fire"), (f" {total} events · {engine}\n", None),
        ("🏆", "crown"), (f" {filt.upper()} · {n_days} day(s)\n", None),
        ("🕐", "clock"), (f" {tz}", None),
    ]
    _news_send_card(uid, ready, None, True)

    bot_uname = os.environ.get("BOT_USERNAME") or "RohailBot"
    for idx, ev in enumerate(events):
        parts = news_mod.build_news_signal_parts(
            ev,
            events_today=total,
            tz_label=tz,
            owner="@Rohailtrader",
        ...
        kb = back_kb if idx == total - 1 else None
        ok = _news_send_card(uid, parts, kb, True)
        if not ok:
            sender.send_message(uid, "".join(s for s, _ in parts), buttons=kb)
        if idx < total - 1:
            time.sleep(0.35)
...
async def news_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_authorized_async(uid):
        ...
        return
    if news_mod is None:
        await query.message.reply_text("❌ news.py not loaded.")
        return

    data = query.data or ""
    draft = _news_draft(context)

    if data in ("menu_news", "news_home"):
        await _news_edit(query, _news_home_text(draft), _news_home_markup(draft))
        return

    if data == "news_pairs":
        n = len(draft.get("pairs") or [])
        pool = len(getattr(news_mod, "NEWS_GLOBAL_PAIRS", ()) or ())
        await _news_edit(
            query,
            f"📰 NEWS PAIRS\n\nTap to toggle · selected {n}/{pool}",
            _news_pairs_markup(draft),
        )
        return

    if data.startswith("news_ptog_"):
        p = data.replace("news_ptog_", "")
        selected = list(draft.get("pairs") or [])
        if p in selected:
            selected = [x for x in selected if x != p]
        else:
            selected.append(p)
        order = {x: i for i, x in enumerate(news_mod.NEWS_GLOBAL_PAIRS)}
        draft["pairs"] = sorted(selected, key=lambda x: order.get(x, 999))
        n = len(draft["pairs"])
        pool = len(news_mod.NEWS_GLOBAL_PAIRS)
        await _news_edit(
            query,
            f"📰 NEWS PAIRS\n\nTap to toggle · selected {n}/{pool}",
            _news_pairs_markup(draft),
    ...
    if data.startswith("news_ppage_"):
        try:
            draft["page"] = int(data.replace("news_ppage_", ""))
        except ValueError:
            draft["page"] = 0
        n = len(draft.get("pairs") or [])
        pool = len(news_mod.NEWS_GLOBAL_PAIRS)
    ...
    if data == "news_sel_all":
        draft["pairs"] = list(news_mod.NEWS_GLOBAL_PAIRS)
        n = len(draft["pairs"])
        await _news_edit(
            query,
            f"📰 NEWS PAIRS\n\nTap to toggle · selected {n}/{n}",
            _news_pairs_markup(draft),
        )
        return
    ...
    if data == "news_days":
        rows = [[colored_button(
            ...
            f"news_days_set_{d}",
            KeyboardButtonStyle.SUCCESS if int(draft.get("n_days") or 1) == d else KeyboardButtonStyle.PRIMARY,
            "6102906733842144545",
        )] for d in getattr(news_mod, "NEWS_DAYS", (1, 2, 3))]
        rows.append([colored_button(" Back ", "menu_news", KeyboardButtonStyle.DANGER, "5258084656674250503")])
        await _news_edit(query, "📅 How many days ahead?", InlineKeyboardMarkup(rows))
        return

    if data.startswith("news_days_set_"):
        try:
            draft["n_days"] = int(data.replace("news_days_set_", ""))
        except ValueError:
            draft["n_days"] = 1
        await _news_edit(query, _news_home_text(draft), _news_home_markup(draft))
        return
    ...
    if data == "news_filter":
        rows = []
        cur = str(draft.get("filter") or "all").lower()
        for f in getattr(news_mod, "NEWS_FILTERS", ("all", "high", "medium", "low")):
            rows.append([colored_button(
                f" {f.upper()} ",
                f"news_filter_set_{f}",
                KeyboardButtonStyle.SUCCESS if cur == f else KeyboardButtonStyle.PRIMARY,
                "6231237393119191254",
            )])
        rows.append([colored_button(" Back ", "menu_news", KeyboardButtonStyle.DANGER, "5258084656674250503")])
        await _news_edit(query, "🏆 Impact filter", InlineKeyboardMarkup(rows))
        return

    if data.startswith("news_filter_set_"):
        f = data.replace("news_filter_set_", "").lower()
        if f in getattr(news_mod, "NEWS_FILTERS", ("all", "high", "medium", "low")):
            draft["filter"] = f
        await _news_edit(query, _news_home_text(draft), _news_home_markup(draft))
        return
    ...
    if data == "news_generate":
        if not draft.get("pairs"):
            await safe_query_answer(query, "Select at least one pair first!", show_alert=True)
            return
        await safe_query_answer(query, "Generating…")
        snap = dict(draft)
        snap["pairs"] = list(draft.get("pairs") or [])
        threading.Thread(target=_run_news_generate, args=(uid, snap), daemon=True).start()
        return
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # log_user_to_all_users() does a blocking requests.post() (up to 5s) —
    # was running directly on MAIN_LOOP on every single /start, which could
    # freeze the whole bot for everyone during that window. We don't need
    # its result before replying, so dispatch it on a background thread
    # instead of blocking /start on it.
    threading.Thread(target=log_user_to_all_users, args=(update,), daemon=True).start()
    ...
    buttons = [
        # Row 1: OTC LIVE MODE + Settings (top-right)
        [colored_button("     OTC LIVE MODE     ", "menu_analysis", KeyboardButtonStyle.SUCCESS, "6145248943807667330"),
         colored_button("       Settings       ", "menu_settings", KeyboardButtonStyle.SUCCESS, "5451827186833051828")],
        ...
        # Row 13: News + SWAP C/P
        [colored_button("        News        ", "menu_news", KeyboardButtonStyle.SUCCESS, "5971837723676249096"),
    ...
    # ─── Row 16: Forex Live Mode ────────────────────────
    buttons.append([
        colored_button("     FOREX LIVE MODE   ", "menu_forex_live", KeyboardButtonStyle.SUCCESS, "6102524232644698570"),
    ])

    # ─── Broadcast + Maintenance buttons (owner only) ───
    if uid == OWNER_ID:
        buttons.append([
            colored_button(" Broadcast", "admin_broadcast", KeyboardButtonStyle.DANGER, "6129805886383723340")
        ])
        maint_label = "🛠 Maintenance: ON (tap to turn OFF)" if MAINTENANCE_MODE["on"] else "🛠 Maintenance: OFF (tap to turn ON)"
        buttons.append([
            colored_button(maint_label, "admin_maintenance_toggle", KeyboardButtonStyle.DANGER, "6129805886383723340")
        ])

    # ─── Leaderboard button (authorized users only) ───
    if await is_authorized_async(uid):
        buttons.append([
            colored_button(" Leaderboard", "menu_leaderboard", KeyboardButtonStyle.SUCCESS, LEADERBOARD_BUTTON_EMOJI_ID)
        ])
        buttons.append([
            colored_button("Best Trading Hours", "hours_start", KeyboardButtonStyle.SUCCESS, "5217880283860194582")
        ])
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data != "menu_admin" and not await is_authorized_async(uid):
        await send_restriction_message(update, context)
        return
    ...
    if data == "menu_admin":
        msg = (
            ...
            "🧠 Owner  : @Rohailtrader\n"
            "😔 Broker : QUOTEX, TRADOWIX , POCKET OPTION\n"
            "🤮 Timeframe : M1\n\n"
    ...
    if not await is_authorized_async(uid):
        await safe_query_answer(query, "⛔ Access denied. Contact Admin to get access.", show_alert=True)
        return

    # ==================== REST OF YOUR MENU (including Auto Trade) ====================
    if data == "menu_settings":
        await show_smzx_settings(update, context, edit=False)
        return

    if data == "smzx_settings_back":
        await show_smzx_settings(update, context, edit=True)
        return

    if data == "smzx_settings_timezone":
        await show_timezone_settings(update, context, edit=True)
        return

    if data == "smzx_settings_name_template":
        context.user_data['state'] = STATE_SMZX_NAME_TEMPLATE
        current = get_name_template(uid)
        msg = (
            "🎯 Name Template\n\n"
            f"Current: {current}\n\n"
            "Send the @username you want shown as the owner/developer on "
            "your OTC Live, AI OTC, Auto Signal, Multi Engine and Forex "
            "Live signal & result messages.\n\n"
    ...
    if data == "auto_trade_start":
        ...
        broker_buttons = [
            [colored_button(" Quotex ", "aiotcbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
             colored_button(" Pocket Option ", "aiotcbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
            [colored_button(" Tradowix ", "aiotcbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
        ]
        entities = build_custom_emoji_entities(broker_text)
        await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
        return

    elif data in ("aiotcbroker_quotex", "aiotcbroker_pocket", "aiotcbroker_tradowix"):
        uid = query.from_user.id
        context.user_data['ai_broker'] = data.replace("aiotcbroker_", "")
        st = get_state(uid)
        st.ai_broker = context.user_data['ai_broker']
        st.ai_mode = True
       ...
       broker_buttons = [
           [colored_button(" Quotex ", "otcbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
            colored_button(" Pocket Option ", "otcbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
           [colored_button(" Tradowix ", "otcbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
       ]
       entities = build_custom_emoji_entities(broker_text)
       await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
       return

    elif data in ("otcbroker_quotex", "otcbroker_pocket", "otcbroker_tradowix"):
       context.user_data['live_broker'] = data.replace("otcbroker_", "")
       ...
       await query.message.reply_text(
         ...
         "📸 To include a picture, send a photo with your text as the caption.\n"
         "⚠️ Photo captions are limited to 1024 characters (plain text messages can be longer).\n\n"
         "Type /cancel to abort."
       )
       await safe_query_answer(query)

    elif data == "broadcast_send_yes":
       if uid != OWNER_ID:
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       if context.user_data.get('state') != STATE_BROADCAST_CONFIRM:
          await safe_query_answer(query, "⌛ This preview has expired.", show_alert=True)
          return
       await safe_query_answer(query, "📡 Sending...")
       await query.message.edit_reply_markup(reply_markup=None)
       await _execute_broadcast(update, context)
       return

    elif data == "hours_start":
       if not await is_authorized_async(uid):
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       await safe_query_answer(query)
       msg = (
           "▰▰▰ SMZ X ▰▰▰\n\n"
           "⏰ 𝙱𝙴𝚂𝚃 𝚃𝚁𝙰𝙳𝙸𝙽𝙶 𝙷𝙾𝚄𝚁𝚂\n"
           "╰➤ Select broker · hours in UTC+5\n\n"
           "𝚃𝙸𝙼𝙴𝚉𝙾𝙽𝙴\n"
           "╰➤ UTC+5\n\n"
           "𝚂𝚃𝙴𝙿 𝟷\n"
           "╰➤ Choose your broker"
       )
       hdr_offset = len("▰▰▰ SMZ X ▰▰▰\n\n".encode('utf-16-le')) // 2
       entities = [MessageEntity(type='custom_emoji', offset=hdr_offset, length=1,
                                  custom_emoji_id=HOURS_HEADER_EMOJI_IDS["⏰"])]
       buttons = [
           [colored_button("Quotex", "hours_broker_quotex", KeyboardButtonStyle.SUCCESS, "6217713374327738118")],
           [colored_button("Pocket Option", "hours_broker_pocket", KeyboardButtonStyle.SUCCESS, "5258033997535012714")],
           [colored_button("Tradowix", "hours_broker_tradowix", KeyboardButtonStyle.SUCCESS, "6337101525147458464")],
       ]
       await query.message.reply_text(msg, entities=entities, reply_markup=InlineKeyboardMarkup(buttons))
       return

    elif data.startswith("hours_broker_"):
       if not await is_authorized_async(uid):
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       broker = data[len("hours_broker_"):]
       context.user_data['hours_broker'] = broker
       await safe_query_answer(query)
       msg = (
           "𝚂𝚃𝙴𝙿 𝟸\n"
           "╰➤ Kitne din ka data check karna hai?"
       )
       row1 = [colored_button(str(d), f"hours_days_{d}", KeyboardButtonStyle.SUCCESS, "6217575613251719126") for d in range(1, 5)]
       row2 = [colored_button(str(d), f"hours_days_{d}", KeyboardButtonStyle.SUCCESS, "6217575613251719126") for d in range(5, 8)]
       await query.message.reply_text(msg, reply_markup=InlineKeyboardMarkup([row1, row2]))
       return

    elif data.startswith("hours_days_"):
       if not await is_authorized_async(uid):
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       days = int(data[len("hours_days_"):])
       context.user_data['hours_days'] = days
       await safe_query_answer(query)
       msg = (
           "𝚂𝚃𝙴𝙿 𝟹\n"
           "╰➤ Kis ka data dekhna hai?"
       )
       buttons = [
           [colored_button("Your Trades", "hours_scope_self", KeyboardButtonStyle.SUCCESS, "5258362837411045098")],
           [colored_button("All Users Trades", "hours_scope_all", KeyboardButtonStyle.SUCCESS, "5393579201141946670")]
       ]
       await query.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(buttons))
       return

    elif data in ("hours_scope_self", "hours_scope_all", "hours_refresh"):
       if not await is_authorized_async(uid):
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       if data == "hours_scope_self":
          context.user_data['hours_scope'] = 'self'
       elif data == "hours_scope_all":
          context.user_data['hours_scope'] = 'all'
       broker = context.user_data.get('hours_broker')
       days = context.user_data.get('hours_days')
       scope = context.user_data.get('hours_scope')
       if not broker or not days or not scope:
          await safe_query_answer(query, "⌛ Session expired, /start se dobara try karein.", show_alert=True)
          return
       await safe_query_answer(query, "🔄 Loading..." if data == "hours_refresh" else None)
       telegram_id = uid if scope == 'self' else None
       text = await MAIN_LOOP.run_in_executor(_AUTH_EXECUTOR, build_best_hours_message, broker, days, telegram_id)
       entities = build_best_hours_entities(text, broker)
       refresh_markup = InlineKeyboardMarkup([
           [colored_button("Refresh", "hours_refresh", KeyboardButtonStyle.SUCCESS, LEADERBOARD_REFRESH_EMOJI_ID)],
           [colored_button("Back to Menu", "back_to_main", KeyboardButtonStyle.SUCCESS, "5269720500468201056")],
       ])
       if data == "hours_refresh":
          try:
             await query.edit_message_text(text, entities=entities, reply_markup=refresh_markup)
          except Exception:
             await query.message.reply_text(text, entities=entities, reply_markup=refresh_markup)
       else:
          await query.message.reply_text(text, entities=entities, reply_markup=refresh_markup)
       return

    elif data in ("menu_leaderboard", "leaderboard_refresh"):
       if not await is_authorized_async(uid):
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       await safe_query_answer(query, "🔄 Loading..." if data == "leaderboard_refresh" else None)
       text = await MAIN_LOOP.run_in_executor(_AUTH_EXECUTOR, build_leaderboard_message)
       entities = build_leaderboard_entities(text)
       refresh_markup = InlineKeyboardMarkup([
           [colored_button(" Refresh", "leaderboard_refresh", KeyboardButtonStyle.SUCCESS, LEADERBOARD_REFRESH_EMOJI_ID)],
           [colored_button(" Back to Menu", "back_to_main", KeyboardButtonStyle.SUCCESS, "5269720500468201056")],
       ])
       ...
       if MAINTENANCE_MODE["on"]:
          ...
          MAINTENANCE_MODE["eta"] = ""
          _MAINTENANCE_LAST_NOTICE.clear()
          await safe_query_answer(query, "✅ Maintenance mode OFF.")
          await start_cmd(update, context)
          return
       else:
          context.user_data['state'] = STATE_MAINTENANCE_MSG
          await safe_query_answer(query)
          await query.message.reply_text(
              "🛠 Turning maintenance mode ON.\n\n"
              "Send the message authorized users should see when they tap any button.\n\n"
              "Type /cancel to abort."
          )
          return

    elif data == "broadcast_send_no":
       if uid != OWNER_ID:
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       context.user_data['broadcast_pending'] = None
       context.user_data.pop('state', None)
       await safe_query_answer(query, "❌ Broadcast cancelled.")
       await query.message.edit_reply_markup(reply_markup=None)
       await start_cmd(update, context)
       return
        ...
        broker_buttons = [
            [colored_button(" Quotex ", "livefsmain_broker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
             colored_button(" Pocket Option ", "livefsmain_broker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
            [colored_button(" Tradowix ", "livefsmain_broker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
        ]
        entities = build_custom_emoji_entities(broker_text)
        await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
        return

    elif data in ("livefsmain_broker_quotex", "livefsmain_broker_pocket", "livefsmain_broker_tradowix"):
        uid = query.from_user.id
        context.user_data['livefs_broker'] = data.replace("livefsmain_broker_", "")
       ...
       broker_buttons = [
           [colored_button(" Quotex ", "livefsbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
            colored_button(" Pocket Option ", "livefsbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
           [colored_button(" Tradowix ", "livefsbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
       ]
       entities = build_custom_emoji_entities(broker_text)
       await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
       return

    elif data in ("livefsbroker_quotex", "livefsbroker_pocket", "livefsbroker_tradowix"):
       uid = query.from_user.id
       context.user_data['lfc_broker'] = data.replace("livefsbroker_", "")
       # Default: pull from the unified Settings > UTC Selection value for
       # "livefs" so a UTC set from the main menu is respected even before
       # this screen has been opened once (falls back to +5 Pakistan).
       if 'lfc_utc' not in context.user_data:
          context.user_data['lfc_utc'] = get_tz_minutes(uid, "livefs") // 60
       context.user_data['state'] = STATE_LIVEFS_CHECKER_SETTINGS
       offset = context.user_data['lfc_utc']
       ...
       msg = (
        ...
        f"M1;EURUSD;14:26;CALL\n"
        f"M1 EURUSD 14:42 PUT\n\n"
        f"Press Home button for main menu"
       )
       buttons = [
        [colored_button(" CHANGE UTC", "lfc_change_utc", KeyboardButtonStyle.PRIMARY, "5447410659077661506")],
    ...
    elif data == "lfc_change_utc":
       context.user_data['state'] = STATE_LIVEFS_CHECKER_SETTINGS
       await show_utc_selection(update, context, prefix="lfc_utc_", back_callback="lfc_utc_back")
       return

    elif data == "lfc_utc_back":
       context.user_data['state'] = STATE_LIVEFS_CHECKER_SETTINGS
       uid = query.from_user.id
       offset = context.user_data.get('lfc_utc', 5)
       msg = (
        f"🔍 LIVE FS CHECKER\n"
        f"🎬 Settings:  UTC +{offset:02d}:00  (To change UTC click the UTC Change button)\n\n"
        f"🪇 Send your list now (All valid formats are supported):\n"
    ...
    elif data.startswith("lfc_utc_page_"):
       page = int(data.split("_")[-1])
       context.user_data['utc_page'] = page
       await show_utc_selection(update, context, prefix="lfc_utc_", back_callback="lfc_utc_back")
       return

    elif data.startswith("lfc_utc_"):
       offset_str = data.replace("lfc_utc_", "")
       try:
          offset = int(offset_str)
       except ValueError:
          return
       context.user_data['lfc_utc'] = offset
       # Keep the unified Settings > UTC Selection value in sync so both
       # this legacy button and the new consolidated menu agree.
       get_state(query.from_user.id).tz_settings['livefs'] = offset * 60
       context.user_data['state'] = STATE_LIVEFS_CHECKER_SETTINGS
       uid = query.from_user.id
       msg = (
        f"🔍 LIVE FS CHECKER\n"
       ...
       buttons = [
        ...
        [colored_button(" Custom Date ", "lfc_date_custom", KeyboardButtonStyle.PRIMARY, "5217822164362739968")],
       ]
       markup = InlineKeyboardMarkup(buttons)
       entities = build_custom_emoji_entities(msg)
       await query.edit_message_text(msg, entities=entities, reply_markup=markup)
       ...
       if data == "lfc_date_today":
          date_str = datetime.now(timezone(timedelta(hours=5))).strftime("%Y-%m-%d")
       elif data == "lfc_date_yesterday":
          date_str = (datetime.now(timezone(timedelta(hours=5))) - timedelta(days=1)).strftime("%Y-%m-%d")
       else:  # custom date handled in text handler
          return
       context.user_data['lfc_date'] = date_str
       await show_lfc_payout_filter(update, context)
       return

    elif data == "lfc_payout_yes":
       query = update.callback_query
       await safe_query_answer(query)
       context.user_data['lfc_payout_filter'] = True
       signals = context.user_data.get('lfc_signals_raw')
       date_str = context.user_data.get('lfc_date')
       mtg = context.user_data.get('lfc_mtg', 0)
       if not signals or not date_str:
          await query.edit_message_text("❌ Data missing. Please start again.")
          return
       await query.edit_message_text("⏳ Processing signals...\n🔄 Progress will show below.")
       lines = [f"M1;{p};{t};{d}" for p, t, d in signals]
       signals_text = "\n".join(lines)
       broker = context.user_data.get('lfc_broker', 'quotex')
       threading.Thread(
          target=run_livefs_checker,
          args=(query.from_user.id, date_str, mtg, signals_text, context, True),
          kwargs={"broker": broker},
          daemon=True
       ).start()
       return

    elif data == "lfc_payout_no":
       query = update.callback_query
       await safe_query_answer(query)
       context.user_data['lfc_payout_filter'] = False
       signals = context.user_data.get('lfc_signals_raw')
       date_str = context.user_data.get('lfc_date')
       mtg = context.user_data.get('lfc_mtg', 0)
       if not signals or not date_str:
       ...
       threading.Thread(
          target=run_livefs_checker,
          args=(query.from_user.id, date_str, mtg, signals_text, context, False),
          kwargs={"broker": broker},
          daemon=True
       ).start()
       return

    elif data == "lfc_show_loss":
       query = update.callback_query
       await safe_query_answer(query)
       uid = query.from_user.id
       loss_data_list = context.user_data.get('lfc_loss_signals', [])
       ...
       for idx, loss_data in enumerate(loss_data_list, 1):
          try:
              ...
              chart_candles = []
              for c in candles:
                  ts = c.get('epoch') or c.get('time', 0)
                  try: ts = int(ts)
                  except: ts = 0
                  chart_candles.append({
                      'time': ts,
    ...
    elif data == "lfc_recheck_payout":
       query = update.callback_query
       await safe_query_answer(query)
       uid = query.from_user.id
       signals = context.user_data.get('lfc_raw_signals')
       date_str = context.user_data.get('lfc_date')
       mtg = context.user_data.get('lfc_mtg', 0)
       if not signals or not date_str:
          await query.edit_message_text("❌ Data missing. Please start again.")
          return
       await query.edit_message_text("🔄 Rechecking with payout filter ON...")
       lines = [f"M1;{s['pair']};{s['time']};{s['dir']}" for s in signals]
       signals_text = "\n".join(lines)
       broker = context.user_data.get('lfc_broker', 'quotex')
       threading.Thread(
          target=run_livefs_checker,
          args=(uid, date_str, mtg, signals_text, context, True),
          kwargs={"broker": broker},
          daemon=True
       ).start()
       return
    ...
    # -------- FIX: livefs_pair_done handled BEFORE startswith --------
       ...
       # Strategy 4 – Pattern Replay. Broker was already set by the
       # broker-selection screen (futbroker_quotex/futbroker_pocket) or by
       # /future (which forces quotex) — don't clobber it here.
       context.user_data['uid'] = query.from_user.id
       context.user_data.setdefault('fut4_broker', 'quotex')
       broker_label = fut4_broker_label(context.user_data['fut4_broker'])
       context.user_data['state'] = STATE_FUT4_DAYS
       msg = f"🧠 SMZX PRIV CORE 𝚂𝚃𝚁𝙰𝚃𝙴𝙶𝚈 ({broker_label})\n━━━━━━━━━━━━━━━━━━\n𝚂𝚎𝚕𝚎𝚌𝚝 𝙻𝚘𝚘𝚔𝚋𝚊𝚌𝚔 𝙳𝚊𝚢𝚜 (𝟸-𝟷𝟶):"
       buttons = [
        ...
        [colored_button(" 7 Days ", "fut4_days_7", KeyboardButtonStyle.PRIMARY, "6145553439809084250"),
         colored_button(" 10 Days ", "fut4_days_10", KeyboardButtonStyle.PRIMARY, "6145553439809084250")],
       ...
       if not await is_authorized_async(uid):
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       context.user_data['state'] = STATE_FINDER_BROKER
       msg = "🔍 STRATEGY FINDER\n\n🏦 Select Broker:"
       buttons = [
        [colored_button(" Quotex ", "finderbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
         colored_button(" Pocket Option ", "finderbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
        [colored_button(" Tradowix ", "finderbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
       ]
       await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(buttons))
       await safe_query_answer(query)
       return

    elif data in ("finderbroker_quotex", "finderbroker_pocket", "finderbroker_tradowix"):
       context.user_data['finder_broker'] = data.replace("finderbroker_", "")
       context.user_data['state'] = STATE_FINDER_MARKET
       broker_label = fut4_broker_label(context.user_data['finder_broker'])
       msg = f"🔍 STRATEGY FINDER ({broker_label})\n\nSelect market to scan:"
       buttons = [
       ...
       context.user_data['state'] = STATE_FINDER_MARKET
       broker_label = fut4_broker_label(context.user_data.get('finder_broker', 'quotex'))
       msg = f"🔍 STRATEGY FINDER ({broker_label})\n\nSelect market to scan:"
       buttons = [
        [colored_button("OTC", "finder_market_otc", KeyboardButtonStyle.SUCCESS, "6145248943807667330")],
        [colored_button("LIVE", "finder_market_live", KeyboardButtonStyle.PRIMARY, "6062085844242537125")],
    ...
    # ==================== MULTI ENGINE ====================
       ...
       if not await is_authorized_async(uid):
          await safe_query_answer(query, "⛔ Access denied.", show_alert=True)
          return
       context.user_data['state'] = STATE_MULTI_ENGINE_BROKER
       msg = "🔫 MULTI ENGINE MODE\n\n🏦 Select Broker:"
       entities = build_custom_emoji_entities(msg)
       buttons = [
        [colored_button(" Quotex ", "multibroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
         colored_button(" Pocket Option ", "multibroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
        [colored_button(" Tradowix ", "multibroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
       ]
       await query.edit_message_text(msg, entities=entities, reply_markup=InlineKeyboardMarkup(buttons))
       await safe_query_answer(query)
       return

    elif data in ("multibroker_quotex", "multibroker_pocket", "multibroker_tradowix"):
       uid = query.from_user.id
       context.user_data['multi_broker'] = data.replace("multibroker_", "")
       context.user_data['state'] = STATE_MULTI_ENGINE_MAIN
       broker_label = fut4_broker_label(context.user_data['multi_broker'])
       msg = f"🔫 MULTI ENGINE MODE ({broker_label})\n\nSelect your desired mode:"
       entities = build_custom_emoji_entities(msg)
       buttons = [
       ...
       broker = context.user_data.get('multi_broker', 'quotex')
       pairs = multi_engine_pairs(broker, market)
       ...
       await safe_query_answer(query, f"Analyzing {pair}...")
       ...
       await safe_query_answer(query, f"Re‑analysing {pair}...")
       ...
       context.user_data['multi_ns_market'] = 'otc'
       context.user_data['multi_ns_filter'] = 'all'
       await show_multi_ns_strategy_selection(update, context)
       return

    elif data == "multi_ns_live":
       context.user_data['multi_ns_market'] = 'live'
       context.user_data['multi_ns_filter'] = 'all'
       await show_multi_ns_strategy_selection(update, context)
       return

    elif data == "multi_ns_all":
       context.user_data['multi_ns_market'] = 'both'
       context.user_data['multi_ns_filter'] = 'all'
       await show_multi_ns_strategy_selection(update, context)
       return

    elif data == "multi_ns_avoid":
       context.user_data['multi_ns_filter'] = 'avoid'
       context.user_data['multi_ns_market'] = 'otc'  # default
       await show_multi_ns_strategy_selection(update, context)
       return

    elif data == "multi_ns_manual":
       context.user_data['multi_ns_filter'] = 'manual'
       context.user_data['multi_ns_market'] = 'otc'
       await show_multi_ns_strategy_selection(update, context)
       return

    elif data == "multi_ns_strategy_1":
       query = update.callback_query
       await safe_query_answer(query)
       context.user_data['multi_ns_strategy'] = 1  # Old SuperTrend
       await start_multi_nonstop_with_strategy(update, context)
       return

    elif data == "multi_ns_strategy_2":
       query = update.callback_query
       await safe_query_answer(query)
       context.user_data['multi_ns_strategy'] = 2  # New Pullback Strategy
       await start_multi_nonstop_with_strategy(update, context)
       return
       ...
       if broker == 'quotex':
           ...
           all_pairs = multi_engine_pairs(broker, 'otc') + multi_engine_pairs(broker, 'live')
       ...
       broker = context.user_data.get('multi_broker', 'quotex')
       base_pairs = multi_engine_pairs(broker, market)
       filtered = []
       ...
       await start_multi_nonstop(update, context, filtered)
       return
       ...
       await safe_query_answer(query, "Here are the pairs again.")
       return
       ...
       pairs = multi_engine_pairs(broker, 'otc')
       context.user_data['multi_manual_toggle_pairs'] = pairs
       ...
       # This pool must stay locked to whichever broker was chosen when the
       # toggle list was first built — never fall back to the Quotex pool
       # for a Pocket/Tradowix session.
       broker = context.user_data.get('multi_broker', 'quotex')
       toggle_pairs = context.user_data.get('multi_manual_toggle_pairs') or multi_engine_pairs(broker, 'otc')
       ...
       if pair_data is None:
          ...
          async def fetch_payout(p):
              _, _, payout = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, fetch_candles_mrbeaxt, p, 1, "1m", broker)
              return p, payout
          tasks = [fetch_payout(p) for p in toggle_pairs]
          results = await asyncio.gather(*tasks)
          pair_data = [(pair, payout) for pair, payout in results]
          context.user_data['multi_pair_data'] = pair_data
          await loading.delete()
       await show_multi_pair_list(update, context, toggle_pairs, manual=False, pair_data=pair_data)
       return
       ...
       if data == "multi_pause":
          ...
          await safe_query_answer(query, "⏸️ Paused")
       elif data == "multi_resume":
          context.user_data['multi_paused'] = False
          await safe_query_answer(query, "▶️ Resumed")
       else:  # multi_stop
          # 🔥 FORCE RESET – Saare flags clear karo
          force_reset_user_session(uid, context)
          await safe_query_answer(query, "🔴 Stopping...")
       ...
       context.user_data['state'] = STATE_MULTI_ENGINE_MAIN
       broker_label = fut4_broker_label(context.user_data.get('multi_broker', 'quotex'))
       msg = f"🔫 MULTI ENGINE MODE ({broker_label})\n\nSelect your desired mode:"
       entities = build_custom_emoji_entities(msg)
       buttons = [
        [colored_button(" Manual Signal", "multi_manual", KeyboardButtonStyle.SUCCESS, "5215327832040811010")],   # 🚀
       ...
       msg = (
        ...
        f"⏰ Use {format_utc_offset(get_tz_minutes(uid, 'aifilterfs'))} time\n"
       ...
       await query.message.reply_text(msg, entities=entities)
        ...
        context.user_data['live_broker'] = 'quotex'  # Forex Live Mode is Quotex-only for now
        text = "🤖 𝚂𝙴𝙻𝙴𝙲𝚃 𝚂𝚃𝚁𝙰𝚃𝙴𝙶𝚈 (1-7):"
        buttons = []
        for i in range(1, 8):
           style = KeyboardButtonStyle.SUCCESS if i % 2 else KeyboardButtonStyle.PRIMARY
        ...
        msg = f"⏰ 𝙴𝚗𝚝𝚎𝚛 𝚜𝚝𝚊𝚛𝚝 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {format_utc_offset(get_tz_minutes(uid, 'blackout'))}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 09:00"
        entities = build_custom_emoji_entities(msg)
        await query.message.reply_text(msg, entities=entities)
        return
       ...
       selected = context.user_data.get('fut5_selected_pairs', set())
       if not selected:
          await safe_query_answer(query, "❌ 𝙰𝚝 𝚕𝚎𝚊𝚜𝚝 𝚘𝚗𝚎 𝚙𝚊𝚒𝚛 𝚜𝚎𝚕𝚎𝚌𝚝 𝚔𝚊𝚛𝚎𝚒𝚗!", show_alert=True)
          return
       context.user_data['fut5_selected_pairs'] = selected
       ...
       msg = f"✅ 𝙰𝚌𝚌𝚞𝚛𝚊𝚌𝚢: {accuracy}%\n\n⏰ 𝙴𝚗𝚝𝚎𝚛 𝚜𝚝𝚊𝚛𝚝 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {format_utc_offset(get_tz_minutes(uid, 'otcfuture'))}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 09:00"
       entities = build_custom_emoji_entities(msg)
       await query.edit_message_text(msg, entities=entities)
       return
       ...
       return
        ...
        if 'bl_checker_utc' not in context.user_data:
            context.user_data['bl_checker_utc'] = get_tz_minutes(uid, "blackout") // 60
        offset = context.user_data['bl_checker_utc']
        msg = (
            f"🗓 𝙱𝙻𝙰𝙲𝙺𝙾𝚄𝚃 𝙲𝙷𝙴𝙲𝙺𝙴𝚁\n"
            f"🎬 UTC +{offset:02d}:00  (To change UTC click the UTC Change button)\n\n"
            f"𝚂𝚎𝚕𝚎𝚌𝚝 𝚍𝚊𝚝𝚎:"
        )
        buttons = [
            [colored_button(" CHANGE UTC", "bl_check_change_utc", KeyboardButtonStyle.PRIMARY, "5447410659077661506")],
    ...
    elif data == "bl_check_change_utc":
        context.user_data['state'] = STATE_BLACKOUT_CHECKER_DATE
        await show_utc_selection(update, context, prefix="bl_utc_", back_callback="bl_check_back_to_menu")
        return

    elif data.startswith("bl_utc_page_"):
        page = int(data.split("_")[-1])
        context.user_data['utc_page'] = page
        await show_utc_selection(update, context, prefix="bl_utc_", back_callback="bl_check_back_to_menu")
        return

    elif data.startswith("bl_utc_"):
        offset_str = data.replace("bl_utc_", "")
        try:
            offset = int(offset_str)
        except ValueError:
            return
        context.user_data['bl_checker_utc'] = offset
        # Keep the unified Settings > UTC Selection value in sync so both
        # this legacy button and the new consolidated menu agree.
        get_state(uid).tz_settings['blackout'] = offset * 60
        context.user_data['state'] = STATE_BLACKOUT_CHECKER_DATE
        msg = (
            f"🗓 𝙱𝙻𝙰𝙲𝙺𝙾𝚄𝚃 𝙲𝙷𝙴𝙲𝙺𝙴𝚁\n"
            f"🎬 UTC +{offset:02d}:00  (To change UTC click the UTC Change button)\n\n"
    ...
    elif data == "bl_check_back_to_menu":
        context.user_data['state'] = STATE_BLACKOUT_CHECKER_DATE
        offset = context.user_data.get('bl_checker_utc', 5)
        msg = (
            f"🗓 𝙱𝙻𝙰𝙲𝙺𝙾𝚄𝚃 𝙲𝙷𝙴𝙲𝙺𝙴𝚁\n"
            f"🎬 UTC +{offset:02d}:00  (To change UTC click the UTC Change button)\n\n"
            f"𝚂𝚎𝚕𝚎𝚌𝚝 𝚍𝚊𝚝𝚎:"
        ...
        if not signals or not date_str:
            ...
            return
        context.user_data['state'] = STATE_BLACKOUT_CHECKER_PAYOUT
        ...
        buttons = [
            [
                colored_button(" Yes", "bl_payout_yes", KeyboardButtonStyle.SUCCESS, "6147440218942218700"),
                colored_button(" No", "bl_payout_no", KeyboardButtonStyle.DANGER, "6145317070578916456")
            ]
        ]
        markup = InlineKeyboardMarkup(buttons)
        entities = build_custom_emoji_entities(msg)
    ...
    elif data.startswith("bl_payout_"):
        payout_filter = data == "bl_payout_yes"
        context.user_data['bl_checker_payout_filter'] = payout_filter
        signals = context.user_data.get('bl_checker_signals', [])
        date_str = context.user_data.get('bl_checker_date')
        mtg_level = context.user_data.get('bl_checker_mtg', 1)
        if not signals or not date_str:
            await query.message.reply_text("❌ 𝙳𝚊𝚝𝚊 𝚖𝚒𝚜𝚜𝚒𝚗𝚐. 𝙿𝚕𝚎𝚊𝚜𝚎 𝚜𝚝𝚊𝚛𝚝 𝚊𝚐𝚊𝚒𝚗.")
            return
        await query.message.reply_text(f"⏳ 𝙲𝚑𝚎𝚌𝚔𝚒𝚗𝚐 {len(signals)} 𝚜𝚒𝚐𝚗𝚊𝚕𝚜 (𝙼𝚃𝙶={mtg_level})...\n⏰ 𝚃𝚑𝚒𝚜 𝚖𝚊𝚢 𝚝𝚊𝚔𝚎 𝚊 𝚏𝚎𝚠 𝚜𝚎𝚌𝚘𝚗𝚍𝚜.")
        threading.Thread(target=run_blackout_checker_worker, args=(query.from_user.id, date_str, signals, mtg_level, context, payout_filter), daemon=True).start()
        context.user_data['state'] = None
        return
        ...
        broker_buttons = [
            [colored_button(" Quotex ", "checkerbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
             colored_button(" Pocket Option ", "checkerbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
            [colored_button(" Tradowix ", "checkerbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
        ]
        entities = build_custom_emoji_entities(broker_text)
        await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
        return

    elif data in ("checkerbroker_quotex", "checkerbroker_pocket", "checkerbroker_tradowix"):
        uid = query.from_user.id
        context.user_data['checker_broker'] = data.replace("checkerbroker_", "")
        if 'checker_utc' not in context.user_data:
          context.user_data['checker_utc'] = int(get_tz_minutes(uid, "checker") / 60)
        context.user_data['state'] = STATE_CHECKER_SETTINGS
        broker_label = fut4_broker_label(context.user_data['checker_broker'])
        msg = (
        f"🔍 SMZX OTC CHECKER ({broker_label})\n"
       ...
       # Keep the unified Settings > UTC Selection value in sync so both
       # this legacy button and the new consolidated menu agree.
       get_state(uid).tz_settings['checker'] = offset * 60
       context.user_data['state'] = STATE_CHECKER_SETTINGS
       msg = (
        f"🔍 SMZX OTC CHECKER\n"
        f"🎬 Settings:  UTC +{offset:02d}:00  (To change UTC click the UTC Change button)\n\n"
       ...
       buttons = [
        ...
        [
            colored_button(" Custom", "checker_date_custom", KeyboardButtonStyle.PRIMARY, "6102689176568732825")
        ]
       ]
       markup = InlineKeyboardMarkup(buttons)
       ...
       now_pk = datetime.now(timezone(timedelta(hours=5)))
       if data == "checker_date_custom":
          context.user_data['state'] = STATE_CHECKER_CUSTOM_DATE
          msg = (
              "🗓 𝙲𝚞𝚜𝚝𝚘𝚖 𝙳𝚊𝚝𝚎\n\n"
              "📝 Send the date to check (YYYY-MM-DD), e.g. 2026-08-12\n"
              "⏳ Allowed range: 1–25 days back from today"
          )
          entities = build_custom_emoji_entities(msg) + build_bold_entities(msg, ["Custom Date"])
          await query.edit_message_text(msg, entities=entities)
          return
       ...
       broker = context.user_data.get('checker_broker', 'quotex')
       await query.edit_message_text(":)")
       await run_advanced_checker(update, context, signals, mtg, date_str, user_utc, enable_filter, broker=broker)
       return
       ...
       await run_advanced_checker(update, context, signals, mtg, date_str, user_utc, True, broker=broker)
       return
        ...
        broker_buttons = [
            [colored_button(" Quotex ", "futbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
             colored_button(" Pocket Option ", "futbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
            [colored_button(" Tradowix ", "futbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
        ]
        entities = build_custom_emoji_entities(broker_text)
        await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
        return

    elif data == "futbroker_quotex":
        uid = query.from_user.id
        context.user_data['fut4_broker'] = 'quotex'
        fut_text = (
            "🔥 𝙵𝚄𝚃𝚄𝚁𝙴 𝚂𝙸𝙶𝙽𝙰𝙻𝚂 (𝚀𝚄𝙾𝚃𝙴𝚇)\n\n"
            "📊 Select Strategy:\n\n"
    ...
    elif data == "futbroker_pocket":
        # Pocket Option supports Strategy 4 (Pattern Replay) and
        # Strategy 2 (SMZ Hacking Mode) — both candle-fetch driven, so both
        # can point at the Pocket candle source.
        uid = query.from_user.id
        context.user_data['fut4_broker'] = 'pocket'
        context.user_data['uid'] = uid
        fut_text = (
            "🔥 𝙵𝚄𝚃𝚄𝚁𝙴 𝚂𝙸𝙶𝙽𝙰𝙻𝚂 (𝙿𝙾𝙲𝙺𝙴𝚃 𝙾𝙿𝚃𝙸𝙾𝙽)\n\n"
            "📊 Select Strategy:\n\n"
            "🥷 Strategy 2 – SMZ Hacking Mode\n"
            "   └ Advanced signals from candle data\n\n"
            "🧠 Strategy 4 – SMZ PRIV CORE\n"
            "   └ Pattern Replay signals\n"
        )
        fut_buttons = [
            [colored_button(" Strategy 2 – SMZ Hacking", "fut_strategy_2", KeyboardButtonStyle.PRIMARY, "6217370240800527004")],
            [colored_button(" Strategy 4 – SMZ PRIV CORE", "fut_strategy_4", KeyboardButtonStyle.SUCCESS, "6145248943807667330")],
    ...
    elif data == "futbroker_tradowix":
        # Tradowix supports the same 2 candle-fetch driven strategies as
        # Pocket Option: Strategy 2 (SMZ Hacking Mode) and Strategy 4
        # (Pattern Replay).
        uid = query.from_user.id
        context.user_data['fut4_broker'] = 'tradowix'
        context.user_data['uid'] = uid
        fut_text = (
            "🔥 𝙵𝚄𝚃𝚄𝚁𝙴 𝚂𝙸𝙶𝙽𝙰𝙻𝚂 (𝚃𝚁𝙰𝙳𝙾𝚆𝙸𝚇)\n\n"
            "📊 Select Strategy:\n\n"
            "🥷 Strategy 2 – SMZ Hacking Mode\n"
            "   └ Advanced signals from candle data\n\n"
            "🧠 Strategy 4 – SMZ PRIV CORE\n"
       ...
       await query.message.reply_text(msg, entities=entities, reply_markup=markup)
    ...
    elif data in ("menu_news", "menu_news_filter", "menu_news_signal"):
        await show_news_home(update, context)
        return
        ...
        # Strategy 2 – SMZ Hacking Mode. Broker was already set by the
        # broker-selection screen or /future (forces quotex) — don't clobber.
        context.user_data.setdefault('fut4_broker', 'quotex')
        context.user_data['smz_step'] = 'start_time'
        msg = f"⏰ 𝙴𝚗𝚝𝚎𝚛 𝚜𝚝𝚊𝚛𝚝 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {format_utc_offset(get_tz_minutes(uid, 'otcfuture'))}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 09:00"
        entities = build_custom_emoji_entities(msg)
        await query.message.reply_text(msg, entities=entities)
        return
        ...
        broker_buttons = [
            [colored_button(" Quotex ", "btbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
             colored_button(" Pocket Option ", "btbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
            [colored_button(" Tradowix ", "btbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
        ]
        entities = build_custom_emoji_entities(broker_text)
        await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
        return

    elif data in ("btbroker_quotex", "btbroker_pocket", "btbroker_tradowix"):
        if data == "btbroker_pocket":
            context.user_data['backtest_broker'] = 'pocket'
        elif data == "btbroker_tradowix":
            context.user_data['backtest_broker'] = 'tradowix'
        else:
            context.user_data['backtest_broker'] = 'quotex'
       ...
       broker_buttons = [
           [colored_button(" Quotex ", "trendbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
            colored_button(" Pocket Option ", "trendbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
           [colored_button(" Tradowix ", "trendbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
       ]
       entities = build_custom_emoji_entities(broker_text)
       await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
       return

    elif data in ("trendbroker_quotex", "trendbroker_pocket", "trendbroker_tradowix"):
       uid = query.from_user.id
       context.user_data['trend_broker'] = data.replace("trendbroker_", "")
       if 'trend_mode' not in context.user_data:
          context.user_data['trend_mode'] = 'EMA'
       if 'trend_ema' not in context.user_data:
          context.user_data['trend_ema'] = 20  # default
    
       context.user_data['state'] = STATE_TREND_FILTER_SETTINGS
       mode = context.user_data['trend_mode']
    
       msg = (
        f"↗️ {fancy_font('TREND FILTER')}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🌐 {fancy_font('Paste signals in ANY FORMAT:')}\n"
        f"M1;EURUSD-OTC;04:22;CALL\n"
        f"⧉ GBPUSD - 13:00 - PUT\n"
        f"EUR/USD = 14:30 = BUY\n\n"
        f"⏰ {fancy_font('Use ' + format_utc_offset(get_tz_minutes(uid, 'trendfilter')) + ' time')}\n"
        f"🌐 {fancy_font('Mode:')} {fancy_font(mode)}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📌 {fancy_font('Click Change Mode to switch')}\n"
        f"📝 {fancy_font('Paste your signals below:')}"
       )
       buttons = [
        [colored_button(" Change Mode", "trend_change_mode", KeyboardButtonStyle.PRIMARY, "6104726047628990417")],
        [colored_button(" Home", "back_to_main", KeyboardButtonStyle.SUCCESS, "5416041192905265756")]
       ]
       markup = InlineKeyboardMarkup(buttons)
       entities = build_custom_emoji_entities(msg) + build_bold_entities(msg, ["TREND FILTER", f"Mode: {mode}"])
       await query.message.reply_text(msg, entities=entities, reply_markup=markup)
       return

    elif data == "trend_change_mode":
       query = update.callback_query
       await safe_query_answer(query)
       # Toggle between EMA and SMA
       current = context.user_data.get('trend_mode', 'EMA')
       new_mode = "SMA" if current == "EMA" else "EMA"
       context.user_data['trend_mode'] = new_mode
       ...
       msg = (
        ...
        f"⧉ GBPUSD - 13:00 - PUT\n"
        f"EUR/USD = 14:30 = BUY\n\n"
        f"🌐 {fancy_font('Mode:')} {fancy_font(new_mode)}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📌 {fancy_font('Click Change Mode to switch')}\n"
        f"📝 {fancy_font('Paste your signals below:')}"
       )
       ...
       markup = InlineKeyboardMarkup(buttons)
       entities = build_custom_emoji_entities(msg) + build_bold_entities(msg, ["TREND FILTER", f"Mode: {new_mode}"])
       await query.edit_message_text(msg, entities=entities, reply_markup=markup)
       return

    elif data.startswith("trend_ema_"):
       query = update.callback_query
       await safe_query_answer(query)
       ema_period = data.replace("trend_ema_", "")
    
       if ema_period == "custom":
          context.user_data['state'] = STATE_TREND_FILTER_EMA
          msg = "🔢 Enter custom EMA period (e.g., 50):"
          entities = build_custom_emoji_entities(msg)
          await query.edit_message_text(msg, entities=entities)
          return
       ...
       try:
          ...
          await query.edit_message_text("❌ Invalid EMA period.")
          return
    
       context.user_data['trend_ema'] = ema_period
    
       loading_msg = "⏳ Processing signals...\n🔄 0% [░░░░░░░░░░] 0/0"
       entities = build_custom_emoji_entities(loading_msg)
       await query.edit_message_text(loading_msg, entities=entities)
    
       signals = context.user_data.get('trend_signals_raw', [])
       mode = context.user_data.get('trend_mode', 'EMA')
       broker = context.user_data.get('trend_broker', 'quotex')
       threading.Thread(
          target=run_trend_filter,
          args=(query.from_user.id, signals, mode, ema_period, context),
          kwargs={"broker": broker},
          daemon=True
       ).start()
       return
        ...
        broker_buttons = [
            [colored_button(" Quotex ", "asigbroker_quotex", KeyboardButtonStyle.SUCCESS, "6214983170991853422"),
             colored_button(" Pocket Option ", "asigbroker_pocket", KeyboardButtonStyle.PRIMARY, "5258033997535012714")],
            [colored_button(" Tradowix ", "asigbroker_tradowix", KeyboardButtonStyle.PRIMARY, "6337101525147458464")],
        ]
        entities = build_custom_emoji_entities(broker_text)
        await query.message.reply_text(broker_text, entities=entities, reply_markup=InlineKeyboardMarkup(broker_buttons))
        return

    elif data in ("asigbroker_quotex", "asigbroker_pocket", "asigbroker_tradowix"):
        uid = query.from_user.id
        context.user_data['auto_signal_broker'] = data.replace("asigbroker_", "")
        context.user_data['uid'] = uid
        ...
        msg = (
            "📸 𝙰𝙸 𝙲𝙷𝙰𝚁𝚃 𝙰𝙽𝙰𝙻𝚈𝚉𝙴𝚁\n\n"
            "⏰ Pehle timeframe select karein — signal usi ke hisaab se banega:"
        )
        buttons = [
            [colored_button(" 1 Minute ", "chart_analyzer_tf_1m", KeyboardButtonStyle.SUCCESS, "6145553439809084250"),
             colored_button(" 5 Minute ", "chart_analyzer_tf_5m", KeyboardButtonStyle.SUCCESS, "6145553439809084250")],
        ]
        entities = build_custom_emoji_entities(msg)
        await query.message.reply_text(msg, entities=entities, reply_markup=InlineKeyboardMarkup(buttons))
    ...
    elif data in ("chart_analyzer_tf_1m", "chart_analyzer_tf_5m"):
        tf = "1M" if data == "chart_analyzer_tf_1m" else "5M"
        context.user_data['chart_analyzer_tf'] = tf
        context.user_data['state'] = STATE_CHART_ANALYZER
        context.user_data['uid'] = uid
        msg = (
            f"📸 𝙰𝙸 𝙲𝙷𝙰𝚁𝚃 𝙰𝙽𝙰𝙻𝚈𝚉𝙴𝚁 — {tf}\n\n"
            "🔰 𝚂𝚎𝚗𝚍 𝚖𝚎 𝚊 𝚌𝚑𝚊𝚛𝚝 𝚜𝚌𝚛𝚎𝚎𝚗𝚜𝚑𝚘𝚝 𝚊𝚗𝚍 𝙸 𝚠𝚒𝚕𝚕 𝚊𝚗𝚊𝚕𝚢𝚣𝚎 𝚒𝚝!\n\n"
            "💎 I will detect:\n"
            "  📊 Candlestick patterns\n"
            "  📈 Trend direction\n"
            "  🔥 Support & Resistance\n"
            f"  🤖 Next {tf} signal (CALL/PUT)\n\n"
    ...
    def target():
        try:
            ...
            if signals:
                ...
                footer = f"\n\n🖥  @Rohailtrader"
                ok = send_signal_list_parts(uid, header, signals, footer)
                if not ok:
                    sender.send_message(uid, "⚠️ Signals generated but Telegram rejected the pages. Try fewer pairs.")
            else:
                sender.send_message(uid, "❌ No signals found. Try different settings.")
        except Exception as e:
            ...
            try:
                mid = getattr(loading_msg, "message_id", None) or getattr(loading_msg, "id", None)
                if mid:
                    sender.delete_message(uid, mid)
            except Exception:
                pass
...
async def send_blackout_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "   𝙼𝟷;𝙶𝙱𝙿𝙹𝙿𝚈-𝙾𝚃𝙲;08:24\n"
        "   𝙼𝟷;𝙴𝚄𝚁𝚄𝚂𝙳-𝙾𝚃𝙲;09:15\n\n"
        f"⏰ 𝚄𝚜𝚎 𝚄𝚃𝙲+{offset:02d} 𝚝𝚒𝚖𝚎\n"
        "📌 𝙿𝚊𝚜𝚝𝚎 𝚢𝚘𝚞𝚛 𝚜𝚒𝚐𝚗𝚊𝚕𝚜 𝚗𝚘𝚠..."
    )
    entities = build_custom_emoji_entities(msg)
    await context.bot.send_message(chat_id=uid, text=msg, entities=entities)

async def show_lfc_payout_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show payout filter Yes/No buttons (same as advance checker)"""
    query = update.callback_query
    if query:
        await safe_query_answer(query)
    uid = update.effective_user.id
    context.user_data['state'] = STATE_LIVEFS_CHECKER_PAYOUT
    
    msg = (
        "💰 𝙿𝚊𝚢𝚘𝚞𝚝 𝙵𝚒𝚕𝚝𝚎𝚛\n\n"
        "✅ Enable Payout Filter\n"
        "Signals with payout below 80% will be shown with their payout% "
        "and will not be counted as WIN or LOSS.\n\n"
        "Would you like to enable the payout filter?"
    )
    buttons = [
        [
            colored_button(" Yes", "lfc_payout_yes", KeyboardButtonStyle.SUCCESS, "6147440218942218700"),
            colored_button(" No", "lfc_payout_no", KeyboardButtonStyle.DANGER, "6145317070578916456")
        ]
    ]
    markup = InlineKeyboardMarkup(buttons)
    entities = build_custom_emoji_entities(msg)
...
async def continue_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    # Capture ai_mode BEFORE force_reset_user_session, since that function
    # unconditionally sets st.ai_mode = False — checking it afterwards would
    # always be False and AI Mode's /continue path would never run.
    st_pre = get_state(uid)
    was_ai_mode = st_pre.ai_mode
    force_reset_user_session(uid, context)
    if not await is_authorized_async(uid):
        await update.message.reply_text("⛔ Access denied.")
        return
    ...
    if was_ai_mode:
        ...
        try:
            await update.message.reply_text("🤖 AI Mode — Scanning for next best signal...")
        except Exception as e:
            print(f"[CONTINUE REPLY ERROR] {e}")
    else:
        bot = SMZXBot(uid, broker=context.user_data.get('live_broker', 'quotex'))
        ...
        threading.Thread(target=bot.run_single_signal, daemon=True).start()
        try:
            await update.message.reply_text("Continuing with next signal...")
        except Exception as e:
            print(f"[CONTINUE REPLY ERROR] {e}")
...
async def stop_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # /stop only used to reset `st` (state object) and the auto-trader, but
    # NEVER cleared context.user_data['multi_running'] / 'multi_paused' —
    # those are only cleared by the in-panel "Stop" button (multi_stop).
    # So if a user started Multi Engine Non-Stop and then typed /stop
    # instead of tapping the panel button, multi_running stayed True
    # forever, and every future "Start Non-Stop" attempt hit the
    # "⚠️ Multi Engine already running. Use Stop first." guard with no way
    # out except literally tapping the old (possibly gone) panel's Stop
    # button. Clear it here too so /stop always fully resets the session.
    context.user_data['multi_running'] = False
    context.user_data['multi_paused'] = False

    # /stop is a safety-critical command — send it the same way /start does
    # (direct native call, bypassing our shared send queue entirely) so it
    # always replies instantly regardless of how busy the queue is with
    # other users' messages.
    try:
        await update.message.reply_text(
            "🤖 Stopping. Returning to main menu. Use /start to see options.")
    except Exception as e:
        print(f"[STOP REPLY ERROR] {e}")
...
async def auto_trade_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Auto Trade Start – with force reset to prevent stuck sessions.
    """
    try:
        query = update.callback_query
        await safe_query_answer(query)
        ...
        # 🔥 FORCE RESET – Previous session auto-stop
        force_reset_user_session(uid, context)
        
        if not await is_authorized_async(uid):
            await safe_query_answer(query, "⛔ Access denied. Contact Admin.", show_alert=True)
            return
        
        trader = get_auto_trader(uid)
        
        # Extra safety check – though force_reset should have cleared it
        if trader.running:
            trader.running = False
            trader.paused = False
        ...
        print(f"[DEBUG] auto_trade_start: User {uid} started Auto Trade setup.")
...
async def auto_pause_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trader = get_auto_trader(uid)
    if not trader.running:
        await safe_query_answer(query, "Not running", show_alert=False)
        return
    trader.paused = (query.data == "atx_pause")
    await safe_query_answer(query, "⏳ Paused" if trader.paused else "🚀 Resumed")
...
async def backtest_mtg_callback(
    ...
    buttons = [[colored_button(" 3 Days",
               ...
               [colored_button(" 7 Days",
                               ...
                               KeyboardButtonStyle.PRIMARY,
                               "6145553439809084250"),
                colored_button(" 10 Days",
                               "backtest_days_10",
                               KeyboardButtonStyle.PRIMARY,
                               "6145553439809084250")],
               [colored_button(" Custom (1-10)",
...
async def backtest_days_callback(
    ...
    if data == "backtest_days_custom":
        ...
        broker = context.user_data.get('backtest_broker', 'quotex')
        ...
        await run_backtest_local(update, context, days, signals_text, mtg, broker=broker)
...
async def global_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('blk_step') == 'start_time':
        ...
        msg = f"⏰ 𝙴𝚗𝚝𝚎𝚛 𝚎𝚗𝚍 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {format_utc_offset(get_tz_minutes(uid, 'blackout'))}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 16:30"
        entities = build_custom_emoji_entities(msg)
        await update.message.reply_text(msg, entities=entities)
        return
        ...
        date_str_norm = normalize_fancy(text.strip())
        date_str_norm = re.sub(r'[—–−]', '-', date_str_norm)
        if not re.match(r'^\d{4}-\d{2}-\d{2}$', date_str_norm):
            await update.message.reply_text("❌ 𝙸𝚗𝚟𝚊𝚕𝚒𝚍 𝚍𝚊𝚝𝚎. 𝚄𝚜𝚎 𝚈𝚈𝚈𝚈-𝙼𝙼-𝙳𝙳")
            return
        try:
            picked_date = datetime.strptime(date_str_norm, "%Y-%m-%d").date()
        except Exception:
            await update.message.reply_text("❌ 𝙸𝚗𝚟𝚊𝚕𝚒𝚍 𝚍𝚊𝚝𝚎. 𝚄𝚜𝚎 𝚈𝚈𝚈𝚈-𝙼𝙼-𝙳𝙳")
            return
        today_pk = datetime.now(timezone(timedelta(hours=5))).date()
        days_back = (today_pk - picked_date).days
        if days_back < 0 or days_back > 25:
            await update.message.reply_text(
                f"❌ 𝙳𝚊𝚝𝚎 𝚘𝚞𝚝 𝚘𝚏 𝚛𝚊𝚗𝚐𝚎.\n"
                f"📅 Allowed: 0–25 days back from today ({today_pk.strftime('%Y-%m-%d')}).\n"
                f"📝 Try again, e.g. {(today_pk - timedelta(days=3)).strftime('%Y-%m-%d')}"
            )
            return
        context.user_data['bl_checker_date'] = date_str_norm
        context.user_data['state'] = STATE_BLACKOUT_CHECKER_SIGNALS
        await send_blackout_prompt(update, context)
        return
        ...
        if not lines:
            ...
            return
        bl_user_utc = context.user_data.get('bl_checker_utc', 5)
        bl_date_str = context.user_data.get('bl_checker_date')
        tz_bl_user = timezone(timedelta(hours=bl_user_utc))
        tz_bl_pk = timezone(timedelta(hours=5))
        parsed_signals = []
        for line in lines:
            pair, time_str = parse_blackout_signal_line(line)
            if pair and time_str:
                # time_str is in the user's selected UTC offset — convert to
                # UTC+5 (the candle API's convention) for lookups, but keep
                # the original for display, same as Run Advance Checker.
                api_time = time_str
                try:
                    dt_user = datetime.strptime(f"{bl_date_str} {time_str}", "%Y-%m-%d %H:%M")
                    dt_user = dt_user.replace(tzinfo=tz_bl_user)
                    api_time = dt_user.astimezone(tz_bl_pk).strftime("%H:%M")
                except Exception as e:
                    print(f"Blackout time conversion error for {pair} {time_str}: {e}")
                parsed_signals.append((pair, time_str, api_time))
            else:
    ...
    # ===== LIVE FS STATES (NEW) =====
    ...
    elif state == STATE_LIVEFS_CHECKER_SETTINGS:
       # User pasted signals directly from settings screen
       ...
       if not parsed:
          ...
          return
       context.user_data['lfc_signals_raw'] = parsed
       context.user_data['lfc_skipped'] = skipped
       # Build preview (same as OTC Checker)
       preview_lines = []
       for i, (p, t, d) in enumerate(parsed[:5]):
          preview_lines.append(f"M1;{p};{t};{d}")
       if len(parsed) > 5:
       ...
       buttons = [
        [
            colored_button(" MTG 1", "lfc_mtg_1", KeyboardButtonStyle.SUCCESS, "6230982053018475627"),
            colored_button(" MTG 2", "lfc_mtg_2", KeyboardButtonStyle.PRIMARY, "6231187897916072711")
        ],
        [
            colored_button(" NON MTG", "lfc_mtg_0", KeyboardButtonStyle.DANGER, "6230840525256136996")
        ]
       ]
       markup = InlineKeyboardMarkup(buttons)
    ...
    elif state == STATE_LIVEFS_CHECKER_DATE:
       date_str_norm = normalize_fancy(text.strip())
       date_str_norm = re.sub(r'[—–−]', '-', date_str_norm)
       if not re.match(r'^\d{4}-\d{2}-\d{2}$', date_str_norm):
          ...
          return
       try:
          datetime.strptime(date_str_norm, "%Y-%m-%d")
       except:
          await update.message.reply_text("❌ Invalid date. Use YYYY-MM-DD")
          return
       context.user_data['lfc_date'] = date_str_norm
       await show_lfc_payout_filter(update, context)
       return
    ...
    # ---- PATTERN REPLAY (Strategy 4) states ----
        ...
        if re.match(r'^\d{2}:\d{2}$', text):
            ...
            msg = f"⏰ 𝙴𝚗𝚝𝚎𝚛 𝚎𝚗𝚍 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {format_utc_offset(get_tz_minutes(uid, 'otcfuture'))}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 16:30"
            entities = build_custom_emoji_entities(msg)
            await update.message.reply_text(msg, entities=entities)
        else:
    ...
    elif state == STATE_MAINTENANCE_MSG:
       if uid != OWNER_ID:
          await update.message.reply_text("⛔ Access denied.")
          context.user_data['state'] = None
          return
       context.user_data['maintenance_msg_draft'] = update.message.text
       context.user_data['state'] = STATE_MAINTENANCE_ETA
       await update.message.reply_text(
           "⏰ Ab estimated time likhein (jaise \"2 hours\" ya \"7:00 PM\") — ye users ko message ke sath dikhega.\n\n"
           "Type /cancel to abort."
       )
       return

    elif state == STATE_MAINTENANCE_ETA:
       if uid != OWNER_ID:
          await update.message.reply_text("⛔ Access denied.")
          context.user_data['state'] = None
          return
       MAINTENANCE_MODE["on"] = True
       MAINTENANCE_MODE["message"] = context.user_data.pop('maintenance_msg_draft', "")
       MAINTENANCE_MODE["eta"] = update.message.text
       _MAINTENANCE_LAST_NOTICE.clear()
       context.user_data['state'] = None
       await update.message.reply_text(
           f"✅ Maintenance mode ON.\n\n"
           f"📝 Message: {MAINTENANCE_MODE['message']}\n"
           f"⏰ ETA: {MAINTENANCE_MODE['eta']}\n\n"
           f"Authorized users ab kisi bhi button pe ye hi dekhenge. Turn off karne ke liye "
           f"dobara Maintenance button dabayein."
       )
       return
       ...
       await _show_broadcast_preview(update, context, msg_text, msg_entities)
       return

    elif state == STATE_BROADCAST_CONFIRM:
       if uid != OWNER_ID:
          return
       await update.message.reply_text("⬆️ Please tap ✅ Yes or ❌ No on the preview above to continue.")
       return
    ...
    if state == STATE_AUTO_LOGIN_EMAIL:
        ...
        try:
            def _do_login():
                ...
                c.connect()
                return c
            client = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, _do_login)
    ...
    # ---- Auto Trade per-strategy parameter states (mirror main-menu params) ----
       ...
       if re.match(r'^\d{2}:\d{2}$', text):
           ...
           msg = f"⏰ 𝙴𝚗𝚝𝚎𝚛 𝚎𝚗𝚍 𝚝𝚒𝚖𝚎 (𝙷𝙷:𝙼𝙼, {format_utc_offset(get_tz_minutes(uid, 'whiteout'))}):\n📝 𝙴𝚡𝚊𝚖𝚙𝚕𝚎: 16:30"
           entities = build_custom_emoji_entities(msg)
           await update.message.reply_text(msg, entities=entities)
       else:
           await update.message.reply_text("❌ Invalid format. Use HH:MM")
    ...
    # ---- Other existing states ----
    ...
    elif state == STATE_CHECKER_CUSTOM_DATE:
       date_str_norm = normalize_fancy(text.strip())
       date_str_norm = re.sub(r'[—–−]', '-', date_str_norm)
       try:
          picked_date = datetime.strptime(date_str_norm, "%Y-%m-%d").date()
       except Exception:
          await update.message.reply_text("❌ Invalid date format. Use YYYY-MM-DD, e.g. 2026-08-12")
          return
       ...
       if days_back < 1 or days_back > 25:
          await update.message.reply_text(
              f"❌ Date out of range.\n"
              f"📅 Allowed: 1–25 days back from today ({today_pk.strftime('%Y-%m-%d')}).\n"
              f"📝 Try again, e.g. {(today_pk - timedelta(days=3)).strftime('%Y-%m-%d')}"
          )
          return
    ...
    elif state == STATE_SMZX_NAME_TEMPLATE:
       new_name = text.strip()
       if not new_name:
          await update.message.reply_text("❌ Name cannot be empty. Please try again.")
          return
       get_state(uid).name_template = new_name
       msg = (
        f"✅ Name Template updated!\n\n"
        f"🎯 New Name: {new_name}\n\n"
        f"This will now appear on your OTC Live, AI OTC, Auto Signal, "
        f"Multi Engine and Forex Live signal & result messages."
       )
       buttons = [[colored_button(" Back", "smzx_settings_back", KeyboardButtonStyle.PRIMARY, "5152368980190560982")]]
       await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(buttons))
       context.user_data['state'] = None
       return
    ...
    # ===== Backtest 2.0 States =====
        ...
        broker = context.user_data.get('fut4_broker', 'quotex')
        pair_count = len(POCKET_OTC_PAIRS) if broker == "pocket" else len(TRADOWIX_OTC_PAIRS) if broker == "tradowix" else len(SMZ_ALL_PAIRS)
        # Pair selection buttons
        pair_msg = (
            "🥷 𝚂𝙼𝚉 𝙷𝙰𝙲𝙺𝙸𝙽𝙶 𝙼𝙾𝙳𝙴\n\n"
            "💎 𝚂𝚎𝚕𝚎𝚌𝚝 𝙿𝚊𝚒𝚛 𝙼𝚘𝚍𝚎:\n\n"
            f"🔹 𝙰𝚕𝚕 𝙿𝚊𝚒𝚛𝚜 – 𝚜𝚌𝚊𝚗 𝚊𝚕𝚕 {pair_count} 𝙾𝚃𝙲 𝚙𝚊𝚒𝚛𝚜\n"
            "🔹 𝙲𝚞𝚜𝚝𝚘𝚖 𝙿𝚊𝚒𝚛 – 𝚌𝚑𝚘𝚘𝚜𝚎 𝚜𝚙𝚎𝚌𝚒𝚏𝚒𝚌"
        )
        buttons = [
            [colored_button(f" All Pairs ({pair_count})", "smz_pair_all", KeyboardButtonStyle.SUCCESS, "6145553439809084250")],
        ...
        smz_broker = context.user_data.get('fut4_broker', 'quotex')
        smz_pair_pool = POCKET_OTC_PAIRS if smz_broker == "pocket" else TRADOWIX_OTC_PAIRS if smz_broker == "tradowix" else SMZ_ALL_PAIRS
        custom_list = [p.strip().upper() for p in text.split(',')]
        valid_pairs = []
        invalid = []
        for p in custom_list:
            if p in smz_pair_pool:
        ...
        threading.Thread(target=run_smz_hacking_mode, args=(uid, 2, start_time, end_time, "M1", valid_pairs, context.user_data.get('fut4_broker', 'quotex')), daemon=True).start()
        return
        ...
        # Was a blocking call sitting directly on MAIN_LOOP — for many pairs
        # fetched sequentially (up to 15-20s each) this froze the bot's
        # single event loop for ALL users, not just this one. Run it in a
        # background thread instead so it doesn't block anyone.
        sender.send_message(uid, "⏳ Generating future signals...")
        threading.Thread(
            target=_run_future_signals_bg,
            args=(uid, min_conf, start, end, pairs_list),
            daemon=True
        ).start()
        context.user_data['state'] = None
    ...
    elif state == STATE_TREND_FILTER_SETTINGS:
       raw_lines = [l.strip() for l in text.split('\n') if l.strip()]
       if not raw_lines:
          await update.message.reply_text("❌ No signals found. Send again or /cancel")
          return
       ...
       context.user_data['trend_signals_raw'] = parsed
       context.user_data['trend_skipped'] = skipped
       ...
       buttons = [
         [
            colored_button(" 20 ", "trend_ema_20", KeyboardButtonStyle.PRIMARY, "6145553439809084250"),
            colored_button(" 40 ", "trend_ema_40", KeyboardButtonStyle.PRIMARY, "6147654280112248427"),
            colored_button(" 100 ", "trend_ema_100", KeyboardButtonStyle.PRIMARY, "6145248943807667330"),
            colored_button(" 200 ", "trend_ema_200", KeyboardButtonStyle.PRIMARY, "5316681209026191987")
         ],
         [
            colored_button(" Custom ", "trend_ema_custom", KeyboardButtonStyle.SUCCESS, "5217822164362739968")
         ]
       ]
       markup = InlineKeyboardMarkup(buttons)
       bold_phrases = [f"{len(parsed)} signal(s) loaded"]
       if len(parsed) > 5:
          bold_phrases.append(f"... +{len(parsed)-5} more")
       bold_phrases.append("Select EMA Period:")
       entities = build_custom_emoji_entities(msg) + build_bold_entities(msg, bold_phrases)
       await update.message.reply_text(msg, entities=entities, reply_markup=markup)
       context.user_data['state'] = STATE_TREND_FILTER_EMA
       return

    elif state == STATE_TREND_FILTER_EMA:
       try:
          ema_period = int(clean_int_input(text))
          if ema_period < 5 or ema_period > 500:
              await update.message.reply_text("❌ Enter between 5-500:")
              return
          context.user_data['trend_ema'] = ema_period
        
          loading_msg = "⏳ Processing signals...\n🔄 0% [░░░░░░░░░░] 0/0"
          entities = build_custom_emoji_entities(loading_msg)
          await update.message.reply_text(loading_msg, entities=entities)
          ...
          threading.Thread(
              target=run_trend_filter,
              args=(update.effective_user.id, signals, mode, ema_period, context),
              kwargs={"broker": broker},
              daemon=True
          ).start()
          context.user_data['state'] = None
          ...
          await update.message.reply_text("❌ Invalid number. Enter EMA period (e.g., 50):")
          return
...
async def fut_pair_callback(
    ...
    if data == "pair_all":
        ...
        # Same fix as above — run off MAIN_LOOP so it can't freeze the bot
        # for every other user while this fetches/scans pairs.
        sender.send_message(uid, "⏳ Generating future signals...")
        threading.Thread(
            target=_run_future_signals_bg,
            args=(uid, min_conf, start, end, None),
            daemon=True
        ).start()
        context.user_data['state'] = None
...
def _build_smz_pair_page(page=0, per_page=15, selected=None, broker="quotex"):
    if selected is None:
        selected = set()
    pairs = POCKET_OTC_PAIRS if broker == "pocket" else TRADOWIX_OTC_PAIRS if broker == "tradowix" else SMZ_ALL_PAIRS
    total = len(pairs)
    start_idx = page * per_page
    end_idx = min(start_idx + per_page, total)
    ...
    row = []
    for pair in page_pairs:
        short = pair.replace("-OTC", "").replace("_otc", "")
        label = f"✅ {short}" if pair in selected else short
        style = KeyboardButtonStyle.SUCCESS if pair in selected else KeyboardButtonStyle.PRIMARY
...
def _build_fut4_pair_page(page=0, per_page=15, selected=None, market="otc", broker="quotex"):
    if selected is None:
        selected = set()
    pairs = fut4_pairs_for(broker, market)
    total = len(pairs)
    start_idx = page * per_page
    end_idx = min(start_idx + per_page, total)
    page_pairs = pairs[start_idx:end_idx]
...
async def show_fut4_pair_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    broker = context.user_data.get('fut4_broker', 'quotex')
    selected = context.user_data.get('fut4_selected_pairs', set())
    buttons, page, total_pages = _build_fut4_pair_page(page, selected=selected, market=market, broker=broker)
    selected_count = len(selected)
...
def generate_pattern_replay_signals(uid, context):
    ...
    try:
        ...
        broker = context.user_data.get('fut4_broker', 'quotex')
        broker_label = fut4_broker_label(broker)
        # User's chosen offset for the "OTC Future Signal" timezone setting —
        # was previously hardcoded to +5.
        user_tz_delta = get_tz_delta(uid, "otcfuture")

        # Master switch for the heavy diagnostic logging added while
        # tracking down the 18-Aug data-gap issue. Flip to True only when
        # actively debugging a "no signals" report — leave off otherwise
        # so normal runs stay quiet.
        _ST4_DEBUG = False

        if _ST4_DEBUG:
            print(f"[DBG-PARAMS] days={days} confidence={confidence} market={market} "
                  f"pairs={selected_pairs} start_time={start_time} end_time={end_time}")
        ...
        def update_progress(stage, current, total, pair="", signals_found=0, elapsed=0):
            ...
            text = (
                f"{emoji} {fancy_font('PATTERN REPLAY')} ({broker_label})\n"
                f"{fancy_font('━━━━━━━━━━━ • ━━━━━━━━━━━')}\n"
        ...
        def fetch_one_pair(pair):
            """Fetch full candle history for a pair from the broker's candle
            API — Quotex uses qxcandledata (beaxtapi), Pocket Option uses
            pocandledata (beaxtapi). Both mirror the same response shape.
            Pair format matches mrbeaxt's convention (reuse to_new_api_pair):
            OTC -> 'EURUSD_otc', Live -> 'EURUSD'.
            The API's 'epoch' is a true UTC unix timestamp; its own
            human-readable 'time' string is UTC+6. Pattern Replay's
            date/hhmm grouping uses the user's timezone setting
            (Settings > UTC Selection > OTC Future Signal — see
            'user_tz_delta' below), so we build time_str from
            epoch+offset ourselves instead of trusting the API's UTC+6
            string."""
            api_pair = to_new_api_pair(pair)
            max_retries = 3
            primary_url = f"{fut4_candle_api_base(broker)}?pair={api_pair}&timeframe=M1&count=14000"
            urls_to_try = [primary_url] * max_retries
            if broker not in ("pocket", "tradowix"):
                # Quotex: after the primary qxcandledata.beaxtapi.online
                # keeps failing, automatically shift to the mrbeaxt.com backup.
                fallback_url = f"{NEW_CANDLE_API_FALLBACK_BASE}?pair={api_pair}&timeframe=M1&count=14000"
                urls_to_try += [fallback_url] * max_retries
            print(f"[DEBUG] Fetching {pair} from {primary_url}")

            success = False
            resp = None
            for attempt, url in enumerate(urls_to_try):
                try:
                    # ~14k candles per pair, so timeout is generous.
                    ...
                    if resp.status_code == 200:
                        ...
                        print(f"⚠️ {pair} returned {resp.status_code}, retrying in {wait}s (attempt {attempt+1}/{len(urls_to_try)})")
                        ttime.sleep(wait)
                        continue
                    else:
                        print(f"❌ Failed to fetch {pair}: HTTP {resp.status_code} ({url})")
                        continue
                except Exception as e:
                    print(f"⚠️ Network error for {pair}: {e}, retrying (attempt {attempt+1}/{len(urls_to_try)})")
                    ttime.sleep(2 ** (attempt % max_retries))
                    continue

            if not success:
                print(f"❌ Giving up on {pair} after {len(urls_to_try)} attempts")
                return pair, None
            ...
            try:
                ...
                for c in candles:
                    ...
                    if epoch is None:
                        continue
                    dt_utc5 = datetime.fromtimestamp(
                        epoch, tz=timezone.utc) + user_tz_delta
                    o = float(c.get("open", 0))
                    cl = float(c.get("close", 0))
                    parsed.append({
                        "time_str": dt_utc5.strftime("%Y-%m-%d %H:%M:%S"),
                        "open": o,
                        "high": float(c.get("high", 0)),
                        "low": float(c.get("low", 0)),
                        "close": cl,
                ...
                # --- HEAVY DEBUG: raw vs parsed timestamps ---
                try:
                  if _ST4_DEBUG:
                    raw0 = candles[0]
                    rawN = candles[-1]
                    print(f"[DBG-RAW] {pair}: raw keys={list(raw0.keys())}")
                    print(f"[DBG-RAW] {pair}: first raw epoch={raw0.get('epoch')} "
                          f"raw_time_field={raw0.get('time')}")
                    print(f"[DBG-RAW] {pair}: last  raw epoch={rawN.get('epoch')} "
                          f"raw_time_field={rawN.get('time')}")
                    print(f"[DBG-PARSED] {pair}: first parsed time_str={parsed[0]['time_str']} "
                          f"(epoch={parsed[0]['timestamp']})")
                    print(f"[DBG-PARSED] {pair}: last  parsed time_str={parsed[-1]['time_str']} "
                          f"(epoch={parsed[-1]['timestamp']})")
                except Exception as _dbg_e:
                    print(f"[DBG-RAW] {pair}: debug print failed: {_dbg_e}")
                return pair, parsed
            except Exception as e:
        ...
        # ---- Analyze ----
        now_utc5 = datetime.now(timezone(user_tz_delta))
        yesterday_str = (now_utc5 - timedelta(days=1)).strftime("%Y-%m-%d")
        print(f"📅 Yesterday ({format_utc_offset(get_tz_minutes(uid, 'otcfuture'))}) = {yesterday_str}")
        ...
        for pair, candles in all_candles.items():
            ...
            # --- HEAVY DEBUG: full date-bucket breakdown for this pair ---
            _date_keys_sorted = sorted(by_date.keys())
            if _ST4_DEBUG:
                print(f"[DBG-DATES] {pair}: {len(_date_keys_sorted)} distinct dates -> "
                      f"{[(d, len(by_date[d])) for d in _date_keys_sorted]}")
                print(f"[DBG-DATES] {pair}: looking for yesterday_str={yesterday_str}")

            if yesterday_str not in by_date:
                print(f"⚠️ {pair}: no candles for {yesterday_str} "
                      f"(available dates: {_date_keys_sorted})")
                continue
            elif _ST4_DEBUG:
                print(f"[DBG-DATES] {pair}: yesterday bucket has "
                      f"{len(by_date[yesterday_str])} candles")
            ...
            if _ST4_DEBUG:
                print(f"[DBG-HIST] {pair}: used {count} historical day(s) "
                      f"(wanted days={days}) -> {len(historical_candles)} candles total")
            ...
            if _ST4_DEBUG:
                print(f"[DBG-YEST] {pair}: yest_candles count={len(yest_candles)} "
                      f"first={yest_candles[0]['time_str'] if yest_candles else None} "
                      f"last={yest_candles[-1]['time_str'] if yest_candles else None}")
            ...
            expected_minutes = (end_min - start_min + 1)
            if _ST4_DEBUG:
                covered = sum(1 for m in range(start_min, end_min + 1)
                              if f"{(m // 60):02d}:{(m % 60):02d}" in hhmm_to_idx)
                print(f"[DBG-COVERAGE] {pair}: window {start_time}-{end_time} "
                      f"expects {expected_minutes} minutes, hhmm_to_idx has "
                      f"{len(hhmm_to_idx)} total keys, {covered}/{expected_minutes} "
                      f"target minutes actually covered")
            ...
            for minute in range(start_min, end_min + 1):
                ...
                if _ST4_DEBUG:
                    _dists = [d for d, _ in top_k]
                    print(f"🔍 {pair} {target_time_str}: CALL={call_ratio:.1f}% PUT={put_ratio:.1f}% "
                          f"(threshold={confidence}%) top_k_dist_min={min(_dists):.5f} "
                          f"max={max(_dists):.5f}")
            ...
            if _ST4_DEBUG:
                print(f"[DBG-SUMMARY] {pair}: checked={checked} "
                      f"skip_idx_none={skip_idx_none} skip_idx_low={skip_idx_low} "
                      f"skip_pattern_none={skip_pattern_none}")
...
def generate_nightyy_signals(uid, context):
    ...
    # Convert user times to API times. The API's raw time convention is a
    # fixed UTC+6; how far the user's chosen otcfuture display offset sits
    # from that fixed point determines the shift (was a hardcoded 1h when
    # the display was always assumed to be UTC+5).
    _api_offset_min = 360  # API's raw time convention, fixed
    _shift = _api_offset_min - get_tz_minutes(uid, "otcfuture")
    ...
    tz_line = fancy_font(format_utc_offset(get_tz_minutes(uid, "otcfuture")) + " (PK)")
...
def get_pairs_by_market(market_type, broker="quotex"):
    """
    Return pairs based on market selection and broker.
    """
    if broker == "pocket":
        otc_pairs = POCKET_OTC_PAIRS.copy()
        forex_pairs = POCKET_LIVE_PAIRS.copy()
    elif broker == "tradowix":
        otc_pairs = TRADOWIX_OTC_PAIRS.copy()
        forex_pairs = TRADOWIX_LIVE_PAIRS.copy()
    else:
...
def start_auto_signal_session(uid, strategy_id, context):
    ...
    st.running = True
    st.session_token += 1
    my_session_token = st.session_token
    st.stop_requested = False
    st.paused = False
    ...
    threading.Thread(target=auto_signal_loop, args=(uid, strategy_id, context, my_session_token), daemon=True).start()
...
def send_result_message(st, sender, fmt, pair, entry_t, direction, payout, result_type,
                        ...
                        entry_candle=None, second_candle=None, strategy=1):
    ...
    if fmt == 3:
        ...
        # Format 1
        if result_type == "WIN":
            result_text = build_result_message_first_win(pair, entry_t, payout, wins, losses, owner_name=owner)
        elif result_type == "MTG WIN":
            result_text = build_result_message_second_win(pair, entry_t, payout, wins, losses, owner_name=owner)
        else:
            result_text = build_result_message_loss(pair, entry_t, payout, wins, losses, owner_name=owner)
        # Format 1 signal comes with a neon chart, so the result should
        # match it with a result chart (WIN/LOSS overlay) instead of text-only.
        chart_path = draw_result_chart(
            candles, pair, payout, result_type,
            entry_candle or {'open': open_price, 'close': close_price},
            second_candle,
...
def auto_signal_loop(uid, strategy_id, context, session_token=None):
    import time as t
    from datetime import timedelta

    st = get_state(uid)
    bot = SMZXBot(uid, broker=context.user_data.get('auto_signal_broker', 'quotex'), tz_feature="autosignal")
    bot.strategy = strategy_id

    # If no token was passed (older/other call sites), fall back to the
    # current one so the loop behaves as before.
    if session_token is None:
        session_token = st.session_token

    market = context.user_data.get('auto_signal_market', 'otc')
    pairs = get_pairs_by_market(market, broker=context.user_data.get('auto_signal_broker', 'quotex'))
    st.session_stats = {'wins': 0, 'losses': 0}
    st.signal_history = []
    st.running = True
    st.paused = False
    ...
    # Multi Engine's own "Change owner name" setting takes priority if the
    # user has set one; otherwise falls back to the global ⚙️ Settings →
    # 🎯 Name Template.
    owner = context.user_data.get('multi_owner_name') or get_name_template(uid)
    ...
    while st.running:
        try:
            if st.stop_requested:
                break
            # Stale-thread guard: if a NEWER session has started for this
            # uid since this thread began (session_token bumped), this
            # thread is a leftover from an old session — stop instead of
            # continuing to run/interfere alongside the new one.
            if st.session_token != session_token:
                print(f"[AUTO SIGNAL] Stale thread for uid={uid} (token {session_token} != {st.session_token}), exiting.")
                break
            if st.paused:
                t.sleep(1)
                continue
            ...
            for pair in pairs:
                ...
                candles, price, payout = fetch_candles_mrbeaxt(
                    pair, count=300, timeframe=tf,
                    broker=context.user_data.get('auto_signal_broker', 'quotex'))
                if not candles:
                    continue
                ...
                if bias:
                    ...
                    if tf == '5m':
                        ...
                        entry_dt = (now_pk + timedelta(minutes=1)).replace(second=0, microsecond=0)
                    entry_t = bot.display_time(entry_dt)
                    direction = "CALL" if bias == "CALL" else "PUT"
                    confidence = int(score) if score else 80
                    ...
                    # ─── Send Signal ──────────────────────────────
                    if fmt == 2:
                        signal_text = build_signal_format2(pair, entry_t, direction, tf=tf_display, owner_name=owner)
                        ...
                        signal_text = build_signal_format3(pair, entry_t, direction, payout,
                                                           ...
                                                           tz_label=format_utc_offset(get_tz_minutes(uid, "autosignal")))
                        if chart_path and os.path.exists(chart_path):
                    ...
                    candles2, first = bot.fetch_candle_at_time_with_retry(
                        pair, entry_dt, limit=50, attempts=25, delay=3, timeframe=tf)
                    if not candles2:
                        sender.send_message(
                            st.target_chat,
                    ...
                    if win1:
                        ...
                        record_trade_result(uid, True)
                        record_trade_hour(uid, bot.broker, True)
                        ...
                        send_result_message(st, sender, fmt, pair, entry_t, direction, payout, "WIN",
                                            ...
                                            entry_candle=first, strategy=strategy_id)
                        ...
                        candles3, second = bot.fetch_candle_at_time_with_retry(
                            pair, entry_dt + timedelta(minutes=tf_minutes), limit=50, attempts=25, delay=3, timeframe=tf)
                        if not candles3:
                            sender.send_message(
                                st.target_chat,
                        ...
                        if win2:
                            ...
                            send_result_message(st, sender, fmt, pair, entry_t, direction, payout, "MTG WIN",
                                                ...
                                                entry_candle=first, second_candle=second, strategy=strategy_id)
                        else:
                            st.session_stats['losses'] += 1
                            record_trade_result(uid, False)
                            record_trade_hour(uid, bot.broker, False)
            ...
            consecutive_flood_errors = 0  # a full iteration completed without raising — reset the streak
            ...
            err_str = str(_loop_err).lower()
            if "flood" in err_str or "retry after" in err_str or "retryafter" in err_str:
                # We're being flood-blocked by Telegram — hammering sendMessage
                # again in 3s is exactly what escalates a normal rate-limit
                # into an hours-long ban. Back off hard, and ONLY this
                # specific failure type counts toward the circuit breaker —
                # ordinary transient errors (network hiccup, momentary API
                # blip) keep retrying forever exactly like before, since
                # those are usually self-recovering and stopping on them was
                # cutting off most users' sessions over brief, harmless blips.
                consecutive_flood_errors += 1
                print(f"[AUTO SIGNAL LOOP] Flood control hit for uid={uid} — backing off 60s "
                      f"(consecutive_flood_errors={consecutive_flood_errors}).")
                t.sleep(60)
                if consecutive_flood_errors >= MAX_CONSECUTIVE_ERRORS:
                    print(f"[AUTO SIGNAL LOOP] uid={uid} hit {consecutive_flood_errors} consecutive "
                          f"flood-control errors — stopping session instead of retrying forever.")
                    st.stop_requested = True
                    try:
                        sender.send_message(
                            uid,
                            f"🔴 Auto signal stopped — Telegram is rate-limiting messages to you. "
                            f"Please restart once that clears.")
                    except Exception:
                        pass
                    break
            else:
    ...
    # Only clear shared state if THIS thread's session is still the current
    # one — a stale thread that got superseded by a newer session must not
    # reset st.running/target_chat and kill the newer session's run.
    if st.session_token == session_token:
        st.running = False
        st.target_chat = None
...
async def auto_control_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "autopause":
        ...
        await safe_query_answer(query, "Partial results sent to channel.")
...
async def smz_pair_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "smz_pair_all":
        broker = context.user_data.get('fut4_broker', 'quotex')
        all_pairs = POCKET_OTC_PAIRS if broker == "pocket" else TRADOWIX_OTC_PAIRS if broker == "tradowix" else SMZ_ALL_PAIRS
        await query.edit_message_text(f"⏳ Processing {len(all_pairs)} pairs...\n🕒 {start_time} - {end_time}")
        threading.Thread(target=run_smz_hacking_mode, args=(uid, 2, start_time, end_time, "M1", all_pairs, broker), daemon=True).start()
        context.user_data['smz_step'] = None
        return
        ...
        buttons, page, total_pages = _build_smz_pair_page(0, selected=set(), broker=broker)
        selected_count = 0
        msg = f"🎯 Select pairs (Page 1/{total_pages}):\n\nTap pairs to select/deselect, then press Done\nSelected: {selected_count} pairs"
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(buttons))
        return
    ...
    # Handle pagination navigation
        ...
        broker = context.user_data.get('fut4_broker', 'quotex')
        buttons, page, total_pages = _build_smz_pair_page(page, selected=selected, broker=broker)
        selected_count = len(selected)
        msg = f"🎯 Select pairs (Page {page+1}/{total_pages}):\n\nTap pairs to select/deselect, then press Done\nSelected: {selected_count} pairs"
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(buttons))
        return
    ...
    # Done button
        ...
        threading.Thread(target=run_smz_hacking_mode, args=(uid, 2, start_time, end_time, "M1", pairs_list, broker), daemon=True).start()
        return
...
def _build_chart_analysis_prompt(tf: str = "1M") -> str:
    """Same analysis prompt, worded for whichever timeframe the user picked
    (1-minute vs 5-minute next-candle signal)."""
    next_label = "5-minute" if tf == "5M" else "1-minute"
    return (
        "You are an elite binary options analyst with 15+ years experience on Quotex. "
        f"Analyze this chart screenshot for the NEXT {next_label} candle direction.\n\n"
...
CHART_ANALYSIS_PROMPT = _build_chart_analysis_prompt("1M")
...
def _gemini_analyze_chart(image_bytes: bytes, tf: str = "1M") -> dict:
    ...
    prompt = _build_chart_analysis_prompt(tf)
    headers = {
...
def _build_chart_analyzer_msg(result: dict, tf: str = "1M") -> str:
    ...
    dir_emoji = "📉" if direction == "CALL" else "📈"
    step_minutes = 5 if tf == "5M" else 1
    now_utc5 = datetime.now(timezone.utc) + timedelta(hours=5)
    if tf == "5M":
        # Next 5-minute candle boundary (:00, :05, :10, ...)
        base = now_utc5.replace(second=0, microsecond=0)
        minutes_to_next = step_minutes - (base.minute % step_minutes)
        entry_dt = base + timedelta(minutes=minutes_to_next)
    else:
        entry_dt = now_utc5.replace(second=0, microsecond=0) + timedelta(minutes=1)
    entry_time = entry_dt.strftime("%H:%M")
    conf_bar = "█" * (confidence // 10) + "░" * (10 - confidence // 10)
    tf_label = "5-min" if tf == "5M" else "1-min"
    msg = (
        f"❀° ┄────────=─────────╮\n"
        f"   📸 𝙲𝙷𝙰𝚁𝚃 𝙰𝙽𝙰𝙻𝚈𝚉𝙴𝚁 ({tf_label}) 📸\n"
        f"╰────────=───=─────┄ °❀\n"
        f"┏───♡─────────── ⊹˚───┓\n"
        f"📊 Pair∶— {fancy_font(pair)}\n"
        f"{dir_emoji} Direction∶— {fancy_font(direction)}\n"
        ...
        f"🤖 𝚁𝚎𝚊𝚜𝚘𝚗\n"
        f"💪 {reason}\n\n"
        f"⚠️ Trade on next {tf_label} candle at {fancy_font(entry_time)}\n"
        f"✨ ©OWNER @Rohailtrader ✨"
    )
    return msg
...
async def chart_analyzer_photo_handler(
        update: Update,
        context: ContextTypes.DEFAULT_TYPE):
    """Handle photo messages: broadcasting a picture (owner, when in
    STATE_BROADCAST_WAIT_FOR_MESSAGE) or Chart Analyzer mode."""
    if not update.effective_user:
        return
    uid = update.effective_user.id
    if not await is_authorized_async(uid):
    ...
    if state == STATE_BROADCAST_WAIT_FOR_MESSAGE:
        if uid != OWNER_ID:
            await update.message.reply_text("⛔ Access denied.")
            context.user_data['state'] = None
            return
        ...
        await _show_broadcast_preview(update, context, caption_text, caption_entities, photo_file_id=photo_file_id)
        return
    ...
    tf = context.user_data.get('chart_analyzer_tf', '1M')
    ...
    try:
        ...
        # _gemini_analyze_chart() is a blocking requests.post() to the
        # Vision API — up to 60s timeout, with retries across multiple
        # models, plus a time.sleep() on rate-limit (429). Calling it
        # directly here (this handler is async, running on MAIN_LOOP) froze
        # the ENTIRE bot for every user for the whole 10-20s+ this takes,
        # every single time someone submitted a chart screenshot. This was
        # by far the longest and most frequent MAIN_LOOP block in the bot.
        result = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, _gemini_analyze_chart, bytes(photo_bytes), tf)
        analysis_msg = _build_chart_analyzer_msg(result, tf)
...
async def future_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_authorized_async(uid):
        ...
        return
    context.user_data['strategy_active'] = False
    context.user_data['fut4_broker'] = 'quotex'  # /future has no broker screen — always Quotex
    fut_text = (
        "🔥 𝙵𝚄𝚃𝚄𝚁𝙴 𝚂𝙸𝙶𝙽𝙰𝙻𝚂\n\n"
        "📊 Select Strategy:\n\n"
        "🚀 Strategy 1 – SMZ Future OTC\n"
...
async def _on_bot_startup(application):
    global MAIN_LOOP, BOT_INSTANCE, BOT_APP
    MAIN_LOOP = asyncio.get_running_loop()
    BOT_INSTANCE = application.bot
    BOT_APP = application

    # Explicit fresh-boot wipe: every redeploy/restart, force EVERY user's
    # in-memory state clean, so nobody can carry a stuck/stale session
    # across a deploy. user_states/context.user_data are plain in-memory
    # dicts with no disk persistence configured, so this is normally already
    # empty on a fresh process — this just makes that guarantee explicit and
    # future-proof (e.g. if persistence ever gets added later, or a platform
    # does something unusual on redeploy) instead of relying on it silently.
    wiped_states = len(user_states)
    user_states.clear()
    _auth_cache.clear()
    _last_known_auth.clear()
    try:
        for uid in list(application.user_data.keys()):
            try:
                ...
                try:
                    application.user_data[uid].clear()
                except Exception:
                    pass
    except Exception as _wipe_err:
        print(f"[STARTUP WIPE] user_data clear error: {_wipe_err}")
    print(f"{Fore.YELLOW}[✓] Fresh boot: cleared {wiped_states} cached user session(s) — every user starts clean.{Style.RESET_ALL}")

    print(f"{Fore.GREEN}[✓] Event loop ready (MAIN_LOOP bound){Style.RESET_ALL}")


async def _on_bot_error(update, context):
    err = context.error
    err_type = type(err).__name__
    err_str = str(err).lower()
    print(f"[HANDLER ERROR] {err_type}: {err}")
    
    # Ignore harmless errors – no user notification needed
    harmless_patterns = (
        "query is too old",
        "message is not modified",
        "message to edit not found",
        ...
        "retryafter",
        "attributeerror",
        "userstate' object has no attribute '_thread'",  # specific to our reset
        "forbidden",
        "user is deactivated",
        "flood control",  # RetryAfter's actual message text, e.g. "Flood
        # control exceeded. Retry in 49830 seconds" — does NOT contain the
        # literal word "retryafter", so the old check below never matched
        # it. That meant every RetryAfter exception fell through to
        # context.bot.send_message() as an "unexpected error" notice, which
        # itself got flood-blocked too — spending an extra API call per
        # user, per error, while the bot was already rate-limited.
    )
    for h in harmless_patterns:
        if h in err_str:
            print(f"[IGNORED] {h} – no user notification sent.")
            return
    if err_type == "RetryAfter":
        print(f"[IGNORED] RetryAfter – no user notification sent.")
        return
    
    # For other errors, send a cooldown-limited message
    try:
        chat = update.effective_chat if update else None
        if chat is not None:
            uid = chat.id
            # Cooldown: don't send more than 1 error per 60 seconds per user
            last_error_time = context.user_data.get('last_error_time', 0)
            if time.time() - last_error_time < 60:
                return
            context.user_data['last_error_time'] = time.time()
            
            await context.bot.send_message(
                chat_id=chat.id,
                text="⚠️ An unexpected error occurred. Please try again. If this persists, use /reset and /start.",
            )
    except Exception as e:
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(_maintenance_gate_callback, pattern=".*"), group=-1)
    app.add_handler(CallbackQueryHandler(news_callback, pattern="^news_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^menu_|^fut5_|^trend_|^fut5$"))
    ...
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^checkerbroker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^otcbroker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^aiotcbroker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^livefsbroker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^livefsmain_broker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^trendbroker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^asigbroker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^btbroker_"))
    ...
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^futbroker_"))
    ...
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^smzx_settings_"))
    ...
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^multibroker_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^admin_broadcast$"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^admin_maintenance_toggle$"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^leaderboard_refresh$"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^hours_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^chart_analyzer_tf_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^broadcast_send_yes$|^broadcast_send_no$"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^finder_"))
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^finderbroker_"))
    app.add_handler(CallbackQueryHandler(timezone_settings_callback, pattern="^tzpick_|^tzset_|^tzback_settings$"))
