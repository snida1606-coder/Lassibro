...
def run_ai_filter_pattern_match(uid, signals_text, threshold, context):
    ...
    MIN_MATCHES = 4                  # minimum non-overlapping historical matches required
    ...
    def find_non_overlapping_matches(candles, current_pattern, pattern_start, plen):
        ...
        non-overlapping ones (gap >= plen from every already-chosen match) so the
        same real-world event isn't counted multiple times."""
        similarities = []
        ...
        for dist, i in similarities:
            if len(chosen) >= MAX_MATCHES:
                break
            if all(abs(i - c_i) >= plen for _, c_i in chosen):
