...
# ================================================================
#   NEW CANDLE DATA API (mrbeaxt.com) — replaces gm-vlni.onrender.com
#   Differences vs the old API, handled here so nothing downstream
#   has to change:
#     1) mrbeaxt.com returns candles NEWEST-FIRST (index 0 = latest).
#        We reverse it -> oldest-first, so candles[-1] is still "latest"
#        exactly like every existing function in this file expects.
#     2) mrbeaxt.com's 'epoch' field is a normal UTC unix timestamp
...
# ================================================================
NEW_CANDLE_API_BASE = "https://mrbeaxt.com/Qxapi/qx.php"
# Quotex live-tick endpoint on the same mrbeaxt.com API, used everywhere
...
# low,close,payout,...}]} response shape as mrbeaxt.com (newest-first), so
# it's a drop-in fallback. Every Quotex candle-fetch site in this file
# automatically shifts to this URL whenever the primary mrbeaxt.com API
# above is unreachable or returns no usable data.
NEW_CANDLE_API_FALLBACK_BASE = "https://qxcandledata.beaxtapi.online/api/v1/candles"
...
def fetch_new_api_candles(pair: str, count: int = 1500, timeframe="M1",
    ...
    if broker == "pocket":
        ...
        # Quotex: interleave mrbeaxt.com (primary) with the
        # qxcandledata.beaxtapi.online backup — primary, fallback, primary,
...
def fetch_beaxt_candles(pair: str, count: int = 2000,
    ...
    if broker == "pocket":
        ...
        # Quotex: interleave mrbeaxt.com (primary) with the
        # qxcandledata.beaxtapi.online backup instead of exhausting primary
...
async def run_backtest_local(
    ...
    for sig in signals:
        ...
        if broker == "pocket":
            ...
            # Quotex: mrbeaxt.com first, then automatically shift to the
            # qxcandledata.beaxtapi.online backup if it fails.
            urls_to_try = [
...
def _run_livefs_worker(uid: int, context):
    ...
    for idx, pair in enumerate(pairs, 1):
        ...
        if broker == "pocket":
            ...
            # Quotex: mrbeaxt.com first, then automatically shift to the
            # qxcandledata.beaxtapi.online backup if it keeps failing.
    ...
    try:
        ...
        def fetch_one_pair(pair):
            ...
            if broker not in ("pocket", "tradowix"):
                # Quotex: after mrbeaxt.com keeps failing, automatically
                # shift to the qxcandledata.beaxtapi.online backup.
