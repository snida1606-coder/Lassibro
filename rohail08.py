...
def fetch_new_api_candles(pair: str, count: int = 1500, timeframe="M1",
    ...
    if broker == "pocket":
        ...
        # Quotex: try mrbeaxt.com for `retries` attempts, then automatically
        # shift to the qxcandledata.beaxtapi.online backup for `retries`
        ...
        urls_to_try = [primary_url] * retries + [fallback_url] * retries
...
async def send_broadcast_with_retry(context, user_ids, text, entities=None, max_retries=3, concurrency=5):
    """
    Send broadcast with:
    - Concurrency limit (5 messages at a time)
    - Retry on failure with exponential backoff
    - Skip users who have blocked the bot (Forbidden)
    """
    import asyncio
    ...
    sem = Semaphore(concurrency)  # Max 5 concurrent sends
    success_count = 0
    failed_count = 0
        ...
        async with sem:
            for attempt in range(max_retries):
                try:
                    ...
                    err_str = str(e).lower()
                    # If user blocked bot, don't retry
                    if "forbidden" in err_str or "user is deactivated" in err_str or "bot was blocked" in err_str:
                        failed_count += 1
                        return
                    ...
                    if "retry after" in err_str:
                        try:
                            ...
                            wait = min(wait, 5)  # Cap at 5 seconds
                        except:
...
TZ_FEATURES = [
    ("checker",         "Signal Checkers Timezone", 360),   # UTC +06:00
    ("otcfuture",       "OTC Future Signal",        300),   # UTC +05:00
    ("otclive",         "OTC LIVE MODE",             360),   # UTC +06:00
    ("multiengine",     "MULTI ENGINE",              300),   # UTC +05:00
    ("blackout",        "Blackout Future",           360),   # UTC +06:00
    ("whiteout",        "Whiteout Future",           360),   # UTC +06:00
    ("backtest",        "Backtest",                  360),   # UTC +06:00
    ("aiotc",           "AI OTC",                    360),   # UTC +06:00
    ("stratfinder",     "Strategy finder",           330),   # UTC +05:30
    ("autosignal",      "Auto signal",              -180),   # UTC -03:00
]
...
def build_tz_settings_keyboard() -> "InlineKeyboardMarkup":
    ...
    rows.append([colored_button(" Back", "back_to_main", KeyboardButtonStyle.DANGER)])
...
class SMZXBot:
    def __init__(self, uid, broker="quotex"):
        self.uid = uid
        st = get_state(uid)
...
def run_trend_filter(uid, signals, mode, ema_period, context, broker="quotex"):
    ...
    for pair, tm, direction in signals:
        ...
        try:
            ...
            entry_dt = entry_dt.replace(tzinfo=timezone(timedelta(hours=5)))
            target_ts = int((entry_dt - timedelta(hours=5)).timestamp())
        except Exception:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── Row 16: Settings ──────────────────────────────
    buttons.append([
        colored_button("  ⚙️ Settings  ", "menu_settings", KeyboardButtonStyle.PRIMARY),
    ])
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_settings":
        await show_timezone_settings(update, context, edit=False)
        return
    ...
    if data == "auto_trade_start":
       ...
       # Default UTC +5 for Pakistan
       if 'lfc_utc' not in context.user_data:
          context.user_data['lfc_utc'] = 5
    ...
    # ==================== MULTI ENGINE ====================
       ...
       return
        ...
        if 'bl_checker_utc' not in context.user_data:
            context.user_data['bl_checker_utc'] = 5
...
async def global_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- PATTERN REPLAY (Strategy 4) states ----
       ...
       total = len(user_ids)
       progress_msg = await update.message.reply_text(f"📡 Sending broadcast to {total} users...\n⏳ Please wait.")
    
       # Send broadcast with retry and rate limiting
       success, failed = await send_broadcast_with_retry(
        context, user_ids, msg_text, msg_entities
       )
...
def generate_pattern_replay_signals(uid, context):
    ...
    try:
        ...
        def fetch_one_pair(pair):
            ...
            date/hhmm grouping assumes the user's timezone (UTC+5,
            Pakistan — see 'now_utc5' below and the +5:00 header), so we
            build time_str from epoch+5h ourselves instead of trusting
            the API's UTC+6 string."""
            api_pair = to_new_api_pair(pair)
            max_retries = 3
...
def auto_signal_loop(uid, strategy_id, context, session_token=None):
    ...
    st = get_state(uid)
    bot = SMZXBot(uid, broker=context.user_data.get('auto_signal_broker', 'quotex'))
