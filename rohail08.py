...
_NETWORK_EXECUTOR = _cf.ThreadPoolExecutor(
    max_workers=150, thread_name_prefix="net-io")

# =====================================================================
#   BOUNDED TASK EXECUTOR + STUCK-TASK WATCHDOG + RETRY SUPPORT
# ---------------------------------------------------------------------
#   ROOT CAUSE FIXED HERE: every button/click used to do a raw
#   `threading.Thread(..., daemon=True).start()` with NO limit. At
#   50-100 concurrent users that spawns hundreds of uncontrolled OS
#   threads at once, which starves/stalls most of them silently (the
#   user's button "answers" on Telegram but nothing ever comes back).
#
#   Fix: one bounded pool for everything (TASK_EXECUTOR), every task
#   is tracked per-uid, a watchdog force-recovers anyone stuck past a
#   timeout, and any failure notifies the user with a Retry button
#   instead of leaving them stuck forever.
# =====================================================================
TASK_EXECUTOR = _cf.ThreadPoolExecutor(max_workers=250, thread_name_prefix="task")

_active_tasks = {}            # uid -> {future,label,start,timeout,_tag}
_active_tasks_lock = threading.Lock()

_retry_registry = {}          # retry_id -> (func,args,kwargs,uid,label,timeout)
_retry_registry_lock = threading.Lock()

DEFAULT_TASK_TIMEOUT = 120    # seconds; pass timeout=... to spawn_task for longer jobs


def _register_retry(func, args, kwargs, uid, label, timeout):
    rid = uuid.uuid4().hex[:10]
    with _retry_registry_lock:
        _retry_registry[rid] = (func, args, kwargs, uid, label, timeout)
        if len(_retry_registry) > 2000:
            for k in list(_retry_registry.keys())[:500]:
                _retry_registry.pop(k, None)
    return rid


def _retry_button(rid):
    return {"inline_keyboard": [[{"text": "🔁 Retry", "callback_data": f"retry_task:{rid}"}]]}


def _task_runner(func, args, kwargs, uid, label):
    try:
        func(*args, **kwargs)
    except Exception as e:
        print(f"[TASK ERROR] uid={uid} label={label}: {type(e).__name__}: {e}")
        if uid is not None:
            try:
                rid = _register_retry(func, args, kwargs, uid, label, DEFAULT_TASK_TIMEOUT)
                sender.send_message(
                    uid,
                    f"⚠️ {label or 'Action'} fail ho gaya ({type(e).__name__}). Dobara try karein.",
                    buttons=_retry_button(rid),
                )
            except Exception as e2:
                print(f"[TASK ERROR NOTIFY FAIL] uid={uid}: {e2}")
    finally:
        if uid is not None:
            with _active_tasks_lock:
                info = _active_tasks.get(uid)
                if info and info.get("_tag") == (func, id(args)):
                    _active_tasks.pop(uid, None)


def spawn_task(func, *args, uid=None, label=None, timeout=DEFAULT_TASK_TIMEOUT, **kwargs):
    """Bounded, tracked, retry-capable replacement for
    `threading.Thread(target=func, args=args, daemon=True).start()`."""
    label = label or getattr(func, "__name__", "task")
    future = TASK_EXECUTOR.submit(_task_runner, func, args, kwargs, uid, label)
    if uid is not None:
        with _active_tasks_lock:
            _active_tasks[uid] = {
                "future": future,
...
def spawn_bg_task(func, *args, **kwargs):
    """For system/background threads (no per-user tracking needed) that
    still shouldn't be able to raise unnoticed or bypass the pool."""
    def _run():
        try:
            func(*args, **kwargs)
        except Exception as e:
            print(f"[BG TASK ERROR] {getattr(func, '__name__', func)}: {type(e).__name__}: {e}")
    return TASK_EXECUTOR.submit(_run)


def _stuck_task_watchdog(check_interval=20):
    """Runs forever. Python can't force-kill a raw thread, but it CAN
    unstick the *user*: if a tracked task has run past its timeout,
    clear their running/state flags and prompt a retry, so a hung
    backend task never leaves someone permanently stuck in the UI."""
    while True:
        ...
        try:
            ...
            stuck = []
            with _active_tasks_lock:
                for uid, info in list(_active_tasks.items()):
                    if now - info["start"] > info["timeout"] and not info["future"].done():
                        stuck.append((uid, info))
                        _active_tasks.pop(uid, None)
            for uid, info in stuck:
                try:
                    st = get_state(uid)
                    st.running = False
                ...
                try:
                    sender.send_message(
                        uid,
                        f"⚠️ '{info['label']}' bohot der se atka hua tha, reset kar diya.\nDobara try karein 🔁",
                    )
                except Exception as e:
                    print(f"[WATCHDOG NOTIFY FAIL] uid={uid}: {e}")
                print(f"[WATCHDOG] uid={uid} label={info['label']} stuck >{info['timeout']}s, force-reset.")
        except Exception as e:
            print(f"[WATCHDOG LOOP ERROR] {e}")


async def _retry_task_callback(update, context):
    query = update.callback_query
    await safe_query_answer(query)
    data = query.data or ""
    rid = data.split(":", 1)[1] if ":" in data else None
    entry = None
    if rid:
        with _retry_registry_lock:
            entry = _retry_registry.pop(rid, None)
    if not entry:
        await query.edit_message_text("⏳ Retry expired ya already run ho chuka, /start se dobara try karein.")
        return
    func, args, kwargs, uid, label, timeout = entry
    await query.edit_message_text(f"🔁 Retrying: {label}...")
    spawn_task(func, *args, uid=uid, label=label, timeout=timeout, **kwargs)
...
spawn_bg_task(run_uptime_server)
# =============================================
...
# ================================================================
NEW_CANDLE_API_BASE = "https://qxcandledata.beaxtapi.online/candles"
...
def safe_draw_pro_chart(*args, **kwargs):
    ...
    try:
        ...
        # way to the user.
        spawn_bg_task(_release_chart_memory)
...
async def run_backtest_local(
    ...
    for sig in signals:
        ...
        # Need enough 1-minute candles to actually cover the selected
        # 'days' range. This used to be a fixed count=14000 (~9.7 days
        # worth of M1 candles) no matter what the user picked — so
        # selecting 20 days silently only ever tested against the last
        # ~9-10 days of data. Scale the fetch to the actual days
        # requested (+2 day buffer), capped at a sane upper bound.
        candle_count = min((days + 2) * 1440, 40000)
        url = f"https://qxcandledata.beaxtapi.online/candles?pair={api}&timeframe=M1&count={candle_count}"
        try:
...
async def start_multi_nonstop(update: Update, context: ContextTypes.DEFAULT_TYPE, pairs: list):
    ...
    spawn_task(multi_nonstop_loop, uid, context, multi_session_token, uid=uid, label="multi_nonstop_loop", timeout=21600)
    # (folded into spawn_task above)
...
def multi_nonstop_loop(uid: int, context: ContextTypes.DEFAULT_TYPE, session_token=None):
    ...
    while context.user_data.get('multi_running', False):
        ...
        try:
            # Stale-thread guard: if a newer session started for this uid
            # (token bumped) since this thread began, stop instead of
            # continuing to run alongside/interfering with the new one.
            ...
            candles, price, payout = fetch_candles_mrbeaxt(pair, count=2000)
            time.sleep(0.2)
    ...
    # Loop ended — only announce "stopped" if this was actually the current
    # session (not a stale thread superseded by a newer one, which would
    # otherwise send a confusing "stopped" message right after a fresh start).
    if context.user_data.get('multi_session_token', 0) == session_token:
...
async def s6_candles_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if 10 <= val <= 200:
            ...
            spawn_task(bot.run_single_signal, uid=bot.uid, label="run_single_signal")
...
def run_livefs_checker(uid: int, date_str: str, mtg_level: int, signals_text: str, context):
    ...
    FIX: previously this hit the sio.tools backend (_sio_checker_request /
    _sio_checker_poll) — a slow, third-party, rate-limited endpoint that was
    unrelated to the rest of the bot's data source. It now uses the exact
    same candle backend and fetch pattern as Run Advance Checker
    (fetch_beaxt_candles -> qxcandledata.beaxtapi.online, parallel
    ThreadPoolExecutor(max_workers=10), same fetch_count scaling based on
    how many days back the date is), so speed and parsing behavior match
    pixel-to-pixel.

    The only real difference from Advance Checker: Live FS pairs are plain
    Forex/live pairs (e.g. "EURUSD"), never OTC, so we do NOT force an
    "_otc" suffix onto the API pair — to_new_api_pair() only appends
    "_otc" when the pair name itself contains "OTC", so passing the plain
    pair straight through already gives the correct live API format.
    """
    from datetime import datetime, timedelta, timezone
    import re
    from concurrent.futures import ThreadPoolExecutor, as_completed
    ...
    # Only fetch pairs that actually have a non-future signal to check
    unique_pairs = list({sig['pair'] for sig in signals if not sig['future']})

    def fetch_pair(pair):
        # api pair is the plain pair itself (e.g. "EURUSD") — no "_otc"
        # suffix, since Live FS pairs are real Forex pairs, not OTC.
    ...
    for sig in signals:
        ...
        if not candle_dict:
            final_display.append((f"M1 {pair} {time_str} {direction}", "❌"))
            losses += 1
            print(f"   {pair} {time_str} → ❌ (no candle data)")
            continue
        ...
        if not entry:
            final_display.append((f"M1 {pair} {time_str} {direction}", "❌"))
            losses += 1
            print(f"   {pair} {time_str} → ❌ (candle not found)")
            continue
        ...
        if win:
            final_display.append((f"M1 {pair} {time_str} {direction}", "✅"))
            wins += 1
            print(f"   {pair} {time_str} → ✅ WIN")
            continue

        # MTG recovery (same recovery logic as Run Advance Checker)
        recovered = False
        if mtg_level > 0:
            for step in range(1, mtg_level + 1):
                h, m = map(int, time_str.split(':'))
                ...
                if next_candle:
                    ...
                    if (direction == "CALL" and next_dir == "up") or (direction == "PUT" and next_dir == "down"):
                        ...
                        wins += 1
                        recovered = True
                        print(f"   {pair} {time_str} → {mtg_icon} (recovered at step {step})")
                        break
        if not recovered:
            final_display.append((f"M1 {pair} {time_str} {direction}", "❌"))
            losses += 1
            print(f"   {pair} {time_str} → ❌ LOSS")
...
async def news_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "news_generate":
        ...
        spawn_task(_run_news_generate, uid, snap, uid=uid, label="news_generate")
        return
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    spawn_bg_task(log_user_to_all_users, update)
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # -------- FIX: livefs_pair_done handled BEFORE startswith --------
       ...
       spawn_task(run_strategy_finder, uid, context, uid=uid, label="strategy_finder")
       return
    ...
    # ==================== MULTI ENGINE ====================
       ...
       spawn_task(run_multi_manual_analysis, uid, pair, context, uid=uid, label="multi_manual_analysis")
       return
       ...
       return
        ...
        spawn_task(run_blackout_checker_worker, query.from_user.id, date_str, signals, mtg_level, context, payout_filter, uid=query.from_user.id, label="blackout_checker")
        context.user_data['state'] = None
        return
       ...
       await query.message.reply_text(msg, entities=entities, reply_markup=markup)
        ...
        spawn_task(run_pair_payout, uid, context, uid=uid, label="pair_payout")
        return

    elif data == "menu_market_trend":
        spawn_task(run_market_trend, uid, context, uid=uid, label="market_trend")
        return

    elif data == "menu_candle_colors":
        spawn_task(run_candle_colors, uid, context, uid=uid, label="candle_colors")
        return
...
async def generate_alcohol_signals_wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    spawn_task(target, uid=uid, label="alcohol_signals")
...
async def continue_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if st.running:
        ...
        return
    if st.ai_mode:
        spawn_task(run_ai_mode, uid, uid=uid, label="ai_mode")
        try:
...
async def auto_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trader._thread = spawn_task(
        auto_trade_loop, trader, context,
        uid=trader.uid, label="auto_trade_loop", timeout=21600)
...
async def global_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- AI Filter States ----
        ...
        if context.user_data.get('waiting_for_strat_list'):
            ...
            entities = build_custom_emoji_entities(msg)
            await update.message.reply_text(msg, entities=entities)
            spawn_task(run_ai_mode, uid2, uid=uid2, label="ai_mode")
            context.user_data['state'] = None
    ...
    # ===== LIVE FS STATES (NEW) =====
        ...
        if re.match(r'^\d{2}:\d{2}$', text):
            ...
            spawn_task(_run_livefs_worker, update.effective_user.id, context, uid=update.effective_user.id, label="livefs_worker")
            context.user_data['state'] = None
        else:
            await update.message.reply_text("❌ Invalid format. Use HH:MM")
        return
        ...
        if not date_str:
            ...
            return
        context.user_data['state'] = None
        spawn_task(run_livefs_checker, uid, date_str, mtg, signals_text, context, uid=uid, label="livefs_checker")
    ...
    # ---- PATTERN REPLAY (Strategy 4) states ----
        ...
        if re.match(r'^\d{2}:\d{2}$', text):
            ...
            spawn_task(generate_pattern_replay_signals, uid, context, uid=uid, label="pattern_replay")
            context.user_data['state'] = None
        else:
        ...
        if re.match(r'^\d{2}:\d{2}$', text):
           ...
           spawn_task(generate_nightyy_signals, update.effective_user.id, context, uid=update.effective_user.id, label="nightyy_signals")
           context.user_data['state'] = None
        else:
           await update.message.reply_text("❌ 𝙸𝚗𝚟𝚊𝚕𝚒𝚍 𝚏𝚘𝚛𝚖𝚊𝚝. 𝚄𝚜𝚎 HH:MM")
        return
    ...
    # ---- Auto Trade per-strategy parameter states (mirror main-menu params) ----
       ...
       if re.match(r'^\d{2}:\d{2}$', text):
          ...
          spawn_task(run_whiteout_signals, uid, context, uid=uid, label="whiteout_signals")
          context.user_data['state'] = None
       else:
          await update.message.reply_text("❌ Invalid format. Use HH:MM")
       return
    ...
    # ===== Checker 2.0 States =====
        ...
        context.user_data['state'] = None
        spawn_task(_run_sio_checker_thread, uid, context, text, uid=uid, label="sio_checker")
        return
    ...
    # ===== Backtest 2.0 States =====
        ...
        spawn_task(_run_sio_backtest_thread, uid, context, text, uid=uid, label="sio_backtest")
        return
        ...
        spawn_task(run_smz_hacking_mode, uid, 2, start_time, end_time, "M1", valid_pairs, uid=uid, label="smz_hacking_mode", timeout=600)
        return
        ...
        spawn_task(_run_future_signals_bg, uid, min_conf, start, end, pairs_list,
                   uid=uid, label="future_signals")
        context.user_data['state'] = None
        ...
        try:
            ...
            if 1 <= days <= 25:
                ...
                await update.message.reply_text("❌ Please enter a number between 1 and 25.")
                return STATE_BACKTEST_CUSTOM_DAYS
        except BaseException:
            await update.message.reply_text("❌ Invalid number. Enter days (1-25):")
            return STATE_BACKTEST_CUSTOM_DAYS
...
async def fut_pair_callback(
    ...
    if data == "pair_all":
        ...
        sender.send_message(uid, "⏳ Generating future signals...")
        spawn_task(_run_future_signals_bg, uid, min_conf, start, end, None,
                   uid=uid, label="future_signals")
        context.user_data['state'] = None
...
def start_auto_signal_session(uid, strategy_id, context):
    ...
    spawn_task(auto_signal_loop, uid, strategy_id, context, my_session_token, uid=uid, label="auto_signal_loop", timeout=21600)
    ...
    while st.running:
        try:
            ...
            for pair in pairs:
                ...
                candles, price, payout = fetch_candles_mrbeaxt(pair, count=2000, timeframe=tf)
                if not candles:
                    continue
...
async def smz_pair_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "smz_pair_all":
        ...
        spawn_task(run_smz_hacking_mode, uid, 2, start_time, end_time, "M1", SMZ_ALL_PAIRS, uid=uid, label="smz_hacking_mode", timeout=600)
        context.user_data['smz_step'] = None
        return
    ...
    # Done button
        ...
        spawn_task(run_smz_hacking_mode, uid, 2, start_time, end_time, "M1", pairs_list, uid=uid, label="smz_hacking_mode", timeout=600)
        return
...
async def aifilter_conf_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    spawn_task(run_ai_filter_pattern_match, uid, signals_text, threshold, context, uid=uid, label="ai_filter_pattern_match")
...
async def _on_bot_error(update, context):
    ...
    try:
        ...
        if chat is not None:
            await context.bot.send_message(
                chat_id=chat.id,
                text="⚠️ Temporary error. Tap /start and try again.",
            )
    except Exception as e:
...
def main():
    ...
    # ---- Stuck-task watchdog (recovers users whose background task hung) ----
    threading.Thread(target=_stuck_task_watchdog, daemon=True, name="task-watchdog").start()

    # ---- HANDLERS (unchanged) ----
    app.add_handler(CommandHandler("start", start_cmd))
    app.add_handler(CallbackQueryHandler(_retry_task_callback, pattern="^retry_task:"))
